...
def get_3d_keypoints_vis(keypoints):
    ...
    joint_vis[:, :, :dim] = np.tile(np.expand_dims(keypoints_vis, axis=2), (1, dim))
...
def _load_test_data():
    ...
    try:
        keypoints3d, keypoints3d_vis = get_3d_keypoints_vis(data["keypoints2d"])
    except KeyError:
        keypoints3d, keypoints3d_vis = get_3d_keypoints_vis(data["keypoints3d"])
    ...
    try:
        ...
        keypoints3d_relative, _ = get_3d_keypoints_vis(data["keypoints3d_relative"])
        keypoints3d17, keypoints3d17_vis = get_3d_keypoints_vis(data["keypoints3d17"])
        keypoints3d17_relative, _ = get_3d_keypoints_vis(data["keypoints3d17_relative"])
    except KeyError:
        info["has_smpl"] = 0
    ...
    try:
        info["intrinsic_param"] = data["cam_param"]["intrinsic"][index].astype(np.float32)
    except KeyError:
    ...
    try:
        ...
        info["target_twist_weight"] = data["phi_weight"][index].astype(np.float32)
    except KeyError:
...
def test_hybrik_pipeline():
    ...
    # test random dpg
    transform = RandomDPG(dpg_prob=1.0)
    ...
    transform = GetRandomScaleRotation(rot_factor=30, scale_factor=0.25, rot_prob=1.0)
    results_scalerot = transform(copy.deepcopy(results))

    assert not np.equal(results_scalerot["rotation"], results["rotation"]).all()
    assert not np.equal(results_scalerot["scale"], results["scale"]).all()

    # test random occlusion
    transform = RandomOcclusion(occlusion_prob=1.0)
    ...
    # test no flip
    transform = HybrIKRandomFlip(flip_prob=0.0, flip_pairs=humandata_flip_pairs)
    ...
    # test flip
    transform = HybrIKRandomFlip(flip_prob=1.0, flip_pairs=humandata_flip_pairs)
    ...
    assert np.equal(results_flip["keypoints3d17_relative"], original_keypoints3d17_relative).all()
    assert np.equal(results_flip["keypoints3d17"], original_keypoints3d17).all()
    assert np.equal(results_flip["keypoints3d_relative"], original_keypoints3d_relative).all()
    assert np.equal(results_flip["pose"], original_pose).all()
    assert not np.equal(results_flip["keypoints3d"], original_keypoints3d).all()
    ...
    assert not np.equal(results_flip_smpl["target_twist"], original_twist).all()
    assert not np.equal(results_flip_smpl["keypoints3d17_relative"], original_keypoints3d_relative).all()
    assert not np.equal(results_flip_smpl["keypoints3d17"], original_keypoints3d17).all()
    assert not np.equal(results_flip_smpl["pose"], original_pose).all()
    assert not np.equal(results_flip_smpl["keypoints3d"], original_keypoints3d).all()
    assert not np.equal(results_flip_smpl["keypoints3d_relative"], original_keypoints3d_relative).all()

    # test keypoints selection
    h36m_idxs = [148, 145, 4, 7, 144, 5, 8, 150, 146, 152, 147, 16, 18, 20, 17, 19, 21]
    hybrik29_idxs = [
        0,
        1,
        2,
    ...
    keypoints_maps = [
        ...
        dict(keypoints=["keypoints3d", "keypoints3d_vis", "keypoints3d_relative"], keypoints_index=hybrik29_idxs),
    ]
    ...
    assert np.equal(results["keypoints3d17_vis"], np.ones([len(h36m_idxs), 3])).all()
    assert np.equal(results["keypoints3d_vis"], np.ones([len(hybrik29_idxs), 3])).all()
    ...
    assert np.equal(results_affine["keypoints3d17"], original_keypoints3d17).all()
    assert not np.equal(results_affine["keypoints3d"], original_keypoints3d).all()
...
def test_human_hybrik_dataset():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        res = custom_dataset.evaluate(outputs, res_folder=tmpdir, metric="mpjpe")
        assert "MPJPE" in res
        assert res["MPJPE"] > 0

        res = custom_dataset.evaluate(outputs, res_folder=tmpdir, metric="pa-3dpck")
        assert "PA-3DPCK" in res
        assert res["PA-3DPCK"] >= 0

        res = custom_dataset.evaluate(outputs, res_folder=tmpdir, metric="3dpck")
        assert "3DPCK" in res
        assert res["3DPCK"] >= 0

        res = custom_dataset.evaluate(outputs, res_folder=tmpdir, metric="pa-3dauc")
        assert "PA-3DAUC" in res
        assert res["PA-3DAUC"] >= 0

        res = custom_dataset.evaluate(outputs, res_folder=tmpdir, metric="3dauc")
        assert "3DAUC" in res
        assert res["3DAUC"] >= 0
