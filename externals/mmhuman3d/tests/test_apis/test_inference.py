...
def test_inference_image_based_model():
    ...
    mesh_results = inference_image_based_model(mesh_model, frames_iter, person_results)
...
def test_inference_video_based_model():
    ...
    feature_results_seq = extract_feature_sequence(person_results_list, frame_idx=0, causal=True, seq_len=16, step=1)

    inference_video_based_model(mesh_model, extracted_results=feature_results_seq, with_track_id=True)

    inference_video_based_model(mesh_model, extracted_results=feature_results_seq, with_track_id=True, causal=False)

    inference_video_based_model(mesh_model, extracted_results=feature_results_seq, with_track_id=False)
...
def test_process_mmdet_results():
    ...
    _ = process_mmdet_results(mmdet_results=(det_results, det_mask_results), cat_id=1, bbox_thr=0.9)

    _ = process_mmdet_results(mmdet_results=det_results, cat_id=1, bbox_thr=0.9)
...
def test_convert_crop_cam_to_orig_img():
    ...
    convert_crop_cam_to_orig_img(pred_cam, bbox, img_width, img_height, bbox_format="xywh")
    convert_crop_cam_to_orig_img(pred_cam, bbox, img_width, img_height, bbox_format="cs")
    with pytest.raises(ValueError):
        convert_crop_cam_to_orig_img(pred_cam, bbox, img_width, img_height, bbox_format="xy")
