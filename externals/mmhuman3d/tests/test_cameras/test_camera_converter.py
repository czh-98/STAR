...
def test_convert_cameras():
    for src in CAMERA_CONVENTIONS:
        for dst in CAMERA_CONVENTIONS:
            R = ee_to_rotmat(np.random.uniform(low=-np.pi, high=np.pi, size=(1, 3))).reshape(1, 3, 3)
            ...
            for is_perspective in [True, False]:
                ...
                check_isclose(
                    K=torch.Tensor(K), R=torch.Tensor(R), T=torch.Tensor(T), src=src, dst=dst, use_numpy=False
                )

                with pytest.raises(TypeError):
                    check_isclose(K=torch.Tensor(K), R=R, T=torch.Tensor(T), src=src, dst=dst, use_numpy=False)
...
def test_convert_K():
    opencv_K = np.array([[1.7, 0.0, 1.0], [0.0, 1.7, 2.0], [0.0, 0.0, 1.0]])
    assert np.isclose(opencv_K.all(), convert_K_4x4_to_3x3(convert_K_3x3_to_4x4(opencv_K)).all(), rtol=0, atol=1e-2)
    assert np.isclose(
        opencv_K.all(),
        convert_K_4x4_to_3x3(convert_K_3x3_to_4x4(opencv_K, "orthograophics"), "orthograophics").all(),
        rtol=0,
        atol=1e-2,
    )

    opencv_K = torch.Tensor(opencv_K)
    assert torch.isclose(opencv_K, convert_K_4x4_to_3x3(convert_K_3x3_to_4x4(opencv_K)), rtol=0, atol=1e-2).all()
    assert torch.isclose(
        opencv_K,
        convert_K_4x4_to_3x3(convert_K_3x3_to_4x4(opencv_K, "orthograophics"), "orthograophics"),
        rtol=0,
        atol=1e-2,
    ).all()
...
def test_convert_ndc_screen():
    ...
    for is_perspective in [True, False]:
        ...
        K1 = convert_ndc_to_screen(K, is_perspective=is_perspective, sign=sign, resolution=(1080, 1920))
        K2 = convert_screen_to_ndc(K1, is_perspective=is_perspective, sign=sign, resolution=(1080, 1920))
        assert np.isclose(K.all(), K2.all(), rtol=0, atol=eps)

        K1 = convert_ndc_to_screen(torch.Tensor(K), is_perspective=is_perspective, sign=sign, resolution=(1080, 1920))
        K2 = convert_screen_to_ndc(K1, is_perspective=is_perspective, sign=sign, resolution=(1080, 1920))
...
def test_camera_utils():
    ...
    _, _ = convert_smpl_from_opencv_calibration(
        R=R, T=T, transl=transl, poses=poses, resolution=resolution, model_path=model_path
    )
...
def test_convert_projection():
    ...
    K1 = convert_weakperspective_to_perspective(K=K, zmean=10, resolution=(1024, 1024), in_ndc=True)
    K2 = convert_perspective_to_weakperspective(K=K1, zmean=10, resolution=(1024, 1024), in_ndc=True)
    assert (K == K2).all()
