...
chessboard_path = "tests/data/camera/" + "calibration_chessboard_05_28_18_05_19.json"
dump_path = "tests/data/camera/" + "camera_parameter_dump.json"
...
def test_type():
    ...
    with pytest.raises(TypeError):
        cam_param.set_value("H", torch.ones(size=(3,), dtype=torch.float32)[0])
    with pytest.raises(TypeError):
        cam_param.set_value("H", torch.ones(size=(3,), dtype=torch.int32)[0:1])
