...
def check_camera_close(cam1, cam2, points=None, eps=1e-3):
    ...
    assert torch.isclose(
        cam1.transform_points_screen(points)[..., :2], cam2.transform_points_screen(points)[..., :2], rtol=0, atol=2e-1
    ).all()

    assert torch.isclose(
        cam1.compute_depth_of_points(points)[..., :1], cam2.compute_depth_of_points(points)[..., :1], rtol=0, atol=eps
    ).all()
...
def test_cameras():
    ...
    K_list = [None, torch.eye(4, 4)[None], torch.eye(4, 4)[None].repeat(10, 1, 1)]
    R_list = [None, torch.eye(3, 3)[None], torch.eye(3, 3)[None].repeat(10, 1, 1)]
    ...
    for camera_type in cameras_list:
        ...
        for idx in range(3):
            cam1 = build_cameras(
                dict(type=camera_type.__name__, K=K_list[idx], R=R_list[idx], T=T_list[idx], image_size=image_size)
            )
            cam2 = camera_type(K=K_list[idx], R=R_list[idx], T=T_list[idx], image_size=image_size)
            ...
            K, R, T = convert_camera_matrix(
                K=K_list[idx], R=R_list[idx], T=T_list[idx], convention_src="opencv", convention_dst="pytorch3d"
            )
            cam4 = build_cameras(
                dict(type=camera_type.__name__, K=K, R=R, T=T, convention="pytorch3d", image_size=image_size)
            )
            check_camera_close(cam3, cam4)

    for camera_type in [PerspectiveCameras, OrthographicCameras]:
        cam1 = camera_type(principal_point=((1000, 1000.0)), focal_length=1000, image_size=(1080, 1920), in_ndc=False)
        cam2 = build_cameras(
            dict(
                type=camera_type.__name__,
        ...
        cam1 = camera_type(
            principal_point=((1.73, 1.73)), focal_length=1.0, batch_size=1, image_size=(1080, 1920), in_ndc=True
        )
        cam1.extend_(100)
        cam2 = build_cameras(
            dict(
...
def test_weakperspective():
    ...
    K = cam.compute_projection_matrix(orig_cam[0, 0], orig_cam[0, 1], orig_cam[0, 2], orig_cam[0, 3], 1)
...
def test_direction_cameras():
    ...
    K1, R1, T1 = compute_direction_cameras(eye=(0, 0, 0), dist=math.sqrt(2), z_vec=(1, 0, 1))
    K2, R2, T2 = compute_direction_cameras(at=(1, 0, 1), dist=math.sqrt(2), z_vec=(1, 0, 1))
    K3, R3, T3 = compute_direction_cameras(at=(1, 0, 1), dist=math.sqrt(2), plane=((0, 1, 0), (1, 0, -1)))
    K4, R4, T4 = compute_direction_cameras(at=(1, 0, 1), eye=(0, 0, 0))

    cam1 = build_cameras(dict(type="perspective", in_ndc=True, image_size=256, K=K1, R=R1, T=T1))
    cam2 = build_cameras(dict(type="perspective", in_ndc=True, image_size=256, K=K2, R=R2, T=T2))
    cam3 = build_cameras(dict(type="perspective", in_ndc=True, image_size=256, K=K3, R=R3, T=T3))
    cam4 = build_cameras(dict(type="perspective", in_ndc=True, image_size=256, K=K4, R=R4, T=T4))
