...
def test_set(HumanDataCls):
    ...
    with pytest.raises(KeyError):
        human_data["keypoints4d_mask"] = np.ones(
            shape=[
                144,
            ]
        )
...
def test_compression(HumanDataCls):
    ...
    assert shape_equal(human_data.get_raw_value("keypoints2d"), sample_keypoints2d)
...
def test_generate_mask_from_keypoints(HumanDataCls):
    ...
    human_data.generate_mask_from_confidence(keys=["keypoints2d", "keypoints3d"])
...
def test_padding_and_slice(HumanDataCls):
    ...
    # test padding
    padding_value = human_data.get_value_in_shape("keypoints2d", [200, 19, 4])
    assert padding_value[199, 18, 3] == 0
    # test padding customed value
    padding_value = human_data.get_value_in_shape("keypoints2d", [200, 19, 4], 1)
    assert padding_value[199, 18, 3] == 1
    # test slice
    slice_value = human_data.get_value_in_shape("keypoints2d", [100, 10, 2])
    assert raw_value[0, 0, 0] == slice_value[0, 0, 0]
    # test padding + slice
    target_value = human_data.get_value_in_shape("keypoints2d", [200, 10, 2])
    assert target_value[199, 9, 1] == 0
    assert raw_value[0, 0, 0] == target_value[0, 0, 0]
    # test default dim
    target_value = human_data.get_value_in_shape("keypoints2d", [-1, 10, 2])
...
def test_slice(HumanDataCls):
    ...
    assert sliced_human_data["misc"]["hd_id_plus_1"] == human_data["misc"]["hd_id_plus_1"]
...
def test_multi_human_data_slice(HumanDataCls):
    ...
    assert sliced_human_data["keypoints2d"].shape[0] == 3
    assert sliced_human_data.get_raw_value("keypoints2d")[2, 0, 0] == raw_value[2, 0, 0]
    # slice with start and stop
    sliced_human_data = human_data.get_slice(1, 4)
    assert sliced_human_data["keypoints2d"].shape[0] == 5
    assert sliced_human_data.get_raw_value("keypoints2d")[-1, 0, 0] == raw_value[6, 0, 0]
    # slice with start, stop and step
    sliced_human_data = human_data.get_slice(0, 5, 2)
    assert sliced_human_data["keypoints2d"].shape[0] == 5
    assert sliced_human_data.get_raw_value("keypoints2d")[-1, 0, 0] == raw_value[8, 0, 0]
...
def test_load(HumanDataCls):
    ...
    human_data["image_path"] = ["1.jpg", "2.jpg"]
    human_data_with_image_path = "tests/data/human_data/human_data_img.npz"
...
def test_construct_from_dict(HumanDataCls):
    ...
    assert human_data["keypoints2d"].shape[2] == 3
    # strict==True does not allow unsupported keys
    human_data = HumanDataCls.new(source_dict=dict_with_keypoints2d, key_strict=False)
    assert human_data["keypoints2d"].shape[2] == 3
    # strict==False allows unsupported keys
    dict_with_keypoints4d = {"keypoints4d": np.zeros(shape=[3, 144, 5])}
    human_data = HumanDataCls.new(source_dict=dict_with_keypoints4d, key_strict=False)
...
def test_dump(HumanDataCls):
    ...
    if check_path_existence(human_data_dump_path, "file") == Existence.FileExist:
        os.remove(human_data_dump_path)
    human_data.dump(human_data_dump_path, overwrite=True)
    assert check_path_existence(human_data_dump_path, "file") == Existence.FileExist
    ...
    old_version_dict = {"bbox_xywh": np.zeros(shape=(3, 4)), "image_path": None}
    ...
    with pytest.raises(ValueError):
        human_data.dump(human_data_dump_path.replace(".npz", ".jpg"), overwrite=True)
    # file exists
...
def test_dump_by_pickle(HumanDataCls):
    ...
    # wrong file extension
    with pytest.raises(ValueError):
        human_data.dump_by_pickle(human_data_dump_path.replace(".pkl", ".jpg"), overwrite=True)
    # file exists
    with pytest.raises(FileExistsError):
...
def test_concat(HumanDataCls):
    ...
    assert cat_human_data["keypoints2d"].shape[1:] == human_data_0["keypoints2d"].shape[1:]
    assert cat_human_data["image_path"][2] == human_data_1["image_path"][0]
    assert cat_human_data["keypoints2d"][2, 0, 0] == human_data_1["keypoints2d"][0, 0, 0]
    # test different mask
    human_data_1 = HumanDataCls()
    human_data_1["image_path"] = ["path_2"]
    human_data_1["keypoints2d"] = np.ones(shape=(1, 190, 3))
    ...
    assert cat_human_data["keypoints3d_1"].shape == human_data_1["keypoints3d"].shape
    ...
    cat_human_data = HumanDataCls.concatenate(human_data_0, human_data_1)
    assert cat_human_data["smpl"]["body_pose"][2, 0, 0] == human_data_1["smpl"]["body_pose"][0, 0, 0]
    assert cat_human_data["smpl"]["transl"][1, 0] == human_data_0["smpl"]["transl"][1, 0]
    assert cat_human_data["smpl"]["betas"][2, 1] == human_data_1["smpl"]["betas"][0, 1]
