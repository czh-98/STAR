...
def _setup_parser():
    ...
    parser.add_argument("--device", default="cpu", help="device used for testing")
...
def test_pymafx_inference_with_openpifpaf():
    ...
    for bhf_mode, global_mode in [("full_body", True), ("body_hand", False)]:
        ...
        mesh_model, _ = init_model(config, args.mesh_reg_checkpoint, device=args.device)
        args.device = torch.device(args.device)
        args.pin_memory = False
        # Prepare input
        frames_iter = prepare_frames(args.input_path)
        joints2d, frames, wb_kps, image_folder, _ = prepare_data_with_pifpaf_detection(args, frames_iter)
