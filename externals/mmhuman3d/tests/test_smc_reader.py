...
def test_get_calibration_dict():
    ...
    assert "0" in calibration_dict.keys(), "Calibration should contain at least a kinect"
...
def test_get_kinect_extrinsics():
    ...
    assert color_extrinsics.shape == (4, 4), "Kinect Color Extrinsic should be a matrix with shape 4x4"

    color_extrinsics = smc.get_kinect_color_extrinsics(0, homogeneous=False)
    assert color_extrinsics["R"].shape == (3, 3), "Kinect Color R should be a matrix with shape 3x3"
    assert color_extrinsics["T"].shape == (3,), "Kinect Color T should be a matrix with shape 3"

    depth_extrinsics = smc.get_kinect_depth_extrinsics(0, homogeneous=True)
    assert depth_extrinsics.shape == (4, 4), "Kinect Depth Extrinsic should be a matrix with shape 4x4"

    depth_extrinsics = smc.get_kinect_depth_extrinsics(0, homogeneous=False)
    assert depth_extrinsics["R"].shape == (3, 3), "Kinect Depth R should be a matrix with shape 3x3"
    assert depth_extrinsics["T"].shape == (3,), "Kinect Depth T should be a matrix with shape 3"
...
def test_get_kinect_intrinsics():
    ...
    assert color_intrinsics.shape == (3, 3), "Kinect Color Intrinsic should be a matrix with shape 3x3"

    depth_intrinsics = smc.get_kinect_depth_intrinsics(0)
    assert depth_intrinsics.shape == (3, 3), "Kinect Depth Intrinsic should be a matrix with shape 3x3"
...
def test_get_kinect_resolution():
    ...
    assert color_resolution.shape == (2,), "Kinect Color Resolution should be a 2D matrix"
    assert tuple(color_resolution) == (1920, 1080)

    depth_resolution = smc.get_kinect_depth_resolution(0)
    assert depth_resolution.shape == (2,), "Kinect Depth Resolution should be a 2D matrix"
...
def test_get_iphone_extrinsics():
    ...
    assert iphone_extrinsics.shape == (4, 4), "Iphone Color Extrinsic should be a matrix with shape 4x4"
...
def test_get_iphone_intrinsics():
    ...
    assert iphone_intrinsics.shape == (3, 3), "Iphone Color Intrinsic should be a matrix with shape 3x3"
...
def test_get_iphone_resolution():
    ...
    assert color_resolution.shape == (2,), "iPhone Color Resolution should be a 2D matrix"
...
def test_get_iphone_color():
    ...
    assert iphone_color.shape == (1, 1920, 1440, 3), (
        "iPhone color image in vertical mode " "should have shape 1x1920x1440x3."
    )

    iphone_color = smc.get_iphone_color(iphone_id=0, frame_id=0, vertical=False)
    assert iphone_color.shape == (1, 1440, 1920, 3), (
        "iPhone color image in horizontal mode " "should have shape 1x1440x1920x3."
    )
...
def test_get_iphone_depth():
    ...
    assert iphone_depth.shape == (1, 256, 192), "iPhone depth image in vertical mode should have shape 1x256x192."

    iphone_depth = smc.get_iphone_depth(iphone_id=0, frame_id=0, vertical=False)
    assert iphone_depth.shape == (1, 192, 256), "iPhone depth image in horizontal mode should have shape 1x192x256."
...
def test_get_kinect_color():
    ...
    assert kinect_color.shape == (1, 1080, 1920, 3), "Kinect Color should have resolution of 1x1080x1920x3"
...
def test_get_kinect_depth():
    ...
    assert kinect_depth.shape == (1, 576, 640), "Kinect depth should have resolution of 1x576x640"
...
def test_get_kinect_rgbd():
    ...
    assert mapped_color.shape == (576, 640, 3), "Kinect RGBD should have color resolution of 576x640x3"
...
def test_get_mask():
    ...
    assert depth_mask.shape == (576, 640), "depth mask should have shape 576x640"

    kinect_mask = smc.get_kinect_mask(0, 0)
    assert kinect_mask.shape == (576, 640), "Kinect mask should have shape 576x640"
...
def test_get_kinect_transformation():
    ...
    assert trans_d2c.shape == (4, 4), "Depth to Color Transformation matrix should have shape 4x4"

    trans_c2d = smc.get_kinect_transformation_color_to_depth(0)
    assert trans_c2d.shape == (4, 4), "Color to Depth Transformation matrix should have shape 4x4"
...
def test_get_depth_floor():
    ...
    assert "center" in depth_floor.keys(), "center should be present in depth floor parameters"
...
def test_get_kinect_keypoints2d_by_frame():
    ...
    keypoints2d, keypoints2d_mask = smc.get_kinect_keypoints2d(device_id=1, frame_id=0)
    assert keypoints2d.shape == (1, 133, 3)
    assert keypoints2d_mask.shape == (133,)
    assert isinstance(keypoints2d, np.ndarray)
    assert isinstance(keypoints2d_mask, np.ndarray)
...
def test_get_kinect_keypoints2d_by_frames():
    ...
    with pytest.raises(AssertionError):
        keypoints2d, keypoints2d_mask = smc.get_kinect_keypoints2d(device_id=100)
...
def test_get_iphone_keypoints2d_by_frame():
    ...
    keypoints2d, keypoints2d_mask = smc.get_iphone_keypoints2d(device_id=0, frame_id=0)
    assert keypoints2d.shape == (1, 133, 3)
    assert keypoints2d_mask.shape == (133,)
    assert isinstance(keypoints2d, np.ndarray)
    assert isinstance(keypoints2d_mask, np.ndarray)
...
def test_get_keypoints3d_by_device():
    ...
    keypoints3d, keypoints3d_mask = smc.get_keypoints3d(device="iPhone", device_id=0, frame_id=0)
    assert keypoints3d.shape == (1, 133, 4)
    assert keypoints3d_mask.shape == (133,)
    assert isinstance(keypoints3d, np.ndarray)
    assert isinstance(keypoints3d_mask, np.ndarray)
...
def test_iphone_rotation():
    ...
    cam2world = smc.get_iphone_extrinsics(homogeneous=True, vertical=True)
    extrinsics = np.linalg.inv(cam2world)

    # transform keypoints3d to vertical iPhone
    keypoints3d = np.concatenate([keypoints3d, np.ones([*keypoints3d.shape[:-1], 1])], axis=-1)  # homogeneous
