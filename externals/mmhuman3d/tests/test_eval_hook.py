...
def test_eval_hook(EvalHookCls):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        runner = EpochBasedRunner(
            model=model, batch_processor=None, optimizer=optimizer, work_dir=tmpdir, logger=logger, max_epochs=1
        )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        logger = get_logger("test_eval")
        runner = EpochBasedRunner(
            model=model, batch_processor=None, optimizer=optimizer, work_dir=tmpdir, logger=logger, max_epochs=8
        )
    ...
    data_loader = DataLoader(EvalDataset(), batch_size=1)
    eval_hook = EvalHookCls(data_loader, key_indicator="mpjpe", save_best=True, rule="less")
    with tempfile.TemporaryDirectory() as tmpdir:
        logger = get_logger("test_eval")
        runner = EpochBasedRunner(
    ...
    data_loader = DataLoader(EvalDataset(), batch_size=1)
    eval_hook = EvalHookCls(data_loader, interval=1, save_best="pve", rule="less")
    with tempfile.TemporaryDirectory() as tmpdir:
        logger = get_logger("test_eval")
        runner = EpochBasedRunner(
            model=model, batch_processor=None, optimizer=optimizer, work_dir=tmpdir, logger=logger
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        logger = get_logger("test_eval")
        runner = EpochBasedRunner(
            model=model, batch_processor=None, optimizer=optimizer, work_dir=tmpdir, logger=logger, max_epochs=2
        )
        runner.register_checkpoint_hook(dict(interval=1))
        runner.register_hook(eval_hook)
        runner.run([loader], [("train", 1)])
