...
def test_human_image_smplx_dataset():
    train_dataset = HumanImageSMPLXDataset(
        data_prefix="tests/data", pipeline=[], dataset_name="EHF", ann_file="sample_ehf_val.npz"
    )
    data_keys = [
        "img_prefix",
        "image_path",
    ...
    test_dataset = HumanImageSMPLXDataset(
        ...
        pipeline=[],
        dataset_name="EHF",
        body_model=dict(type="smplx", keypoint_src="smplx", keypoint_dst="smplx", model_path="data/body_models/smplx"),
    ...
    res = test_dataset.evaluate(outputs, res_folder="tests/data", metric="pa-mpjpe")
    assert "PA-MPJPE" in res
    assert res["PA-MPJPE"] > 0

    res = test_dataset.evaluate(outputs, res_folder="tests/data", metric="mpjpe")
    assert "MPJPE" in res
    assert res["MPJPE"] > 0

    res = test_dataset.evaluate(outputs, res_folder="tests/data", metric="pa-pve")
    assert "PA-PVE" in res
    assert res["PA-PVE"] > 0

    res = test_dataset.evaluate(
        outputs, res_folder="tests/data", metric=["pa-mpjpe"], body_part=[["body", "right_hand", "left_hand"]]
    )
    ...
    res = test_dataset.evaluate(
        outputs, res_folder="tests/data", metric=["pa-pve"], body_part=[["", "right_hand", "left_hand", "face"]]
    )
    assert "PA-PVE" in res
...
def test_pipeline():
    train_dataset = HumanImageSMPLXDataset(
        data_prefix="tests/data", pipeline=[], dataset_name="3dpw", ann_file="sample_3dpw_train.npz", convention="smplx"
    )
    info = train_dataset.prepare_raw_data(0)

    info["keypoints2d"] = np.random.rand(*info["keypoints2d"].shape).astype(np.float32)
    info["keypoints3d"] = np.random.rand(*info["keypoints3d"].shape).astype(np.float32)
    info["smplx_body_pose"] = np.random.rand(*info["smplx_body_pose"].shape).astype(np.float32)
    ...
    transform = RandomHorizontalFlip(flip_prob=0.0, convention="smplx")
    ...
    assert np.equal(results_no_flip["smplx_body_pose"], original_body_pose).all()

    # test flip
    transform = RandomHorizontalFlip(flip_prob=1.0, convention="smplx")
    ...
    assert not np.equal(results_flip["keypoints2d"], original_keypoints2d).all()
    assert not np.equal(results_flip["smplx_body_pose"], original_body_pose).all()
    ...
    assert not np.equal(results_rotated["keypoints3d"], original_keypoints3d).all()
    assert not np.equal(results_rotated["keypoints2d"], original_keypoints2d).all()
    assert np.equal(results_rotated["smplx_body_pose"], original_body_pose).all()
    ...
    assert not np.equal(results_affine["keypoints2d"], original_keypoints2d).all()
    assert np.equal(results_affine["smplx_body_pose"], original_body_pose).all()
    ...
    assert not np.equal(results_noised["ori_img"], results_affine["ori_img"]).all()

    # test low resolution
    transform = SimulateLowRes(dist="categorical", cat_factors=(1.2, 1.5, 2.0, 3.0, 4.0, 8.0))
