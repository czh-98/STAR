...
def test_human_image_dataset():
    ...
    train_dataset = HumanImageDataset(
        data_prefix="tests/data", pipeline=[], dataset_name="h36m", ann_file="sample_3dpw_test.npz"
    )
    ...
    test_dataset = HumanImageDataset(
        ...
        pipeline=[],
        dataset_name="pw3d",
        body_model=dict(type="SMPL", keypoint_src="smpl_45", keypoint_dst="h36m", model_path="data/body_models/smpl"),
    ...
    res = test_dataset.evaluate(outputs, res_folder="tests/data", metric="pa-3dpck")
    assert "PA-3DPCK" in res
    assert res["PA-3DPCK"] >= 0

    res = test_dataset.evaluate(outputs, res_folder="tests/data", metric="3dpck")
    assert "3DPCK" in res
    assert res["3DPCK"] >= 0

    res = test_dataset.evaluate(outputs, res_folder="tests/data", metric="pa-3dauc")
    assert "PA-3DAUC" in res
    assert res["PA-3DAUC"] >= 0

    res = test_dataset.evaluate(outputs, res_folder="tests/data", metric="3dauc")
    assert "3DAUC" in res
    assert res["3DAUC"] >= 0
    ...
    test_dataset = HumanImageDataset(
        ...
        dataset_name="pw3d",
        body_model=dict(
            type="SMPL", keypoint_src="smpl_45", keypoint_dst="smpl_24", model_path="data/body_models/smpl"
        ),
        ann_file="sample_3dpw_test.npz",
    )
    ...
    test_dataset = HumanImageDataset(
        ...
        dataset_name="pw3d",
        body_model=dict(
            type="SMPL", keypoint_src="smpl_45", keypoint_dst="smpl_49", model_path="data/body_models/smpl"
        ),
        ann_file="sample_3dpw_test.npz",
    )
    test_dataset.num_data = 1
...
def test_pipeline():
    train_dataset = HumanImageDataset(
        data_prefix="tests/data", pipeline=[], dataset_name="3dpw", ann_file="sample_3dpw_train.npz"
    )
    ...
    transform = RandomHorizontalFlip(flip_prob=0.0, convention="smpl_54")
    ...
    assert np.equal(results_no_flip["smpl_body_pose"], original_body_pose).all()

    # test flip
    transform = RandomHorizontalFlip(flip_prob=1.0, convention="smpl_54")
    ...
    assert not np.equal(results_flip_smpl["keypoints2d"], original_keypoints2d).all()
    assert not np.equal(results_flip_smpl["smpl_global_orient"], original_body_pose).all()
    ...
    assert not np.equal(results_affine["smpl_global_orient"], original_body_pose).all()


def test_human_image_dataset_smc():
    # test loading smc
    train_dataset = HumanImageDataset(
        data_prefix="tests/data", pipeline=[], dataset_name="humman", ann_file="sample_humman_test_iphone_ds10.npz"
    )
