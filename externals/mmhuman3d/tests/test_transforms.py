...
def check_func(var1, var2, correct_var2_shape=None):
    ...
    if correct_var2_shape:
        ...
        bool_shape = True
    return bool_type and bool_dtype and bool_grad and bool_device and bool_shape
...
def test_transforms_tensor():
    ...
    for _ in range(10):
        # test convention
        assert check_func(aa, aa_to_ee(aa, convention="xyz"), aa.shape[:-1] + (3,))
        assert check_func(aa, aa_to_ee(aa, convention="xzy"), aa.shape[:-1] + (3,))
        assert check_func(aa, aa_to_ee(aa, convention="yxz"), aa.shape[:-1] + (3,))
        assert check_func(aa, aa_to_ee(aa, convention="yzx"), aa.shape[:-1] + (3,))
        assert check_func(aa, aa_to_ee(aa, convention="zyx"), aa.shape[:-1] + (3,))
        assert check_func(aa, aa_to_ee(aa, convention="zxy"), aa.shape[:-1] + (3,))
        ...
        # test ee convention
        assert check_func(ee, ee_to_aa(ee, convention="xyz"), ee.shape[:-1] + (3,))
        assert check_func(ee, ee_to_rotmat(ee, convention="xyz"), ee.shape[:-1] + (3, 3))
        assert check_func(ee, ee_to_quat(ee, convention="xyz"), ee.shape[:-1] + (4,))
        assert check_func(ee, ee_to_rot6d(ee, convention="xyz"), ee.shape[:-1] + (6,))
        ...
        assert check_func(quat, quat_to_ee(quat, convention="xyz"), quat.shape[:-1] + (3,))
        assert check_func(quat, quat_to_rot6d(quat), quat.shape[:-1] + (6,))
        # test rotmat convention
        assert check_func(rotmat, rotmat_to_aa(rotmat), rotmat.shape[:-2] + (3,))
        assert check_func(rotmat, rotmat_to_ee(rotmat, convention="xyz"), rotmat.shape[:-2] + (3,))
        assert check_func(rotmat, rotmat_to_quat(rotmat), rotmat.shape[:-2] + (4,))
        assert check_func(rotmat, rotmat_to_rot6d(rotmat), rotmat.shape[:-2] + (6,))
        # test rot6d convention
        assert check_func(rot6d, rot6d_to_aa(rot6d), rot6d.shape[:-1] + (3,))
        assert check_func(rot6d, rot6d_to_rotmat(rot6d), rot6d.shape[:-1] + (3, 3))
        assert check_func(rot6d, rot6d_to_quat(rot6d), rot6d.shape[:-1] + (4,))
        assert check_func(rot6d, rot6d_to_ee(rot6d, convention="xyz"), rot6d.shape[:-1] + (3,))
        # test standard joint angle convention
        assert check_func(aa_21, aa_to_sja(aa_21, R_t=R_t, R_t_inv=R_t_inv), aa_21.shape[:-2] + (21, 3))
        assert check_func(sja, sja_to_aa(sja, R_t=R_t, R_t_inv=R_t_inv), sja.shape[:-2] + (21, 3))

        # test inverse transform
        assert (aa == ee_to_aa(aa_to_ee(aa, convention="xyz"), convention="xyz")).all()
        assert (aa == rotmat_to_aa(aa_to_rotmat(aa))).all()
        assert (aa == quat_to_aa(aa_to_quat(aa))).all()
        assert (aa == rot6d_to_aa(aa_to_rot6d(aa))).all()

        assert (ee == aa_to_ee(ee_to_aa(ee, convention="xyz"), convention="xyz")).all()
        assert (ee == rotmat_to_ee(ee_to_rotmat(ee, convention="xyz"), convention="xyz")).all()
        assert (ee == quat_to_ee(ee_to_quat(ee, convention="xyz"), convention="xyz")).all()
        assert (ee == rot6d_to_ee(ee_to_rot6d(ee, convention="xyz"), convention="xyz")).all()

        assert (quat == ee_to_quat(quat_to_ee(quat, convention="xyz"), convention="xyz")).all()
        ...
        assert (rotmat == ee_to_rotmat(rotmat_to_ee(rotmat, convention="xyz"), convention="xyz")).all()
        ...
        assert (rot6d == ee_to_rot6d(rot6d_to_ee(rot6d, convention="xyz"), convention="xyz")).all()
        ...
        assert (aa_21 == sja_to_aa(aa_to_sja(aa_21, R_t=R_t, R_t_inv=R_t_inv), R_t=R_t, R_t_inv=R_t_inv)).all()
        assert (sja == aa_to_sja(sja_to_aa(sja, R_t=R_t, R_t_inv=R_t_inv), R_t=R_t, R_t_inv=R_t_inv)).all()
