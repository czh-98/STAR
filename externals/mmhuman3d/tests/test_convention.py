...
def test_conventions():
    ...
    for src_name in data_names:
        for dst_name in data_names:
            ...
            for keypoints in [
                ...
                # with mask
                keypoints_dst, mask = convert_kps(keypoints, src_name, dst_name)

                # without mask
                keypoints_dst_wo_mask = convert_kps(keypoints, src_name, dst_name, return_mask=False)
    ...
    _, mask_coco = convert_kps(keypoints=keypoints, mask=original_mask, src="smpl", dst="coco")
    _, mask_coco_full = convert_kps(keypoints=keypoints, src="smpl", dst="coco")
    ...
    keypoints = np.zeros((1, len(KEYPOINTS_FACTORY["smpl"]), 3))
    _, mask_coco = convert_kps(keypoints=keypoints, src="smpl", dst="coco", approximate=False)
    _, approximate_mask_coco = convert_kps(keypoints=keypoints, src="smpl", dst="coco", approximate=True)
    assert mask_coco[KEYPOINTS_FACTORY["coco"].index("left_hip_extra")] == 0
    assert approximate_mask_coco[KEYPOINTS_FACTORY["coco"].index("left_hip_extra")] == 1

    assert len(KEYPOINTS_FACTORY["human_data"]) == len(set(KEYPOINTS_FACTORY["human_data"]))


def test_duplicate_name():
    for data_name in KEYPOINTS_FACTORY:
        assert len(KEYPOINTS_FACTORY[data_name]) == len(set(KEYPOINTS_FACTORY[data_name]))
...
def test_cache():
    ...
    for dst_key in KEYPOINTS_FACTORY:
        convert_kps(keypoints=coco_wb_keypoints2d, mask=coco_wb_mask, src="coco_wholebody", dst=dst_key)
    ...
    if with_cache_time > without_cache_time:
        warnings.warn(
            ...
            "if convert_kps has not been modified.",
            UserWarning,
        )


def test_get_flip_pairs():
    stable_conventions = ["coco", "smpl", "smplx", "mpi_inf_3dhp", "openpose_25"]
...
def test_get_keypoint_idx():
    ...
    smpl_49_idxs = [get_keypoint_idx(keypoint_name, "smpl_49") for keypoint_name in keypoint_names]

    assert smpl_49_idxs == [1, 5, 2, 12, 9]

    smpl_45_idxs = [get_keypoint_idx(keypoint_name, "smpl_45", approximate=True) for keypoint_name in keypoint_names]
...
def test_mano_reorder():
    ...
    # test left hand
    GT_MANO_LEFT_REORDER_KEYPOINTS = [x.replace("right_", "left_") for x in GT_MANO_RIGHT_REORDER_KEYPOINTS]
