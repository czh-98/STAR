...
from mmhuman3d.utils.mesh_utils import join_batch_meshes_as_scene as join_batch_meshes_as_scene_
...
def test_parametric_meshes_ops():
    ...
    assert meshes2.verts_padded().shape == (2, meshes2.model_class.NUM_VERTS, 3)
    meshes3 = meshes.extend(3, 4)
    assert meshes3.verts_padded().shape == (3, meshes2.model_class.NUM_VERTS * 4, 3)
    meshes4 = meshes3.to(device_name)
    assert meshes4[:2, :3].shape == (2, 3)

    meshes5 = join_batch_meshes_as_scene([meshes.extend(2, 3), meshes.extend(2, 2)])
    assert meshes5.shape == (2, 5)
    assert all(mesh.model_type == mesh.model_type for mesh in [meshes2, meshes3, meshes4, meshes5])
    assert all(
...
def test_parametric_meshes_ops_uv():
    ...
    full_pose = torch.Tensor(1, 72).to(device_name)
    pose_dict = body_model.tensor2dict(full_pose)
    uv_renderer = build_renderer(dict(type="uv", device=device_name, uv_param_path="data/body_models/smpl/smpl_uv.npz"))
    meshes = ParametricMeshes(
        body_model=body_model, uv_renderer=uv_renderer, texture_images=torch.ones(100, 100, 3), **pose_dict
    )
...
def test_save_meshes():
    ...
    save_meshes_as_plys(files=[os.path.join(output_dir, "test.ply")], meshes=Torus)
    mesh = load_plys_as_meshes(files=[os.path.join(output_dir, "test.ply")])
    assert mesh.verts_padded().shape == Torus.verts_padded().shape
    save_meshes_as_plys(
        files=os.path.join(output_dir, "test.ply"), verts=Torus.verts_padded(), faces=Torus.faces_padded()
    )
    save_meshes_as_plys(
    ...
    save_meshes_as_objs(files=os.path.join(output_dir, "test.obj"), meshes=Torus)
    Torus2 = Torus.extend(2)
    Torus2_2 = join_batch_meshes_as_scene_([Torus2, Torus2])
    assert Torus2_2.verts_padded().shape == (2, Torus.verts_padded().shape[1] * 2, 3)
