...
def test_human_data_preprocess():
    ...
    cfg = dict(
        type="H36mConverter", modes=["train", "valid"], protocol=1, mosh_dir="tests/data/dataset_sample/h36m_mosh"
    )
    data_converter = build_data_converter(cfg)
    ...
    cfg = dict(type="AgoraConverter", modes=["train", "validation"], fit="smplx")
    data_converter = build_data_converter(cfg)
    ...
    cfg = dict(type="EftConverter", modes=["coco_all", "coco_part", "mpii", "lspet"])
    data_converter = build_data_converter(cfg)
    ...
    cfg = dict(type="CrowdposeConverter", modes=["train", "val", "test", "trainval"])
    data_converter = build_data_converter(cfg)
    ...
    assert os.path.exists("/tmp/preprocessed_npzs/" + "hybrik_h36m_valid_protocol2.npz")

    cfg = dict(type="MpiInf3dhpHybrIKConverter", modes=["train", "test"])
    data_converter = build_data_converter(cfg)
    data_converter.convert(HYBRIK_ROOT, output_path)
    assert os.path.exists("/tmp/preprocessed_npzs/" + "hybrik_mpi_inf_3dhp_train.npz")
    assert os.path.exists("/tmp/preprocessed_npzs/" + "hybrik_mpi_inf_3dhp_test.npz")
    ...
    assert os.path.exists("/tmp/preprocessed_npzs/" + "hybrik_coco_2017_train.npz")
    ...
    cfg = dict(type="VibeConverter", modes=["mpi_inf_3dhp", "pw3d"], pretrained_ckpt=None)
    data_converter = build_data_converter(cfg)
    data_converter.convert(VIBE_ROOT, output_path)
    assert os.path.exists("/tmp/preprocessed_npzs/" + "vibe_mpi_inf_3dhp_train.npz")
    assert os.path.exists("/tmp/preprocessed_npzs/" + "vibe_pw3d_test.npz")

    SPIN_ROOT = os.path.join(root_path, "spin_data")
    cfg = dict(type="SpinConverter", modes=["coco_2014", "lsp", "mpii", "mpi_inf_3dhp", "lspet"])
    data_converter = build_data_converter(cfg)
    data_converter.convert(SPIN_ROOT, output_path)
    assert os.path.exists("/tmp/preprocessed_npzs/" + "spin_coco_2014_train.npz")
    assert os.path.exists("/tmp/preprocessed_npzs/" + "spin_lsp_train.npz")
    assert os.path.exists("/tmp/preprocessed_npzs/" + "spin_mpi_inf_3dhp_train.npz")
    ...
    assert os.path.exists("/tmp/preprocessed_npzs/humman_train_kinect_ds10_smpl.npz")
    assert os.path.exists("/tmp/preprocessed_npzs/humman_test_kinect_ds10_smpl.npz")
    assert os.path.exists("/tmp/preprocessed_npzs/humman_train_iphone_ds10_smpl.npz")
    assert os.path.exists("/tmp/preprocessed_npzs/humman_test_iphone_ds10_smpl.npz")
    ...
    data_converter = build_data_converter(cfg)
    data_converter.convert(FREIHAND_ROOT, output_path, mean_pose_path="data/body_models/smplx/all_means.pkl")
...
def test_multi_human_data_preprocess():
    ...
    data_converter = build_data_converter(cfg)
    data_converter.convert(H36M_ROOT, output_path, enable_multi_human_data=True)
    cfg = dict(type="H36mConverter", modes=["valid"], protocol=2)
    data_converter = build_data_converter(cfg)
    data_converter.convert(H36M_ROOT, output_path, enable_multi_human_data=True)
    cfg = dict(type="H36mConverter", modes=["train"], protocol=1)
    ...
    cfg = dict(type="CocoConverter")
    data_converter = build_data_converter(cfg)
    data_converter.convert(COCO_ROOT, output_path, enable_multi_human_data=True)
    assert osp.exists(osp.join(output_path, "coco_2014_train.npz"))
    ...
    cfg = dict(type="CrowdposeConverter", modes=["train", "val", "test", "trainval"])
    data_converter = build_data_converter(cfg)
    data_converter.convert(CROWDPOSE_ROOT, output_path, enable_multi_human_data=True)
    assert os.path.exists("/tmp/preprocessed_npzs/" + "crowdpose_val.npz")
    assert os.path.exists("/tmp/preprocessed_npzs/" + "crowdpose_train.npz")
    assert os.path.exists("/tmp/preprocessed_npzs/" + "crowdpose_test.npz")
    assert os.path.exists("/tmp/preprocessed_npzs/" + "crowdpose_trainval.npz")
    ...
    data_converter = build_data_converter(cfg)
    data_converter.convert(CLIFF_ROOT, output_path, enable_multi_human_data=True)
...
def test_preprocessed_npz():
    ...
    all_keys = [
        ...
        "root_cam",
        "depth_factor",
        "keypoints3d17_cam_mask",
        "keypoints3d_cam_mask",
        "keypoints3d17_mask",
        "keypoints3d17_relative_mask",
        "keypoints3d_relative",
        "keypoints3d17_cam",
        "keypoints3d17",
        "keypoints3d17_relative",
        "keypoints3d_cam",
        "keypoints3d_relative_mask",
        "phi",
        "phi_weight",
        "features",
        "has_smpl",
        "keypoints2d_gta",
        "keypoints3d_gta",
        "keypoints2d_gta_mask",
        "keypoints3d_gta_mask",
        "image_id",
        "keypoints2d_humman",
        "keypoints3d_humman",
    ...
    for npf in os.listdir(npz_folder):
        ...
        for k in npfile.files:
            ...
            if k == "image_path":
                ...
                smpl_keys = ["body_pose", "global_orient", "betas", "transl", "thetas"]
                ...
                smplh_keys = ["body_pose", "global_orient", "betas", "transl", "left_hand_pose", "right_hand_pose"]
                ...
                for smplx_key in smplx_dict.keys():
                    ...
                    if smplx_key == "body_pose":
                        ...
                        assert smplx_dict[smplx_key].shape == (N, 10) or smplx_dict[smplx_key].shape == (N, 100)
                        ...
                        assert smplx_dict[smplx_key].shape == (N, 10) or smplx_dict[smplx_key].shape == (N, 50)
