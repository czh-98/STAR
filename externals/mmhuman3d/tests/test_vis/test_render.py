...
def test_render_runner():
    ...
    resolution = 128
    cameras = build_cameras(dict(type="fovperspective", K=K, R=R, T=T, resolution=resolution))
    renderer = build_renderer(
        dict(type="mesh", resolution=resolution, shader=dict(type="soft_phong"), lights=dict(type="ambient"))
    )
