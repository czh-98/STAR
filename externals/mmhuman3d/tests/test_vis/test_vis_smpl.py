...
body_model_config = {"use_pca": False, "use_face_contour": True, "model_path": "data/body_models"}
...
def test_visualize_smpl_pose():
    ...
    with pytest.raises(RuntimeError):
        pose_dict = {"body_pose": torch.zeros(2, 68), "global_orient": torch.zeros(2, 3)}
        body_model_config.update(type="smpl")
        visualize_smpl_pose(
            poses=pose_dict,
            body_model_config=body_model_config,
    ...
    with pytest.raises(KeyError):
        pose_dict = {"wrong_smpl_name": torch.zeros(2, 69), "global_orient": torch.zeros(2, 3)}
        body_model_config.update(type="smpl")
        visualize_smpl_pose(
            poses=pose_dict,
            body_model_config=body_model_config,
    ...
    pose_dict = {"body_pose": torch.zeros(2, 69), "global_orient": torch.zeros(2, 3)}
    body_model_config.update(type="smpl")
    tensor = visualize_smpl_pose(
        poses=pose_dict,
        body_model_config=body_model_config,
    ...
    image_array = np.random.randint(low=0, high=255, size=(3, 128, 128, 3), dtype=np.uint8)
