...
def test_writer():
    writer = video_writer(output_path=osp.join(root, "demo.mp4"), resolution=(512, 512))
...
def test_generate_data():
    ...
    v = np.random.randint(low=0, high=255, size=(30, 512, 512, 3), dtype=np.uint8)
    array_to_images(v, osp.join(root, "input_images"))
    array_to_video(v, osp.join(root, "input_video.mp4"))
    images_to_gif(osp.join(root, "input_images"), osp.join(root, "input_gif.gif"))
    images_to_gif(osp.join(root, "input_images"), osp.join(root, "input_gif.gif"), img_format=None)
...
def test_array_saver():
    ...
    with pytest.raises(AssertionError):
        v = np.random.randint(low=0, high=255, size=(30, 512, 512, 4), dtype=np.uint8)
        array_to_video(v, osp.join(root, "test_saver.mp4"))
        os.makedirs(osp.join(root, "img_folder_saver"))
        array_to_images(v, osp.join(root, "img_folder_saver"))
    with pytest.raises(AssertionError):
        v = np.random.randint(low=0, high=255, size=(512, 512, 3), dtype=np.uint8)
        array_to_video(v, osp.join(root, "test_saver.mp4"))
        array_to_images(v, osp.join(root, "img_folder_saver"))
...
def test_image_reader():
    ...
    v = images_to_array(osp.join(root, "input_images"), resolution=(300, 200), img_format=None)
    v = video_to_array(osp.join(root, "input_video.mp4"), resolution=(300, 200))
    assert v.shape[1:] == (300, 200, 3)
...
def test_temporal_concat_video():
    ...
    temporal_concat_video(
        [osp.join(root, "input_video.mp4"), osp.join(root, "input_video.mp4")],
        output_path=osp.join(root, "test_temporal_concat_output.mp4"),
    )
...
def test_spatial_c1():
    ...
    with pytest.raises(FileNotFoundError):
        spatial_concat_video(
            [osp.join(root, "NoSuchFile.mp4"), osp.join(root, "input_video.mp4"), osp.join(root, "input_video.mp4")],
            output_path=osp.join(root, "test_spacial_concat_output.mp4"),
            array=[2, 2],
            padding=1,
        )

    spatial_concat_video(
        [osp.join(root, "input_video.mp4"), osp.join(root, "input_video.mp4"), osp.join(root, "input_video.mp4")],
        output_path=osp.join(root, "test_spacial_concat_output.mp4"),
        array=[2, 2],
        padding=1,
    )
...
def test_temporal_c2():
    ...
    # wrong input/output
    with pytest.raises(FileNotFoundError):
        slice_video(osp.join(root, "NoSuchFile.mp4"), osp.join(root, "test_temporal_crop_output.mp4"), start=0, end=10)

    slice_video(osp.join(root, "input_video.mp4"), osp.join(root, "test_temporal_crop_output.mp4"), start=0, end=10)
...
def test_spacial_c2():
    ...
    with pytest.raises(AssertionError):
        crop_video(
            osp.join(root, "input_video.mp4"), osp.join(root, "test_spacial_crop_output.mp4"), box=[10, 10, -1, -1]
        )
    # wrong input/output
    with pytest.raises(FileNotFoundError):
        crop_video(
            osp.join(root, "NoSuchFile.mp4"), osp.join(root, "test_spacial_crop_output.mp4"), box=[10, 10, -1, -1]
        )

    crop_video(
        osp.join(root, "input_video.mp4"), osp.join(root, "test_spacial_crop_output.mp4"), box=[10, 10, 100, 100]
    )
...
def test_convert():
    ...
    with pytest.raises(FileNotFoundError):
        images_to_gif(osp.join("/NoSuchDir", "img_folder"), osp.join(root, "images_to_gif_output.gif"))

    images_to_gif(osp.join(root, "input_images"), osp.join(root, "images_to_gif_output.gif"), fps=30)
    assert os.path.isfile(osp.join(root, "images_to_gif_output.gif"))

    # wrong inpath images_to_video
    with pytest.raises(FileNotFoundError):
        images_to_video(osp.join("/NoSuchDir", "input_images"), osp.join(root, "images_to_video_output.mp4"))
    # wrong outpath
    images_to_video(osp.join(root, "input_images"), osp.join(root, "images_to_video_output.mp4"))
    assert os.path.isfile(osp.join(root, "images_to_video_output.mp4"))
    images_to_video(osp.join(root, "input_images"), osp.join(root, "images_to_video_output.mp4"), img_format=None)
    assert os.path.isfile(osp.join(root, "images_to_video_output.mp4"))
    images_to_video(osp.join(root, "input_images"), osp.join(root, "images_to_video_output.mp4"), resolution=(100, 100))
    assert os.path.isfile(osp.join(root, "images_to_video_output.mp4"))

    # wrong inpath gif_to_video
    with pytest.raises(FileNotFoundError):
        gif_to_video(osp.join("/NoSuchDir", "input_gif.gif"), osp.join(root, "gif_to_video_output.mp4"))
    # wrong outpath
    gif_to_video(osp.join(root, "input_gif.gif"), osp.join(root, "gif_to_video_output.mp4"))
    assert os.path.isfile(osp.join(root, "gif_to_video_output.mp4"))

    # wrong inpath gif_to_images
    with pytest.raises(FileNotFoundError):
        gif_to_images(osp.join("/NoSuchDir", "input_gif.gif"), osp.join(root, "gif_to_images_output"))
    gif_to_images(osp.join(root, "input_gif.gif"), osp.join(root, "gif_to_images_output"))
    assert os.path.isdir(osp.join(root, "gif_to_images_output"))
    assert images_to_array(osp.join(root, "gif_to_images_output")).shape[1:] == (512, 512, 3)

    # wrong inpath video_to_gif
    with pytest.raises(FileNotFoundError):
        video_to_gif(osp.join("/NoSuchDir", "input_video.mp4"), osp.join(root, "video_to_gif_output.gif"))
    video_to_gif(osp.join(root, "input_video.mp4"), osp.join(root, "video_to_gif_output.gif"))
    assert os.path.isfile(osp.join(root, "video_to_gif_output.gif"))

    # wrong inpath video_to_images
    with pytest.raises(FileNotFoundError):
        video_to_images(osp.join("/NoSuchDir", "input_video.mp4"), osp.join(root, "video_to_images_output"))

    video_to_images(osp.join(root, "input_video.mp4"), osp.join(root, "video_to_images_output"))
    assert os.path.isdir(osp.join(root, "video_to_images_output"))
    assert images_to_array(osp.join(root, "video_to_images_output")).shape[1:] == (512, 512, 3)
