...
def test_realtime_render_cuda():
    ...
    body_model = build_body_model(dict(type="SMPL", gender="neutral", num_betas=10, model_path="data/body_models/smpl"))
    renderer = VisualizerMeshSMPL(body_models=body_model, resolution=[224, 224], device="cuda")
...
@surrogate("mmhuman3d.core.renderer.mpr_renderer.cuda.rasterizer.estimate_normals")
@patch("mmhuman3d.core.renderer.mpr_renderer.cuda.rasterizer.estimate_normals", mock_estimate_normals_cuda)
@surrogate("mmhuman3d.core.renderer.mpr_renderer.cuda.rasterizer.project_mesh")
@patch("mmhuman3d.core.renderer.mpr_renderer.cuda.rasterizer.project_mesh", mock_project_mesh_cuda)
def test_realtime_render():
    ...
    pinhole2d = Pinhole2D(fx=5000.0, fy=5000.0, cx=112, cy=112, w=1024, h=1024)

    coords, normals = estimate_normals(vertices, faces, pinhole2d, vertices_filter)
    ...
    z_buff = project_mesh(vertices, faces, vertices, pinhole2d, vertices_filter)
    ...
    pinhole2d = Pinhole2D(fx=5000.0, fy=5000.0, cx=112, cy=112, w=1024, h=1024)
    K = pinhole2d.get_K()
    assert (K == np.array([[5000.0, 0, 112], [0, 5000.0, 112], [0, 0, 1]])).all()
    ...
    # test utils
    from mmhuman3d.core.renderer.mpr_renderer.utils import vis_z_buffer, vis_normals
...
@patch("mmhuman3d.core.renderer.mpr_renderer.rasterizer.estimate_normals", mock_estimate_normals)
@surrogate("mmhuman3d.core.renderer.mpr_renderer.cuda.rasterizer.estimate_normals")
@patch("mmhuman3d.core.renderer.mpr_renderer.cuda.rasterizer.estimate_normals", mock_estimate_normals_cuda)
@surrogate("mmhuman3d.core.renderer.mpr_renderer.cuda.rasterizer.project_mesh")
@patch("mmhuman3d.core.renderer.mpr_renderer.cuda.rasterizer.project_mesh", mock_project_mesh_cuda)
def test_smpl_realtime_render():
    ...
    body_model = build_body_model(dict(type="SMPL", gender="neutral", num_betas=10, model_path="data/body_models/smpl"))
    renderer = VisualizerMeshSMPL(body_models=body_model, resolution=[1024, 1024], device="cpu")
