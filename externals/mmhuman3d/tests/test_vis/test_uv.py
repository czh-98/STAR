...
uv_renderer = UVRenderer(resolution=(512, 512), model_type="smpl", uv_param_path=uv_param_path, device=device)
...
def test_uv_resample():
    pose_dict = body_model.tensor2dict(torch.zeros(1, (body_model.NUM_BODY_JOINTS + 1) * 3).to(device))
    smpl_output = body_model(**pose_dict)
    verts = smpl_output["vertices"].view(1, body_model.NUM_VERTS, 3)
    mesh = Meshes(verts=verts, faces=body_model.faces_tensor.to(device).view(1, body_model.NUM_FACES, 3))
    ...
    mesh1 = uv_renderer.wrap_displacement(mesh, displacement_map=displacement_map)
    assert torch.isclose(mesh.verts_padded(), mesh1.verts_padded() - 1, atol=1e-3).all()
...
def test_uv_forward():
    ...
    smpl_output = body_model(**pose_dict)
    verts = smpl_output["vertices"].view(1, body_model.NUM_VERTS, 3)
    normal_map = uv_renderer.forward_normal_map(vertices=verts, resolution=(512, 512))
    assert normal_map.shape == (1, 512, 512, 3)

    uvd_map = uv_renderer.forward_uvd_map(vertices=verts, resolution=(512, 512))
