...
def test_vis_kp2d():
    image_array = np.random.randint(low=0, high=255, size=(2, 512, 512, 3), dtype=np.uint8)
    ...
    with pytest.raises(AssertionError):
        visualize_kp2d(
            ...
            frame_list=["tests/data/test_vis_kp2d/%06d.png" % 1, "tests/data/test_vis_kp2d/%06d.png" % 2],
        )

    # wrong input frame path
    kp2d = np.random.randint(low=0, high=255, size=(10, 133, 2), dtype=np.uint8)
    with pytest.raises(FileNotFoundError):
        visualize_kp2d(
            kp2d,
            output_path="tests/data/test_vis_kp2d/wrong_input_path.mp4",
            frame_list=["tests/data/test_vis_kp2d/1.png", "tests/data/test_vis_kp2d/2.png"],
        )
    ...
    with pytest.raises(IndexError):
        visualize_kp2d(
            ...
            frame_list=["tests/data/test_vis_kp2d/%06d.png" % 0, "tests/data/test_vis_kp2d/%06d.png" % 1],
    ...
    kp2d = np.random.randint(low=0, high=255, size=(10, 2, 133, 2), dtype=np.uint8)
    visualize_kp2d(
        kp2d,
    ...
    kp2d = np.random.randint(low=0, high=255, size=(10, 133, 3), dtype=np.uint8)
    visualize_kp2d(
        kp2d,
    ...
    assert video_to_array("tests/data/test_vis_kp2d/test_confidence1.mp4").shape

    # visualize single frame
    kp2d = np.random.randint(low=0, high=255, size=(1, 17, 2), dtype=np.uint8)
    image_array = visualize_kp2d(
        kp2d, image_array=image_array, data_source="coco", draw_bbox=True, with_number=True, return_array=True
    )
    assert isinstance(image_array, np.ndarray) and image_array.shape == (1, 512, 512, 3)
    ...
    with pytest.raises(FileExistsError):
        visualize_kp2d(
            kp2d,
            output_path=output_folder,
            frame_list=[os.path.join(output_folder, "%06d.png" % 0), os.path.join(output_folder, "%06d.png" % 1)],
            overwrite=False,
        )
