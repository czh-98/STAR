...
def test_transform_to_camera_frame():
    ...
    random_output = body_model(
        global_orient=random_global_orient, body_pose=random_body_pose, transl=random_transl, betas=random_betas
    )
    ...
    # transform params
    transformed_global_orient, transformed_transl = transform_to_camera_frame(
    ...
    random_joints = torch.cat([random_joints, torch.ones(45, 1)], dim=1)  # (45, 4)
    test_joints = torch.einsum("ij,kj->ki", random_extrinsic, random_joints)  # (45, 4)
...
def test_batch_transform_to_camera_frame():
    ...
    # transform params
    transformed_global_orient, transformed_transl = batch_transform_to_camera_frame(
    ...
    random_joints = torch.cat([random_joints, torch.ones(N, 45, 1)], dim=2)  # (N, 45, 4)
    test_joints = torch.einsum("ij,bkj->bki", random_extrinsic, random_joints)  # (N, 45, 4)
