...
def test_keypoint_mse_loss():
    ...
    pred = torch.ones(1, 3, 2)
    target = torch.zeros(1, 3, 2)
    assert torch.allclose(loss(pred, target), torch.tensor(3.0))
    ...
    # test loss weight
    loss_cfg = dict(type="KeypointMSELoss", loss_weight=2.0)
    loss = build_loss(loss_cfg)
    pred = torch.zeros(1, 3, 2)
    target = torch.zeros(1, 3, 2)
    assert torch.allclose(loss(pred, target), torch.tensor(0.0))
    ...
    pred = torch.ones(1, 3, 2)
    target = torch.zeros(1, 3, 2)
    assert torch.allclose(loss(pred, target), torch.tensor(1.0))
