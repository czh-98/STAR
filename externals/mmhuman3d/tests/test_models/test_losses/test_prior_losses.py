...
def test_shape_prior_loss():
    ...
    output = loss(betas)
    assert isinstance(output, torch.Tensor) and output.size() == ()


def test_joint_prior_loss():
    loss_config = dict(type="JointPriorLoss", use_full_body=False, reduction="mean")
    loss = build_loss(loss_config)
...
def test_smooth_joint_loss():
    ...
    loss_config = dict(type="SmoothJointLoss", reduction="mean", loss_func="L2")
    loss = build_loss(loss_config)
    body_pose = torch.rand(2, 69)
    output = loss(body_pose)
    assert isinstance(output, torch.Tensor) and output.size() == ()
...
def test_limb_length_loss():
    ...
    # test with pred_conf
    assert torch.allclose(loss(pred, target, pred_conf=pred_conf), torch.tensor(0.0))
    # test with target conf
    assert torch.allclose(loss(pred, target, target_conf=target_conf), torch.tensor(0.0))
    # test with pred and target conf
    assert torch.allclose(loss(pred, target, pred_conf=pred_conf, target_conf=target_conf), torch.tensor(0.0))
    ...
    assert torch.allclose(loss(pred, target), torch.tensor(3.0 * 3 / M))
    # test with pred_conf (only pelvis-right_hip is valid)
    assert torch.allclose(loss(pred, target, pred_conf=pred_conf), torch.tensor(3.0 * 1 / M))
...
def test_pose_reg_loss():
    ...
    assert torch.allclose(loss(body_pose), torch.tensor(0.0))

    body_pose = torch.ones(1, 3, 2)
    assert torch.allclose(loss(body_pose), torch.tensor(1.0))
    ...
    body_pose = torch.ones(1, 3, 2)
    assert torch.allclose(loss(body_pose), torch.tensor(6.0))
    ...
    # test loss weight
    loss_cfg = dict(type="PoseRegLoss", loss_weight=2.0)
    loss = build_loss(loss_cfg)
    body_pose = torch.zeros(1, 3, 2)
    assert torch.allclose(loss(body_pose), torch.tensor(0.0))

    body_pose = torch.ones(1, 3, 2)
    assert torch.allclose(loss(body_pose), torch.tensor(2.0))


def test_shape_threshold_prior_loss():
    loss_cfg = dict(type="ShapeThresholdPriorLoss", loss_weight=1.0)
    loss = build_loss(loss_cfg)
    shape = torch.zeros(1, 10)
    assert torch.allclose(loss(shape), torch.tensor(0.0))
