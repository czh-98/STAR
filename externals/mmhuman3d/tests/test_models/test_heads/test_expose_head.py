...
def test_expose_body_head():
    ...
    img_meta = dict(ori_img=torch.rand(3, 300, 300), crop_transform=np.random.rand(2, 3), scale=np.random.rand(2))
    ...
    hand_predictions = dict(
        pred_param=dict(
            global_orient=torch.rand(batch_size * 2, 1, 3, 3), right_hand_pose=torch.rand(batch_size * 2, 15, 3, 3)
        )
    )
