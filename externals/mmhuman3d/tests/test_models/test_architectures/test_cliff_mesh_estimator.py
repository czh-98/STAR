...
from mmhuman3d.models.architectures.cliff_mesh_estimator import CliffImageBodyModelEstimator  # noqa: E501
...
def test_cliff_image_body_mesh_estimator():
    ...
    head = dict(type="CliffHead", feat_dim=2048, smpl_mean_params="data/body_models/smpl_mean_params.npz")
    ...
    loss_adv = dict(type="GANLoss", gan_type="lsgan", real_label_val=1.0, fake_label_val=0.0, loss_weight=1)
...
def test_compute_keypoints3d_loss():
    model = CliffImageBodyModelEstimator(
        convention="smpl_54", loss_keypoints3d=dict(type="SmoothL1Loss", loss_weight=100)
    )
    ...
    loss = model.compute_keypoints3d_loss(pred_keypoints3d, gt_keypoints3d, has_keypoints3d=has_keypoints3d)
    assert loss > 0
...
def test_compute_keypoints2d_loss_cliff():
    model = CliffImageBodyModelEstimator(
        convention="smpl_54", loss_keypoints2d=dict(type="SmoothL1Loss", loss_weight=10)
    )
    ...
    loss_empty = model.compute_keypoints2d_loss_cliff(
        pred_keypoints3d, pred_cam, gt_keypoints2d, camera_center, focal_length, trans
    )
    assert loss_empty == 0
    ...
    loss = model.compute_keypoints2d_loss_cliff(
        pred_keypoints3d, pred_cam, gt_keypoints2d, camera_center, focal_length, trans, has_keypoints2d=has_keypoints2d
    )
    assert loss > 0
...
def test_compute_keypoints2d_loss():
    ...
    gt_keypoints2d = torch.zeros((32, 54, 3))
    pred_cam = torch.randn((32, 3))
    loss_empty = model.compute_keypoints2d_loss(pred_keypoints3d, pred_cam, gt_keypoints2d)
    assert loss_empty == 0
    ...
    gt_keypoints2d[:, :, 2] = torch.sigmoid(gt_keypoints2d[:, :, 2])
    pred_cam = torch.randn((32, 3))
    loss = model.compute_keypoints2d_loss(pred_keypoints3d, pred_cam, gt_keypoints2d)
    assert loss > 0

    has_keypoints2d = torch.ones((32))
    loss = model.compute_keypoints2d_loss(pred_keypoints3d, pred_cam, gt_keypoints2d, has_keypoints2d=has_keypoints2d)
    assert loss > 0
...
def test_compute_vertex_loss():
    model = CliffImageBodyModelEstimator(convention="smpl_54", loss_vertex=dict(type="L1Loss", loss_weight=2))

    pred_vertices = torch.randn((32, 4096, 3))
    gt_vertices = torch.randn((32, 4096, 3))
    has_smpl = torch.zeros((32))
    loss_empty = model.compute_vertex_loss(pred_vertices, gt_vertices, has_smpl)
    assert loss_empty == 0
...
def test_compute_smpl_pose_loss():
    model = CliffImageBodyModelEstimator(convention="smpl_54", loss_smpl_pose=dict(type="MSELoss", loss_weight=3))
...
def test_compute_part_segm_loss():
    ...
    cam = torch.ones(N, 3)
    gt_keypoints2d = project_points(gt_model_joins, cam, focal_length=5000, img_res=224)
    ...
    loss = model.compute_part_segmentation_loss(
        pred_heatmap, gt_vertices, has_smpl=has_smpl, gt_keypoints2d=gt_keypoints2d, gt_model_joints=gt_model_joins
    )
    assert loss > 0


def test_compute_smpl_betas_loss():
    model = CliffImageBodyModelEstimator(convention="smpl_54", loss_smpl_betas=dict(type="MSELoss", loss_weight=0.02))
...
def test_compute_losses():
    ...
    model = CliffImageBodyModelEstimator(
        convention="smpl_45",
        body_model_train=dict(
            type="SMPL", keypoint_src="smpl_45", keypoint_dst="smpl_45", model_path="data/body_models/smpl"
        ),
        loss_keypoints3d=dict(type="SmoothL1Loss", loss_weight=100),
        loss_keypoints2d=dict(type="SmoothL1Loss", loss_weight=10),
        loss_vertex=dict(type="L1Loss", loss_weight=2),
...
def test_run_registration():
    ...
    registrant = dict(
        ...
        num_epochs=1,
        stages=[dict(num_iter=1, fit_global_orient=True, fit_transl=True, fit_body_pose=False, fit_betas=False)],
        optimizer=dict(type="Adam", lr=1e-2, betas=(0.9, 0.999)),
        keypoints2d_loss=dict(type="KeypointMSELoss", loss_weight=1.0, reduction="sum", sigma=100),
    ...
    model = CliffImageBodyModelEstimator(body_model_train=body_model, registration=registration)
    ...
    targets = dict(
        img_metas=[dict(dataset_name="coco"), dict(dataset_name="h36m")],
