...
def test_smplx_image_body_mesh_estimator():
    ...
    loss_smplx_body_pose = dict(type="RotationDistance", reduction="sum", loss_weight=1)
...
def test_compute_keypoints3d_loss():
    model = SMPLXImageBodyModelEstimator(
        convention="smplx", loss_keypoints3d=dict(type="L1Loss", reduction="sum", loss_weight=1)
    )
...
def test_compute_keypoints2d_loss():
    model = SMPLXImageBodyModelEstimator(
        convention="smplx", loss_keypoints2d=dict(type="L1Loss", reduction="sum", loss_weight=1)
    )
...
def test_smplx_body_pose_loss():
    model = SMPLXImageBodyModelEstimator(
        convention="smplx", loss_smplx_body_pose=dict(type="L1Loss", reduction="sum", loss_weight=1)
    )
...
def test_smplx_global_orient_loss():
    model = SMPLXImageBodyModelEstimator(
        convention="smplx", loss_smplx_global_orient=dict(type="L1Loss", reduction="sum", loss_weight=1)
    )
...
def test_smplx_jaw_pose_loss():
    model = SMPLXImageBodyModelEstimator(
        convention="smplx", loss_smplx_jaw_pose=dict(type="L1Loss", reduction="sum", loss_weight=1)
    )
...
def test_smplx_hand_pose_loss():
    model = SMPLXImageBodyModelEstimator(
        convention="smplx", loss_smplx_hand_pose=dict(type="L1Loss", reduction="sum", loss_weight=1)
    )
...
def test_smplx_betas_loss():
    model = SMPLXImageBodyModelEstimator(
        convention="smplx", loss_smplx_betas=dict(type="L1Loss", reduction="sum", loss_weight=1)
    )
...
def test_smplx_expression_loss():
    model = SMPLXImageBodyModelEstimator(
        convention="smplx", loss_smplx_expression=dict(type="L1Loss", reduction="sum", loss_weight=1)
    )
...
def test_hand_crop_loss():
    model = SMPLXImageBodyModelEstimator(
        convention="smplx",
        extra_hand_model_cfg=dict(loss_hand_crop=dict(type="L1Loss", reduction="sum", loss_weight=1)),
    )
    pred_keypoints3d = torch.zeros((32, 144, 3))
    ...
    loss_empty = model.compute_hand_crop_loss(pred_keypoints3d, pred_cam, gt_keypoints2d, hand_crop_info)
    assert loss_empty == 0
...
def test_face_crop_loss():
    model = SMPLXImageBodyModelEstimator(
        convention="smplx",
        extra_face_model_cfg=dict(loss_face_crop=dict(type="L1Loss", reduction="sum", loss_weight=1)),
    )
    pred_keypoints3d = torch.zeros((32, 144, 3))
    pred_cam = torch.randn((32, 3))
    gt_keypoints2d = torch.zeros((32, 144, 3))
    ...
    loss_empty = model.compute_face_crop_loss(pred_keypoints3d, pred_cam, gt_keypoints2d, face_crop_info)
    assert loss_empty == 0
...
def test_compute_losses():
    ...
    model = SMPLXImageBodyModelEstimator(
        ...
        loss_keypoints3d=dict(type="L1Loss", reduction="sum", loss_weight=1),
        loss_keypoints2d=dict(type="L1Loss", reduction="sum", loss_weight=1),
        loss_smplx_body_pose=dict(type="L1Loss", reduction="sum", loss_weight=1),
        loss_smplx_global_orient=dict(type="L1Loss", reduction="sum", loss_weight=1),
        loss_smplx_jaw_pose=dict(type="L1Loss", reduction="sum", loss_weight=1),
        loss_smplx_hand_pose=dict(type="L1Loss", reduction="sum", loss_weight=1),
        loss_smplx_betas=dict(type="L1Loss", reduction="sum", loss_weight=1),
        loss_smplx_expression=dict(type="L1Loss", reduction="sum", loss_weight=1),
        loss_smplx_betas_prior=dict(type="ShapeThresholdPriorLoss", margin=3.0, norm="l2", loss_weight=1),
        extra_hand_model_cfg=dict(loss_hand_crop=dict(type="L1Loss", reduction="sum", loss_weight=1)),
        extra_face_model_cfg=dict(loss_face_crop=dict(type="L1Loss", reduction="sum", loss_weight=1)),
    )
