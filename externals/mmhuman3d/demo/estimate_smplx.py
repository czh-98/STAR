...
def single_person_with_mmdet(args, frames_iter):
    ...
    smplx_results = dict(
        global_orient=[], body_pose=[], betas=[], left_hand_pose=[], right_hand_pose=[], jaw_pose=[], expression=[]
    )
    ...
    for i, result in enumerate(mmcv.track_iter_progress(result_list)):
        ...
        for key in smplx_results:
            smplx_results[key].append(mesh_results[0]["param"][key].cpu().numpy())
        ...
        for key in smplx_results:
            if key not in ["betas", "expression"]:
                ...
                smplx_results[key] = smooth_process(
                    smplx_results[key].reshape(frame_num, -1, dim, 9), smooth_type=args.smooth_type
                ).reshape(frame_num, dim, 3, 3)
...
def multi_person_with_mmtracking(args, frames_iter):
    ...
    for i, result in enumerate(mmcv.track_iter_progress(result_list)):
        ...
        for mesh_result in mesh_results:
            ...
            for key in smplx_results:
                smplx_results[key][i, instance_id] = mesh_result["param"][key].cpu().numpy()
    ...
    fullpose = np.concatenate(
        (
            ...
            # Use zero for leye_pose and reye_pose
            np.zeros((frame_num, max_track_id + 1, 2, 3), dtype=smplx_results["jaw_pose"].dtype),
    ...
    for i, track_ids_list in enumerate(track_ids_lists):
        ...
        compressed_betas[i, :instance_num] = smplx_results["betas"][i, track_ids_list]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mesh_reg_config", type=str, default="configs/expose/expose.py", help="Config file for mesh regression"
    )
    parser.add_argument(
        "--mesh_reg_checkpoint",
        type=str,
    ...
    parser.add_argument("--multi_person_demo", action="store_true", help="Multi person demo with MMTracking")
    parser.add_argument(
        "--det_config", default="demo/mmdetection_cfg/faster_rcnn_r50_fpn_coco.py", help="Config file for detection"
    )
    parser.add_argument(
        "--det_checkpoint",
    ...
    parser.add_argument("--input_path", type=str, default="demo/resources/single_person_demo.mp4", help="Input path")
    parser.add_argument("--output", type=str, default="demo_result", help="directory to save output result file")
    parser.add_argument(
        "--show_path", type=str, default="demo_result", help="directory to save rendered images or video"
    )
    ...
    parser.add_argument("--bbox_thr", type=float, default=0.99, help="Bounding box score threshold")
    parser.add_argument("--draw_bbox", action="store_true", help="Draw a bbox for each detected instance")
    parser.add_argument(
        "--smooth_type",
        type=str,
        default=None,
        help="Smooth the data through the specified type." "Select in [oneeuro,savgol].",
    )
