...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Category id for bounding box detection model. " "Default: 1 for human",
    )
    parser.add_argument("--device", default="cuda:0", help="Device used for inference")
    parser.add_argument("--body_model_dir", type=str, default="data/body_models/smpl", help="Body models file path")
    parser.add_argument("--bbox_thr", type=float, default=0.6, help="Bounding box score threshold")
    parser.add_argument(
        "--output", type=str, default=None, help="Record the video into a file. This may reduce the frame rate"
    )
    parser.add_argument("--out_video_fps", type=int, default=20, help="Set the FPS of the output video file.")
    parser.add_argument("--input_video_fps", type=int, default=30, help="The FPS of the input video file.")
    parser.add_argument(
        "--buffer_size",
        type=int,
        default=-1,
...
def inference_mesh():
    ...
    while True:
        ...
        with stop_watch.timeit("Mesh"):
            det_results = process_mmdet_results(mmdet_results, cat_id=args.det_cat_id, bbox_thr=args.bbox_thr)

            mesh_results = inference_image_based_model(
                mesh_model, frame, det_results, bbox_thr=args.bbox_thr, format="xyxy"
            )
        t_info += stop_watch.report_strings()
    ...
    while True:
        with stop_watch.timeit("_FPS_"):
            ...
            if mesh_results:
                ...
                verts, _ = convert_verts_to_cam_coord(verts, pred_cams, bboxes_xyxy, focal_length=5000.0)

                # show bounding boxes
                mmcv.imshow_bboxes(img, bboxes_xyxy[None], colors="green", top_k=-1, thickness=2, show=False)
            ...
            sys_info = [f"RES: {img.shape[1]}x{img.shape[0]}", f"Buffer: {frame_buffer.qsize()}/{frame_buffer.maxsize}"]
            if psutil_proc is not None:
                sys_info += [f"CPU: {psutil_proc.cpu_percent():.1f}%", f"MEM: {psutil_proc.memory_percent():.1f}%"]
            sys_info_str = " | ".join(sys_info)
            cv2.putText(img, sys_info_str, (20, 40), cv2.FONT_HERSHEY_DUPLEX, 0.3, text_color, 1)
            ...
            if args.output is not None:
                if vid_out is None:
                    ...
                    vid_out = cv2.VideoWriter(args.output, fourcc, fps, frame_size)
...
def main():
    ...
    body_model = build_body_model(dict(type="SMPL", gender="neutral", num_betas=10, model_path=args.body_model_dir))
    # build renderer
    renderer = VisualizerMeshSMPL(device=args.device, body_models=body_model, resolution=resolution)
    ...
    if args.buffer_size > 0:
        ...
        buffer_size = round(args.input_video_fps * (1 + max(args.display_delay, 0) / 1000.0))
