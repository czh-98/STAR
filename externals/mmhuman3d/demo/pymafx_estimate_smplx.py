...
def prepare_data_with_mmpose_detection(args, frames_iter):
    ...
    pose_results = []
    all_results = []
    for frame_id, img in tqdm(enumerate(mmcv.track_iter_progress(frames_iter))):
    ...
    frames_idx, final_results, max_instance = process_tracking_results(all_results)
...
def main(args):
    ...
    mesh_model, _ = init_model(pymafx_config, args.mesh_reg_checkpoint, device=args.device.lower())
    ...
    if args.use_openpifpaf:
        ...
        # Prepare input
        joints2d, frames, wb_kps, image_folder, max_instance = prepare_data_with_pifpaf_detection(args, frames_iter)
    else:
        joints2d, frames, wb_kps, image_folder, max_instance = prepare_data_with_mmpose_detection(args, frames_iter)
    ...
    test_dataset = build_dataset(pymafx_config["data"]["test"], dict(test_mode=True))
    bboxes_cs = test_dataset.bboxes
    frame_ids = test_dataset.frames
    dataloader = DataLoader(test_dataset, batch_size=args.model_batch_size, num_workers=0)
    ...
    orig_cam = convert_crop_cam_to_orig_img(
        cam=pred_cam, bbox=bboxes_cs, img_width=orig_width, img_height=orig_height, bbox_format="cs"
    )
    ...
    for data in smplx_params:
        ...
        for key in smplx_results:
            if key == "global_orient":
                smplx_results[key].append(rotmat_to_aa(data["body_pose"].cpu().numpy()[:, :1]))
            elif key == "body_pose":
                smplx_results[key].append(rotmat_to_aa(data["body_pose"].cpu().numpy()[:, 1:22]))
            elif key == "betas":
                smplx_results[key].append(data["betas"].cpu().numpy())
            elif key == "expression":
                smplx_results[key].append(data["expression"].cpu().numpy())
            else:
                smplx_results[key].append(rotmat_to_aa(data[key].cpu().numpy()))
        for key in smplx_results:
    ...
    image_file_names = sorted(
        [osp.join(image_folder, x) for x in os.listdir(image_folder) if x.endswith(".png") or x.endswith(".jpg")]
    )
    ...
    for idx, frame_id in enumerate(frame_ids):
        if idx == 0:
            ...
            compressed_fullpose[frame_id, :n_person] = fullpose[idx : idx + n_person]
    ...
    if args.visualization:
        ...
        if Path(args.input_path).is_dir():
            for i, image in enumerate(image_array):
                image_path = osp.join(args.output_path, image_file_names[i].split("/")[-1].split(".")[0])
                visualize_smpl_vibe(
                    poses=compressed_fullpose.reshape(-1, max_instance, 165)[i : i + 1],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mesh_reg_config", type=str, default="configs/pymafx/pymafx.py", help="Config file for mesh regression"
    )
    parser.add_argument(
        "--mesh_reg_checkpoint",
        type=str,
    ...
    parser.add_argument("--openpifpaf_threshold", type=float, default=0.35, help="pifpaf detection score threshold.")
    parser.add_argument("--use_openpifpaf", action="store_true", help="pifpaf detection")
    # mmpose
    parser.add_argument("--mmpose_bbox_thr", type=float, default=0.8, help="Bounding box score threshold")
    parser.add_argument(
    ...
    parser.add_argument(
        "--pose_config",
        default="demo/mmpose_cfg/" "hrnet_w48_coco_wholebody_384x288_dark_plus.py",
    ...
    parser.add_argument("--input_path", type=str, default=None, help="input folder")
    parser.add_argument("--output_path", type=str, default="output", help="output folder to write results")
    parser.add_argument("--visualization", action="store_true", help="SMPLX Visualization")

    parser.add_argument("--device", choices=["cpu", "cuda"], default="cuda", help="device used for testing")
    parser.add_argument("--model_batch_size", type=int, default=8, help="batch size for SMPL prediction")
