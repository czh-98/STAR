...
def get_tracking_result(args, frames_iter, mesh_model, extractor):
    tracking_model = init_tracking_model(args.tracking_config, None, device=args.device.lower())
    ...
    for i, frame in enumerate(mmcv.track_iter_progress(frames_iter)):
        ...
        if mesh_model.cfg.model.type == "VideoBodyModelEstimator" and extractor is not None:
            result = feature_extract(extractor, frame, result, args.bbox_thr, format="xyxy")
...
def nonlinearWeight(x, thre_low, a1, a2, a3, a4):
    ...
    if abs(x).mean() >= thre_low:
        ratio = (a1 / a3 + a2 * np.power(x, 2)) / np.sqrt(a4 / (a3 * a3) + np.power(x, 4))
    # if ratio > 1:
    #     ratio = 1
    return ratio
...
def get_detection_result(args, frames_iter, mesh_model, extractor):
    ...
    for i, frame in enumerate(mmcv.track_iter_progress(frames_iter)):
        ...
        # keep the person class bounding boxes.
        results = process_mmdet_results(mmdet_results, cat_id=args.det_cat_id, bbox_thr=args.bbox_thr)

        # smooth
        if pre_bbox is not None:
            cur_bbox = results[0]["bbox"]
            dist_tl = np.array([(cur_bbox[0] - pre_bbox[0]) ** 2, (cur_bbox[1] - pre_bbox[1]) ** 2])
            delta_tl = np.array(dist_tl / (np.array(pre_bbox[:2]) + 1e-7)).sum()
            ratio_tl = nonlinearWeight(delta_tl, 0, 0.2, 0.8, 120, 1)
            dist_br = np.array([(cur_bbox[2] - pre_bbox[2]) ** 2, (cur_bbox[3] - pre_bbox[3]) ** 2])
            delta_br = np.array(dist_br / (np.array(pre_bbox[2:4]) + 1e-7)).sum()
        ...
        # extract features from the input video or image sequences
        if mesh_model.cfg.model.type == "VideoBodyModelEstimator" and extractor is not None:
            results = feature_extract(extractor, frame, results, args.bbox_thr, format="xyxy")
        # drop the frame with no detected results
        if results == []:
            continue
        # vis bboxes
    ...
    x = smooth_process(x[:, np.newaxis], smooth_type="savgol").reshape(frame_num, 5)
...
def single_person_with_mmdet(args, frames_iter):
    ...
    mesh_model, extractor = init_model(args.mesh_reg_config, args.mesh_reg_checkpoint, device=args.device.lower())

    pred_cams, verts, smpl_poses, smpl_betas, bboxes_xyxy = [], [], [], [], []

    frame_id_list, result_list = get_detection_result(args, frames_iter, mesh_model, extractor)
    ...
    for i, result in enumerate(mmcv.track_iter_progress(result_list)):
        ...
        if mesh_model.cfg.model.type == "VideoBodyModelEstimator":
            if args.speed_up_type:
                warnings.warn(
                    "Video based models do not support speed up. " "By default we will inference with original speed.",
                    UserWarning,
                )
            feature_results_seq = extract_feature_sequence(result_list, frame_idx=i, causal=True, seq_len=16, step=1)
            ...
            if args.speed_up_type and i % speed_up_interval != 0 and i <= speed_up_frames:
                ...
                mesh_results = inference_image_based_model(
                    mesh_model, frames_iter[frame_id], result, bbox_thr=args.bbox_thr, format="xyxy"
                )
        else:
    ...
    if args.smooth_type is not None:
        smpl_poses = smooth_process(smpl_poses.reshape(frame_num, 24, 9), smooth_type=args.smooth_type).reshape(
            frame_num, 24, 3, 3
        )
        verts = smooth_process(verts, smooth_type=args.smooth_type)
        pred_cams = smooth_process(pred_cams[:, np.newaxis], smooth_type=args.smooth_type).reshape(frame_num, 3)
...
if __name__ == "__main__":
    ...
    parser.add_argument("mesh_reg_config", type=str, default=None, help="Config file for mesh regression")
    parser.add_argument("mesh_reg_checkpoint", type=str, default=None, help="Checkpoint file for mesh regression")
    parser.add_argument("--single_person_demo", action="store_true", help="Single person demo with MMDetection")
    parser.add_argument("--det_config", help="Config file for detection")
    parser.add_argument("--det_checkpoint", help="Checkpoint file for detection")
    parser.add_argument("--det_cat_id", type=int, default=1, help="Category id for bounding box detection model")
    parser.add_argument("--tracking_config", help="Config file for tracking")

    parser.add_argument("--body_model_dir", type=str, default="data/body_models/", help="Body models file path")
    parser.add_argument("--input_path", type=str, default=None, help="Input path")
    parser.add_argument("--output", type=str, default=None, help="directory to save output result file")
    parser.add_argument("--show_path", type=str, default=None, help="directory to save rendered images or video")
    parser.add_argument("--render_choice", type=str, default="hq", help="Render choice parameters")
    parser.add_argument("--palette", type=str, default="segmentation", help="Color theme")
    parser.add_argument("--bbox_thr", type=float, default=0.97, help="Bounding box score threshold")
    parser.add_argument("--draw_bbox", action="store_true", help="Draw a bbox for each detected instance")
    parser.add_argument(
        "--smooth_type",
        type=str,
        default=None,
        help="Smooth the data through the specified type." "Select in [oneeuro,gaus1d,savgol].",
    )
    parser.add_argument(
        "--speed_up_type",
        type=str,
        default=None,
        help="Speed up data processing through the specified type." "Select in [deciwatch].",
    )
    parser.add_argument("--focal_length", type=float, default=5000.0, help="Focal lenght")
