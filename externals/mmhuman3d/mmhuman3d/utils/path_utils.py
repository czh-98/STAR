...
def check_path_suffix(path_str: str, allowed_suffix: Union[str, List[str]] = "") -> bool:
...
def check_path_existence(
    ...
    suffix = pathinfo.suffix.lower()
    if path_type == "dir" or path_type == "auto" and suffix == "":
        if pathinfo.is_dir():
    ...
    elif path_type == "file" or path_type == "auto" and suffix != "":
        if pathinfo.is_file():
...
def prepare_output_path(
    ...
    if exist_result == Existence.MissingParent:
        ...
        if not suffix_matched:
            raise FileNotFoundError(f'The {tag} should be {", ".join(allowed_suffix)}: ' f"{output_path}.")
...
def check_input_path(
    ...
    if exist_result in [Existence.FileExist, Existence.DirectoryExistEmpty, Existence.DirectoryExistNotEmpty]:
        suffix_matched = check_path_suffix(input_path, allowed_suffix=allowed_suffix)
        if not suffix_matched:
            raise FileNotFoundError(f'The {tag} should be {", ".join(allowed_suffix)}:' + f"{input_path}.")
    else:
