...
def search_limbs(
    data_source: str, mask: Optional[Union[np.ndarray, tuple, list]] = None, keypoints_factory: dict = KEYPOINTS_FACTORY
) -> Tuple[dict, dict]:
    ...
    for k, part_limbs in limbs_source.items():
        ...
        for limb in part_limbs:
            ...
            if (keypoints_source[limb[0]] in keypoints_target) and (keypoints_source[limb[1]] in keypoints_target):
                if mask is not None:
                    if (
                        mask[keypoints_target.index(keypoints_source[limb[0]])] != 0
                        and mask[keypoints_target.index(keypoints_source[limb[1]])] != 0
                    ):
                        flag = True
                else:
                    flag = True
...
def transform_kps2d(kps2d: torch.Tensor, transf, img_res=224):
    ...
    kps_new[:, :, :-1] = 2.0 * kps_new[:, :, :-1] / img_res - 1.0
    return kps_new[:, :, :2]
