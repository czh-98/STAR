...
def project_points(points_3d, camera, focal_length, img_res):
    ...
    cam_t = torch.stack([camera[:, 1], camera[:, 2], 2 * focal_length / (img_res * camera[:, 0] + 1e-9)], dim=-1)
    ...
    keypoints_2d = perspective_projection(
        points_3d, rotation=rot_t, translation=cam_t, focal_length=focal_length, camera_center=camera_center
    )
    return keypoints_2d
...
def weak_perspective_projection(points, scale, translation):
    ...
    """
    projected_points = scale.view(-1, 1, 1) * (points[:, :, :2] + translation.view(-1, 1, 2))
...
def cam_crop2full(crop_cam, center, scale, full_img_shape, focal_length):
    ...
    tx = (2 * (cx - img_w / 2.0) / bs) + crop_cam[:, 1]
    ty = (2 * (cy - img_h / 2.0) / bs) + crop_cam[:, 2]
...
def projection(pred_joints, pred_camera, iwp_mode=True):
    ...
    if iwp_mode:
        ...
        pred_cam_t = torch.stack([cam_sxy[:, 1], cam_sxy[:, 2], 2 * 5000.0 / (224.0 * cam_sxy[:, 0] + 1e-9)], dim=-1)
