...
def convert_smpl_from_opencv_calibration(
    ...
    if verts is not None:
        ...
        body_model = build_body_model(
            dict(type=model_type, model_path=os.path.join(model_path, model_type), gender=gender, model_type=model_type)
        )
        if isinstance(poses, dict):
            ...
            if isinstance(poses, np.ndarray):
                poses = torch.tensor(poses)
            poses = body_model.tensor2dict(full_pose=poses, transl=transl, betas=betas)
    ...
    verts_converted = RT @ torch.cat([verts_converted, torch.ones(verts_converted.shape[0], 1)], dim=-1).unsqueeze(-1)
    ...
    if K is not None:
        ...
        orig_cam = convert_perspective_to_weakperspective(K=K, zmean=zmean, in_ndc=True, resolution=resolution)
...
def project_points(
    ...
    if cameras is None:
        cameras = build_cameras(
            dict(type="perspective", convention=convention, in_ndc=in_ndc, resolution=resolution, K=K, R=R, T=T)
        )
    ...
    if isinstance(points3d, np.ndarray):
        ...
        points2d = cameras.transform_points_screen(points3d, image_size=image_size).cpu().numpy()
    elif isinstance(points3d, torch.Tensor):
        points3d = points3d[..., :3].to(cameras.device)
        points2d = cameras.transform_points_screen(points3d, image_size=image_size)
    return points2d
