...
class video_writer:
    ...
    def __init__(
        ...
        prepare_output_path(output_path, allowed_suffix=[".mp4"], tag="output video", path_type="file", overwrite=True)
...
def array_to_images(
    ...
    Returns:
        None
    """
    prepare_output_path(output_folder, allowed_suffix=[], tag="output image folder", path_type="dir", overwrite=True)
    ...
    if not disable_log:
        print(f'Running "{" ".join(command)}"')
    process = subprocess.Popen(command, stdin=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=10**8, close_fds=True)
    if process.stdin is None or process.stderr is None:
        raise BrokenPipeError("No buffer received.")
    index = 0
    while True:
...
def video_to_array(
    ...
    """
    check_input_path(input_path, allowed_suffix=[".mp4", "mkv", "avi", ".gif"], tag="input video", path_type="file")
    ...
    end = (min(end, num_frames - 1) + num_frames) % num_frames if end is not None else num_frames
    command = [
        "ffmpeg",
        "-i",
        input_path,
...
def images_to_sorted_images(input_folder, output_folder, img_format="%06d"):
    ...
    for index, file_name in enumerate(file_list):
        shutil.copy(file_name, os.path.join(output_folder, (img_format + ".%s") % (index, ext)))
...
def images_to_array(
    ...
    Returns:
        np.ndarray: shape will be (f * h * w * 3).
    """
    check_input_path(input_folder, allowed_suffix=[""], tag="input image folder", path_type="dir")

    input_folderinfo = Path(input_folder)

    temp_input_folder = None
    if img_format is None:
        temp_input_folder = os.path.join(input_folderinfo.parent, input_folderinfo.name + "_temp")
        img_format = images_to_sorted_images(input_folder=input_folder, output_folder=temp_input_folder)
    ...
    end = min(end, num_frames) % (num_frames + 1) if end is not None else num_frames
    command = [
        "ffmpeg",
        "-y",
        "-threads",
...
class vid_info_reader(object):
    ...
    def __init__(self, input_path) -> None:
        ...
        """
        check_input_path(
            input_path, allowed_suffix=[".mp4", ".gif", ".png", ".jpg", ".jpeg"], tag="input file", path_type="file"
        )
        cmd = ["ffprobe", "-show_format", "-show_streams", "-of", "json", input_path]
...
def video_to_gif(
    ...
    Returns:
        None.
    """
    check_input_path(input_path, allowed_suffix=[".mp4"], tag="input video", path_type="file")
    prepare_output_path(output_path, allowed_suffix=[".gif"], tag="output gif", path_type="file", overwrite=True)
...
def images_to_video(
    ...
    command = [
        ...
        f"{fps}",
        "-i",
        f"{input_folder}/{img_format}" if temp_input_folder is None else f"{temp_input_folder}/{img_format}",
        "-frames:v",
        f"{end - start}",
        "-profile:v",
        "baseline",
...
def images_to_gif(
    ...
    if img_format is None:
        ...
        file_list.sort()
        for index, file_name in enumerate(file_list):
            shutil.copy(file_name, os.path.join(temp_input_folder, "%06d.%s" % (index + 1, ext)))
        input_folder = temp_input_folder
        img_format = "%06d." + ext
...
def gif_to_video(
    ...
    Returns:
        None
    """
    check_input_path(input_path, allowed_suffix=[".gif"], tag="input gif", path_type="file")
    prepare_output_path(output_path, allowed_suffix=[".mp4"], tag="output video", path_type="file", overwrite=True)
    command = ["ffmpeg", "-i", input_path, "-r", f"{fps}", "-loglevel", "error", "-y", output_path, "-threads", "4"]
    if resolution:
        height, width = resolution
        command.insert(3, "-s")
        command.insert(4, "%dx%d" % (width, height))
...
def crop_video(
    ...
    """
    check_input_path(input_path, allowed_suffix=[".gif", ".mp4"], tag="input video", path_type="file")
    prepare_output_path(
        output_path, allowed_suffix=[".gif", ".mp4"], tag="output video", path_type="file", overwrite=True
    )
...
def spatial_concat_video(
    input_path_list: List[str],
    output_path: str,
    array: List[int] = [1, 1],
    direction: Literal["h", "w"] = "h",
    resolution: Union[Tuple[int, int], List[int], List[float], Tuple[float, float]] = (512, 512),
    remove_raw_files: bool = False,
    padding: int = 0,
    disable_log: bool = False,
) -> None:
    ...
    for path in input_path_list:
        check_input_path(path, allowed_suffix=[".gif", ".mp4"], tag="input video", path_type="file")
    prepare_output_path(
        output_path, allowed_suffix=[".gif", ".mp4"], tag="output video", path_type="file", overwrite=True
    )
    ...
    for index, vid_file in enumerate(input_path_list):
        ...
        scale_command.append("[%d:v]scale=%d:%d:force_original_aspect_ratio=0[v%d];" % (index, width, height, index))

    scale_command = " ".join(scale_command)
    pad_command = "[v%d]pad=%d:%d[%s];" % (
        0,
        width * array[1] + padding * (array[1] - 1),
        height * array[0] + padding * (array[0] - 1),
        lowercase[0],
    )
    for index in range(1, len(input_path_list)):
        ...
        pad_command += "[%s][v%d]overlay=%d:%d" % (lowercase[index - 1], index, pad_width, pad_height)
        if index != len(input_path_list) - 1:
            pad_command += "[%s];" % lowercase[index]

    command += ["-filter_complex", "%s%s" % (scale_command, pad_command), "-loglevel", "error", "-y", output_path]
    if not disable_log:
        print(f'Running "{" ".join(command)}"')
    subprocess.call(command)
...
def temporal_concat_video(
    input_path_list: List[str],
    output_path: str,
    resolution: Union[Tuple[int, int], Tuple[float, float]] = (512, 512),
    remove_raw_files: bool = False,
    disable_log: bool = False,
) -> None:
    ...
    command += [
        "-filter_complex",
        "%s%sconcat=n=%d:v=1:a=0[v]" % (scale_command, concat_command, len(input_path_list)),
        "-loglevel",
        "error",
        "-map",
        "[v]",
...
def compress_video(
    ...
    if (output_path == input_path) or (not output_path):
        temp_outpath = os.path.join(os.path.abspath(input_pathinfo.parent), "temp_file" + input_pathinfo.suffix)
    else:
    ...
    command = [
        ...
        "error",
        "-b:v",
        f"{bit_rate / (compress_rate * down_sample_scale)}",
        "-r",
        f"{fps}",
        "-t",
        f"{duration}",
