...
def join_batch_meshes_as_scene(
    ...
    for i in range(num_scene_size):
        assert len(meshes[i]) == num_batch_size, "Please make sure that the Meshes all have"
        "the same batch size."
    meshes_all = []
...
def mesh_to_pointcloud_vc(
    ...
    """
    assert isinstance(meshes.textures, TexturesVertex), "textures of input meshes should be `TexturesVertex`"
    vertices = meshes.verts_padded()
    if include_textures:
        verts_rgb = meshes.textures.verts_features_padded()
        verts_rgba = torch.cat([verts_rgb, torch.ones_like(verts_rgb)[..., 0:1] * alpha], dim=-1)
    else:
        verts_rgba = None
...
def texture_uv2vc(meshes: Meshes) -> Meshes:
    ...
    for index in range(batch_size):
        face_uv = vert_uv[index][meshes.textures.faces_uvs_padded()[index].view(-1)]

        img = meshes.textures._maps_padded[index]
        width, height, _ = img.shape

        face_uv = face_uv * torch.Tensor([width - 1, height - 1]).long().to(device)
        ...
        verts_rgb[:, faces.view(-1)] = img[height - 1 - face_uv[:, 1], face_uv[:, 0]]
...
def load_objs_as_meshes(
    files: List[str], device: Optional[Union[torch.device, str]] = None, load_textures: bool = True, **kwargs
) -> Meshes:
    if not isinstance(files, list):
        files = [files]
    return _load_objs_as_meshes(files=files, device=device, load_textures=load_textures, **kwargs)
...
def load_plys_as_meshes(
    ...
    for idx in range(len(files)):
        ...
        mesh = writer.load_mesh(path=files[idx], include_textures=load_textures, device=device)
        meshes.append(mesh)
    meshes = join_meshes_as_batch(meshes, include_textures=load_textures)
    return meshes


def save_meshes_as_plys(
    files: List[str],
    meshes: Meshes = None,
    ...
    if meshes is None:
        ...
        meshes = Meshes(
            verts=verts,
            faces=faces,
            textures=TexturesVertex(verts_features=verts_rgb) if verts_rgb is not None else None,
        )
    else:
    ...
    for idx in range(len(meshes)):
        ...
        writer.save_mesh(meshes[idx], files[idx], colors_as_uint8=True, binary=False)
...
def save_meshes_as_objs(files: List[str], meshes: Meshes = None) -> None:
    ...
    for idx in range(len(meshes)):
        prepare_output_path(files[idx], allowed_suffix=[".obj"], path_type="file"), "Please save as .obj files."
