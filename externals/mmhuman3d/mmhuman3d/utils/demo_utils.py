...
def xyxy2xywh(bbox_xyxy):
    ...
    if not isinstance(bbox_xyxy, np.ndarray):
        raise TypeError(f"Input type is {type(bbox_xyxy)}, which should be numpy.ndarray.")
...
def xywh2xyxy(bbox_xywh):
    ...
    if not isinstance(bbox_xywh, np.ndarray):
        raise TypeError(f"Input type is {type(bbox_xywh)}, which should be numpy.ndarray.")
...
def box2cs(bbox_xywh, aspect_ratio=1.0, bbox_scale_factor=1.25):
    ...
    pixel_std = 1
    center = np.stack([bbox_xywh[..., 0] + bbox_xywh[..., 2] * 0.5, bbox_xywh[..., 1] + bbox_xywh[..., 3] * 0.5], -1)
    ...
    scale = np.stack([bbox_xywh[..., 2] * 1.0 / pixel_std, bbox_xywh[..., 3] * 1.0 / pixel_std], -1)
    scale = scale * bbox_scale_factor

    return center, scale


def convert_crop_cam_to_orig_img(
    cam: np.ndarray,
    bbox: np.ndarray,
    img_width: int,
    img_height: int,
    ...
    bbox_format: Literal["xyxy", "xywh", "cs"] = "xyxy",
):
    ...
    if not isinstance(bbox, np.ndarray):
        raise TypeError(f"Input type is {type(bbox)}, which should be numpy.ndarray.")
    bbox = bbox.copy()
    if bbox_format == "xyxy":
...
def convert_bbox_to_intrinsic(
    ...
    if not isinstance(bboxes, np.ndarray):
        raise TypeError(f"Input type is {type(bboxes)}, which should be numpy.ndarray.")
...
def get_default_hmr_intrinsic(num_frame=1, focal_length=1000, det_width=224, det_height=224) -> np.ndarray:
...
def convert_kp2d_to_bbox(kp2d: np.ndarray, bbox_format: Literal["xyxy", "xywh"] = "xyxy") -> np.ndarray:
...
def convert_verts_to_cam_coord(
    verts, pred_cams, bboxes_xy, focal_length=5000.0, bbox_scale_factor=1.25, bbox_format="xyxy"
):
    ...
    """
    K0 = get_default_hmr_intrinsic(focal_length=focal_length, det_height=224, det_width=224)
    K1 = convert_bbox_to_intrinsic(bboxes_xy, bbox_scale_factor=bbox_scale_factor, bbox_format=bbox_format)
    # K1K0(RX+T)-> K0(K0_inv K1K0)
    Ks = np.linalg.inv(K0) @ K1 @ K0
    # convert vertices from world to camera
    cam_trans = np.concatenate(
        [pred_cams[..., [1]], pred_cams[..., [2]], 2 * focal_length / (224 * pred_cams[..., [0]] + 1e-9)], -1
    )
...
def smooth_process(x, smooth_type="savgol", cfg_base_dir="configs/_base_/post_processing/"):
    ...
    assert smooth_type in [
        ...
        "gaus1d",
        "savgol",
        "smoothnet_windowsize8",
        "smoothnet_windowsize16",
        "smoothnet_windowsize32",
    ...
    if isinstance(cfg, str):
        ...
        raise TypeError("config must be a filename or Config object, " f"but got {type(cfg)}")
...
def speed_up_process(x, speed_up_type="deciwatch", cfg_base_dir="configs/_base_/post_processing/"):
...
def get_speed_up_interval(speed_up_type, cfg_base_dir="configs/_base_/post_processing/"):
...
def speed_up_interpolate(selected_frames, speed_up_frames, smpl_poses, smpl_betas, pred_cams, bboxes_xyxy):
    ...
    bboxes_xyxy[:speed_up_frames, :] = interpolate.interp1d(
        selected_frames, bboxes_xyxy[selected_frames, :], kind="linear", axis=0
    )(np.arange(0, max(selected_frames)))
    smpl_betas[:speed_up_frames, :] = interpolate.interp1d(
        selected_frames, smpl_betas[selected_frames, :], kind="linear", axis=0
    )(np.arange(0, max(selected_frames)))

    return smpl_poses, smpl_betas, pred_cams, bboxes_xyxy


def process_mmtracking_results(mmtracking_results, max_track_id, bbox_thr=None):
...
def prepare_frames(input_path=None):
    ...
    if Path(input_path).is_file():
        ...
        # input_type = 'folder'
        file_list = [
            os.path.join(input_path, fn) for fn in os.listdir(input_path) if fn.lower().endswith((".png", ".jpg"))
        ]
        file_list.sort()
        img_list = [mmcv.imread(img_path) for img_path in file_list]
        assert len(img_list), f"Failed to load image from {input_path}"
    else:
        raise ValueError("Input path should be an file or folder." f" Got invalid input path: {input_path}")
    return img_list


def extract_feature_sequence(extracted_results, frame_idx, causal, seq_len, step=1):
    ...
    extracted_results_seq = (
        [extracted_results[0]] * pad_left + extracted_results[start:end:step] + [extracted_results[-1]] * pad_right
    )
    return extracted_results_seq


def get_different_colors(number_of_colors, flag=0, alpha: float = 1.0, mode: str = "bgr", int_dtype: bool = True):
