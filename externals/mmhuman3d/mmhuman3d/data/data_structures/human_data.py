...
_HumanData_SUPPORTED_KEYS = {
    ...
    "bbox_xywh": {"type": np.ndarray, "shape": (-1, 5), "dim": 0},
    "config": {"type": str, "dim": None},
    "keypoints2d": {"type": np.ndarray, "shape": (-1, -1, 3), "dim": 0},
    "keypoints3d": {"type": np.ndarray, "shape": (-1, -1, 4), "dim": 0},
    "smpl": {"type": dict, "slice_key": "betas", "dim": 0},
    "smplh": {"type": dict, "slice_key": "betas", "dim": 0},
    "smplx": {"type": dict, "slice_key": "betas", "dim": 0},
    "meta": {
        "type": dict,
    },
    "keypoints2d_mask": {"type": np.ndarray, "shape": (-1,), "dim": None},
    "keypoints2d_convention": {"type": str, "dim": None},
    "keypoints3d_mask": {"type": np.ndarray, "shape": (-1,), "dim": None},
    "keypoints3d_convention": {"type": str, "dim": None},
    "vertices": {"type": np.ndarray, "shape": (-1,), "dim": None},
    "misc": {
        "type": dict,
    },
}
...
class HumanData(dict):
    ...
    @classmethod
    def new(cls, source_dict: dict = None, key_strict: bool = False) -> _HumanData:
    ...
    def set_key_strict(self, value: bool):
        ...
        if former__key_strict__ is False and value is True:
        ...
        with np.load(npz_path, allow_pickle=True) as npz_file:
            ...
            for key, value in list(tmp_data_dict.items()):
                if isinstance(value, np.ndarray) and len(value.shape) == 0:
                    ...
                    value = value.item()
                elif key in supported_keys and type(value) != supported_keys[key]["type"]:
                    value = supported_keys[key]["type"](value)
                if value is None:
                    tmp_data_dict.pop(key)
                elif key == "__key_strict__" or key == "__data_len__" or key == "__keypoints_compressed__":
    ...
    def get_sliced_cache(self, slice_size=10) -> List:
        ...
        for key, dim in dim_dict.items():
            # no dim to slice
            if dim is None:
                if key.startswith("keypoints") and (key.endswith("_mask") or key.endswith("_convention")):
                ...
                for sub_key in value_dict.keys():
                    ...
                    if dim[sub_key] is None:
                        ...
                        for slice_index in range(slice_num):
                            ...
                            slice_end = min((slice_index + 1) * slice_size, self.__data_len__)
                            slice_range = slice(slice_start, slice_end)
                            sliced_sub_value = HumanData.__get_sliced_result__(sub_value, sub_dim, slice_range)
                            if key not in sliced_data[str(slice_index)]:
                                sliced_data[str(slice_index)][key] = {}
                            sliced_data[str(slice_index)][key][sub_key] = sliced_sub_value
        ...
        for key in self.keys():
            ...
            if tensor_value is None:
                ...
                if memory_format is None:
                    ret_dict[key] = tensor_value.to(device, dtype, non_blocking, copy)
                else:
                    ret_dict[key] = tensor_value.to(device, dtype, non_blocking, copy, memory_format=memory_format)
        return ret_dict
    ...
    def __getitem__(self, key: _KT) -> _VT:
        ...
        if self.__keypoints_compressed__:
            ...
            if key in self and isinstance(value, np.ndarray) and "keypoints" in key and mask_key in self:
                mask_array = np.asarray(super().__getitem__(mask_key))
                value = self.__class__.__add_zero_pad__(value, mask_array)
        return value
    ...
    def get_value_in_shape(self, key: _KT, shape: Union[list, tuple], padding_constant: int = 0) -> np.ndarray:
        ...
        for dim_index in range(len(shape)):
            if shape[dim_index] == -1:
                ...
                # invalid
                raise ValueError
        pad_value = np.pad(value, pad_width=pad_width_list, mode="constant", constant_values=padding_constant)
    ...
    def get_slice(self, arg_0: int, arg_1: Union[int, Any] = None, step: int = 1) -> _HumanData:
            ...
            if dim is None:
                ...
                for sub_key in value_dict.keys():
                    ...
                    if dim[sub_key] is None:
                        ...
                        sub_dim = dim[sub_key]
                        sliced_sub_value = HumanData.__get_sliced_result__(sub_value, sub_dim, slice_index)
                        sliced_dict[sub_key] = sliced_sub_value
                ret_human_data[key] = sliced_dict
            else:
                value = self[key]
                sliced_value = HumanData.__get_sliced_result__(value, dim, slice_index)
    ...
    def __get_slice_dim__(self) -> dict:
        ...
        for key in self.keys():
            # keys not expected be sliced
            if key in supported_keys and "dim" in supported_keys[key] and supported_keys[key]["dim"] is None:
                ret_dict[key] = None
            else:
                value = self[key]
                ...
                slice_dim = 0
                if key in supported_keys and "dim" in supported_keys[key]:
                    slice_dim = supported_keys[key]["dim"]
                data_len = value_len if slice_dim == 0 else value.shape[slice_dim]
                # dim not for slice
    ...
    def __setitem__(self, key: _KT, val: _VT) -> None:
        ...
        if self.__keypoints_compressed__:
            ...
            if "keypoints" in key and "_mask" in key:
                msg = "Mask cannot be modified " + "in keypoints_compressed mode."
                print_log(msg=msg, logger=class_logger, level=logging.WARN)
                return
            elif isinstance(val, np.ndarray) and "keypoints" in key and "_mask" not in key:
                mask_key = f"{key}_mask"
                if mask_key in self:
                    mask_array = np.asarray(super().__getitem__(mask_key))
                    val = self.__class__.__remove_zero_pad__(val, mask_array)
                else:
                    msg = f"Mask for {key} has not been set." + f" Please set {mask_key} before compression."
                    print_log(msg=msg, logger=class_logger, level=logging.ERROR)
                    raise ValueError
    ...
    def __check_key__(self, key: Any) -> _KeyCheck:
        ...
        if self.get_key_strict():
            ...
            if key not in self.__class__.SUPPORTED_KEYS and key not in self.__class__.WARNED_KEYS:
        ...
        if ret_key_check == _KeyCheck.ERROR:
            ...
            if class_logger == "silent":
                ...
                print_log(msg=self.__class__.__get_key_warn_msg__(key), logger=class_logger, level=logging.WARN)
    ...
    def __check_value__(self, key: Any, val: Any) -> bool:
        ...
        ret_bool = (
            ...
            and self.__check_value_shape__(key, val)
            and self.__check_value_len__(key, val)
        )
        if not ret_bool:
    ...
    def __check_value_type__(self, key: Any, val: Any) -> bool:
        ...
        if not ret_bool:
            ...
            print_log(msg=err_msg, logger=self.__class__.logger, level=logging.ERROR)
        return ret_bool
        ...
        if key in supported_keys:
            ...
            if "shape" in supported_keys[key]:
                ...
                for shape_ind in range(len(supported_keys[key]["shape"])):
                    ...
                    # value not match
                    if expect_val > 0 and expect_val != val_shape[shape_ind]:
                        ret_bool = False
                        break
        if not ret_bool:
    ...
    def __check_value_len__(self, key: Any, val: Any) -> bool:
        ...
        if key in supported_keys:
            ...
            if "dim" in supported_keys[key] and supported_keys[key]["dim"] is not None:
    ...
    def generate_mask_from_confidence(self, keys=None) -> None:
        ...
        if keys is None:
            ...
            for key in self.keys():
                ...
                if isinstance(val, np.ndarray) and "keypoints" in key and "_mask" not in key:
                    keys.append(key)
            ...
            raise TypeError(f"`Keys` must be None, str, or list of str, " f"got {type(keys)}.")
        ...
        for key in self.keys():
            ...
            if isinstance(val, np.ndarray) and "keypoints" in key and "_mask" not in key:
                if mask_key in self:
                    ...
                    class_logger = self.__class__.logger
                    msg = f"Mask for {key} has not been found." + f"Please remove {key} before decompression."
                    print_log(msg=msg, logger=class_logger, level=logging.ERROR)
                    raise KeyError
        decompressed_dict = {}
        for kpt_key, mask_key in key_pairs:
            ...
            kpt_array = self.__class__.__add_zero_pad__(compressed_kpt, mask_array)
    ...
    def dump_by_pickle(self, pkl_path: str, overwrite: bool = True) -> None:
        ...
        with open(pkl_path, "wb") as f_writeb:
            pickle.dump(dict_to_dump, f_writeb, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def __set_default_values__(self) -> None:
        ...
        if self.__data_len__ == -1:
            for key in supported_keys:
                if key in self and "dim" in supported_keys[key] and supported_keys[key]["dim"] is not None:
                    if "slice_key" in supported_keys[key] and supported_keys[key]["type"] == dict:
                        sub_key = supported_keys[key]["slice_key"]
                        slice_dim = supported_keys[key]["dim"]
                        self.__data_len__ = self[key][sub_key].shape[slice_dim]
                    else:
                        slice_dim = supported_keys[key]["dim"]
        ...
        for key in list(self.keys()):
            ...
            if (
                key.startswith("keypoints")
                and not key.endswith("_mask")
                and not key.endswith("_convention")
                and convention_key not in self
            ):
                self[convention_key] = "human_data"

    @classmethod
    def concatenate(cls, human_data_0: _HumanData, human_data_1: _HumanData) -> _HumanData:
        ...
        for key in common_keys:
            ...
            # align type
            value_0 = list(value_0) if isinstance(value_0, tuple) else value_0
            value_1 = list(value_1) if isinstance(value_1, tuple) else value_1
            assert type(value_0) == type(value_1)
            # align convention
            if key.startswith("keypoints") and key.endswith("_convention"):
                assert value_0 == value_1
                ret_human_data[key] = value_0
                continue
            # mask_0 and mask_1
            elif key.startswith("keypoints") and key.endswith("_mask"):
            ...
            # try concat
                ...
                concat_dict = cls.__concat_value__(
                    key=key, value_0=value_0, dim_0=dim_dict_0[key], value_1=value_1, dim_1=dim_dict_1[key]
                )
    ...
    @classmethod
    def __concat_value__(
        cls, key: Any, value_0: Any, value_1: Any, dim_0: Union[None, int], dim_1: Union[None, int]
    ) -> dict:
    ...
    @classmethod
    def __add_zero_pad__(cls, compressed_array: np.ndarray, mask_array: np.ndarray) -> np.ndarray:
        ...
        ret_value = np.zeros(shape=[data_len, mask_len, dim], dtype=compressed_array.dtype)
        valid_mask_index = np.where(mask_array == 1)[0]
        ret_value[:, valid_mask_index, :] = compressed_array
        return ret_value

    @classmethod
    def __remove_zero_pad__(cls, zero_pad_array: np.ndarray, mask_array: np.ndarray) -> np.ndarray:
    ...
    def __get_key_warn_msg__(cls, key: Any) -> str:
        ...
        warn_message = f"{key} is absent in" + f" {class_name}.SUPPORTED_KEYS.\n"
        suggestion_message = (
    ...
    def __get_key_error_msg__(cls, key: Any) -> str:
        ...
        """
        class_name = cls.__name__
        absent_message = f"{key} is absent in" + f" {class_name}.SUPPORTED_KEYS.\n"
        suggestion_message = "Call self.set_key_strict(False)" + " to allow unsupported keys.\n"
    ...
    def __get_value_error_msg__(cls) -> str:
        ...
        Returns:
            str:
                The error message.
        """
        error_message = "An supported value doesn't " + "match definition.\n"
        suggestion_message = "See error log for details.\n"
        return error_message + suggestion_message

    @classmethod
    def __get_sliced_result__(
        cls, input_data: Union[np.ndarray, list, tuple], slice_dim: int, slice_range: slice
        ...
        if isinstance(input_data, np.ndarray):
            ...
            sliced_data = input_data[slice_range]
        return sliced_data
