...
_MultiHumanData_SUPPORTED_KEYS.update({"frame_range": {"type": np.ndarray, "shape": (-1, 2), "dim": 0}})
...
class MultiHumanData(HumanData):
        ...
        with np.load(npz_path, allow_pickle=True) as npz_file:
            ...
            for key, value in list(tmp_data_dict.items()):
                ...
                if value is None:
                    ...
                    key == "__key_strict__"
                    or key == "__data_len__"
                    or key == "__instance_num__"
                    or key == "__keypoints_compressed__"
                ):
    ...
    def get_slice(self, arg_0: int, arg_1: Union[int, Any] = None, step: int = 1) -> _HumanData:
        ...
        """
        ret_human_data = MultiHumanData.new(key_strict=self.get_key_strict())
        if arg_1 is None:
            start = 0
            stop = arg_0
        else:
            ...
            if dim is None:
                ...
                for sub_key in value_dict.keys():
                    ...
                    if dim[sub_key] is None:
                        ...
                        sub_dim = dim[sub_key]
                        sliced_sub_value = MultiHumanData.__get_sliced_result__(
                ...
                value = self[key]
                sliced_value = MultiHumanData.__get_sliced_result__(value, dim, slice_index, frame_range)
    ...
    def __get_slice_dim__(self) -> dict:
        ...
        for key in self.keys():
            ...
            if key in supported_keys and "dim" in supported_keys[key] and supported_keys[key]["dim"] is None:
                ...
                if isinstance(value, dict) and len(value) > 0:
                    ...
                    for sub_key in value.keys():
                        try:
                            ...
                            if sub_value_len != self.instance_num and sub_value_len != self.data_len:
                ...
                # dim not for slice
                if data_len != self.__instance_num__ and data_len != self.__data_len__:
                    ret_dict[key] = None
                    continue
                else:
        ...
        if "frame_range" not in self:
            ...
            for key in supported_keys:
                if key in self and "dim" in supported_keys[key] and supported_keys[key]["dim"] is not None:
                    ...
                    frame_range = [[i, i + 1] for i in range(self.data_len)]
    ...
    def __get_sliced_result__(
        cls, input_data: Union[np.ndarray, list, tuple], slice_dim: int, slice_range: slice, frame_index: list = None
        ...
        if frame_index is not None:
            ...
            for frame_range in frame_index[slice_range]:
                ...
                slice_result = HumanData.__get_sliced_result__(input_data, slice_dim, slice_index)
            ...
            slice_data = HumanData.__get_sliced_result__(input_data, slice_dim, slice_range)
        return slice_data
