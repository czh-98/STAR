...
class SMCReader:
    ...
    def __init__(self, file_path, body_model=None):
        ...
        if self.iphone_exists:
            ...
            self.iphone_color_resolution = self.smc["iPhone"].attrs["color_resolution"]  # vertical
            self.iphone_depth_resolution = self.smc["iPhone"].attrs["depth_resolution"]  # vertical
        ...
        if self.keypoint_exists:
            ...
            self.keypoints_convention = self.smc["Keypoints3D"].attrs["convention"]
            self.keypoints_created_time = self.smc["Keypoints3D"].attrs["created_time"]
    ...
    def get_kinect_color_extrinsics(self, kinect_id, homogeneous=True):
        ...
        """
        R = np.asarray(self.calibration_dict[str(kinect_id * 2)]["R"]).reshape(3, 3)
        T = np.asarray(self.calibration_dict[str(kinect_id * 2)]["T"]).reshape(3)
        if homogeneous:
        ...
        Returns:
            ...
            homogeneous is False
                dict: A dict of rotation and translation,
                        ...
                        each value is an ndarray.
        """
        R = np.asarray(self.calibration_dict[str(kinect_id * 2 + 1)]["R"]).reshape(3, 3)
        T = np.asarray(self.calibration_dict[str(kinect_id * 2 + 1)]["T"]).reshape(3)
        if homogeneous:
            extrinsics = np.identity(4, dtype=float)
            extrinsics[:3, :3] = R
            extrinsics[:3, 3] = T
    ...
    def get_kinect_color_intrinsics(self, kinect_id):
        ...
        intrinsics = kinect_dict["Calibration"]["Color"]["Intrinsics"][()]
    ...
    def get_kinect_color_resolution(self, kinect_id):
        ...
        """
        kinect_dict = self.smc["Kinect"][str(kinect_id)]
        resolution = kinect_dict["Calibration"]["Color"]["Resolution"][()]
        return resolution
    ...
    def get_kinect_depth_resolution(self, kinect_id):
        ...
        """
        kinect_dict = self.smc["Kinect"][str(kinect_id)]
        resolution = kinect_dict["Calibration"]["Depth"]["Resolution"][()]
        return resolution
    ...
    def get_kinect_depth_intrinsics(self, kinect_id):
        ...
        """
        kinect_dict = self.smc["Kinect"][str(kinect_id)]
        intrinsics = kinect_dict["Calibration"]["Depth"]["Intrinsics"][()]
        cx, cy, fx, fy = intrinsics[:4]
        intrinsics = np.asarray([[fx, 0, cx], [0, fy, cy], [0, 0, 1]])
        return intrinsics
    ...
    def get_iphone_intrinsics(self, iphone_id=0, frame_id=0, vertical=True):
        ...
        Returns:
            ndarray: A 3x3 matrix.
        """
        camera_info = self.smc["iPhone"][str(iphone_id)]["CameraInfo"][str(frame_id)]
    ...
    def get_kinect_color(self, kinect_id, frame_id=None, disable_tqdm=True):
        ...
        if frame_id is None:
            ...
            assert frame_id < self.get_kinect_num_frames(), "Index out of range..."
            frame_list = [frame_id]
        else:
            raise TypeError("frame_id should be int, list or None.")
        for i in tqdm.tqdm(frame_list, disable=disable_tqdm):
            frames.append(self.__read_color_from_bytes__(self.smc["Kinect"][str(kinect_id)]["Color"][str(i)][()]))
        return np.stack(frames, axis=0)

    def get_kinect_rgbd(self, kinect_id, frame_id, mode="color2depth", threshold=0):
        if mode == "color2depth":
            mapped_color = self.__map_color_to_depth__(kinect_id, frame_id, threshold=threshold)
    ...
    def get_kinect_depth(self, kinect_id, frame_id=None, disable_tqdm=True):
        ...
        for i in tqdm.tqdm(frame_list, disable=disable_tqdm):
            frames.append(self.smc["Kinect"][str(kinect_id)]["Depth"][str(i)][()])
        return np.stack(frames, axis=0)

    def __read_color_from_bytes__(self, color_array):
        """Decode an RGB image from an encoded byte array."""
        return cv2.cvtColor(cv2.imdecode(color_array, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
    ...
    def get_iphone_color(self, iphone_id=0, frame_id=None, disable_tqdm=True, vertical=True):
        ...
        if frame_id is None:
            ...
            assert frame_id < self.get_iphone_num_frames(), "Index out of range..."
            frame_list = [frame_id]
        else:
            raise TypeError("frame_id should be int, list or None.")
        for i in tqdm.tqdm(frame_list, disable=disable_tqdm):
            frame = self.__read_color_from_bytes__(self.smc["iPhone"][str(iphone_id)]["Color"][str(i)][()])
            if vertical:
                frame = cv2.rotate(frame, cv2.ROTATE_90_CLOCKWISE)
            frames.append(frame)
        return np.stack(frames, axis=0)

    def get_iphone_depth(self, iphone_id=0, frame_id=None, disable_tqdm=True, vertical=True):
        """Get several frames captured by an iPhone RGB camera.
    ...
    def get_kinect_transformation_depth_to_color(self, device_id):
        ...
        """
        return np.linalg.inv(self.get_kinect_color_extrinsics(device_id)) @ self.get_kinect_depth_extrinsics(device_id)
    ...
    def get_kinect_transformation_color_to_depth(self, device_id):
        ...
        Returns:
            ndarray: A 4x4 transformation matrix.
        """
        return np.linalg.inv(self.get_kinect_depth_extrinsics(device_id)) @ self.get_kinect_color_extrinsics(device_id)
    ...
    def __map_color_to_depth__(self, device_id, frame_id, threshold=100):
        ...
        normalized_points = depth_intrinsic_inv[None, ...] @ homo_indices[..., None]
        ...
        R = Td2c[:3, :3]
        T = Td2c[:3, 3]
        valid_points = R[None, ...] @ valid_points[..., None] + T[None, ..., None]
        valid_uvs = color_intrinsic[None, ...] @ valid_points / valid_points[:, 2][..., None]
        ...
        mapped_color = np.ones((depth_image.shape[0], depth_image.shape[1], 3), dtype=np.uint8) * 255
        mapped_color[mask >= threshold] = color_image[valid_uvs[:, 1], valid_uvs[:, 0]]
    ...
    def get_keypoints2d(self, device, device_id, frame_id=None, vertical=True):
        ...
        """
        assert device in {"Kinect", "iPhone"}, f'Undefined device: {device}, should be "Kinect" or "iPhone"'
        assert device_id >= 0
        ...
        if frame_id is None:
            ...
            assert frame_id < self.get_keypoints_num_frames(), "Index out of range..."
            frame_list = [frame_id]
        else:
            raise TypeError("frame_id should be int, list or None.")
        ...
        if device == "iPhone" and vertical:
            ...
            W, H = self.get_iphone_color_resolution(vertical=False)
            xs, ys, conf = keypoints2d[..., 0], keypoints2d[..., 1], keypoints2d[..., 2]
    ...
    def get_iphone_keypoints2d(self, device_id=0, frame_id=None, vertical=True):
        ...
        """
        assert device_id >= 0
        return self.get_keypoints2d("iPhone", device_id, frame_id, vertical=vertical)

    def get_color(self, device, device_id, frame_id=None, disable_tqdm=True, vertical=True):
        ...
        if device == "Kinect":
            ...
            img = self.get_iphone_color(device_id, frame_id, disable_tqdm, vertical=vertical)
    ...
    def get_keypoints3d(self, device=None, device_id=None, frame_id=None, vertical=True):
        ...
        # return keypoints3d in device coordinate system
            ...
            if device == "Kinect":
                cam2world = self.get_kinect_color_extrinsics(kinect_id=device_id, homogeneous=True)
            else:
                cam2world = self.get_iphone_extrinsics(iphone_id=device_id, vertical=vertical)
    ...
    def get_smpl(self, device=None, device_id=None, frame_id=None, vertical=True):
        ...
        if device is None:
            smpl_dict = dict(global_orient=global_orient, body_pose=body_pose, transl=transl, betas=betas)
        ...
        # return SMPL parameters in device coordinate system
            ...
            if self.body_model is None:
                self.body_model = build_body_model(self.default_body_model_config)
            ...
            if device == "Kinect":
                T_cam2world = self.get_kinect_color_extrinsics(kinect_id=device_id, homogeneous=True)
            else:
                T_cam2world = self.get_iphone_extrinsics(iphone_id=device_id, vertical=vertical)
            ...
            new_global_orient, new_transl = batch_transform_to_camera_frame(
                global_orient=global_orient, transl=transl, pelvis=pelvis, extrinsic=T_world2cam
            )

            smpl_dict = dict(global_orient=new_global_orient, body_pose=body_pose, transl=new_transl, betas=betas)
