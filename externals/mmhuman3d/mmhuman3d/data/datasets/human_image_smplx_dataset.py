...
class HumanImageSMPLXDataset(HumanImageDataset):
    ...
    # metric
    ALLOWED_METRICS = {"mpjpe", "pa-mpjpe", "pve", "3dpck", "pa-3dpck", "3dauc", "pa-3dauc", "3DRMSE", "pa-pve"}
    ...
    def __init__(
        ...
        super().__init__(
            data_prefix, pipeline, dataset_name, body_model, ann_file, convention, cache_data_path, test_mode
        )
    ...
    def _parse_result(self, res, mode="keypoint", body_part=""):
        if mode == "vertice":
            # pred
            pred_vertices = res["vertices"] * 1000.0
            # gt
            ...
            if self.dataset_name in {"pw3d", "3DPW", "3dpw"}:
                ...
                gt_vertices = self.human_data["vertices"].copy()
                if body_part == "J14":
                    gt_keypoints3d = torch.einsum(
                        "bik,ji->bjk", [torch.from_numpy(gt_vertices).float(), self.body_model.joints_regressor]
                    ).numpy()
                    pred_vertices = res["vertices"]
                    pred_keypoints3d = torch.einsum(
                        "bik,ji->bjk", [torch.from_numpy(pred_vertices).float(), self.body_model.joints_regressor]
                    ).numpy()
                    gt_keypoints3d_mask = np.ones((len(pred_keypoints3d), gt_keypoints3d.shape[1]))
                else:
                    gt_keypoints3d = torch.einsum(
                        "bik,ji->bjk", [torch.from_numpy(gt_vertices).float(), self.body_model.J_regressor]
                    ).numpy()
                    ...
                    pred_vertices = res["vertices"]
                    pred_keypoints3d = torch.einsum(
                        "bik,ji->bjk", [torch.from_numpy(pred_vertices).float(), self.body_model.J_regressor]
                    ).numpy()
    ...
    def _report_3d_rmse(self, res_file):
        ...
        ground truth scan."""
        pred_vertices, gt_vertices, _ = self._parse_result(res_file, mode="vertice")
        pred_keypoints3d, gt_keypoints3d, _ = self._parse_result(res_file, mode="keypoint")
        errors = []
        for pred_vertice, gt_vertice, pred_points, gt_points in zip(
            pred_vertices, gt_vertices, pred_keypoints3d, gt_keypoints3d
        ):
            error = fg_vertices_to_mesh_distance(
                gt_vertice, gt_points, pred_vertice, self.body_model.faces, pred_points
            )
            errors.append(error)
