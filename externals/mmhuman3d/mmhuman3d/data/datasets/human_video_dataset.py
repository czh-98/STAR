...
def split_into_chunks(data_infos: list, seq_len: int, stride: int, test_mode: bool, only_vid_name: bool):
    ...
    for idx in range(len(video_names)):
        ...
        if chunks[-1][-1] < indexes[-1] and test_mode:
            ...
            extra_start_finish = [[start_frame, end_frame, valid_start_frame, valid_end_frame]]
...
class HumanVideoDataset(HumanImageDataset):
    ...
    def __init__(
        ...
        self.vid_indices = split_into_chunks(
            self.human_data["image_path"], self.seq_len, self.stride, test_mode, only_vid_name
        )
    ...
    def prepare_data(self, idx: int):
        ...
        for frame_idx in range(start_idx, end_idx + 1):
            ...
            if "features" in self.human_data:
                frame_results["features"] = copy.deepcopy(self.human_data["features"][frame_idx])
        ...
        img_metas = {"frame_idx": self.vid_indices[idx], "image_path": image_path}
