...
def get_affine_transform(center, scale, rot, output_size, shift=(0.0, 0.0), inv=False, pixel_std=1.0):
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, results):
        ...
        if "smplx_right_hand_pose" in results:
            ...
            results["smplx_right_hand_pose"], results["smplx_left_hand_pose"] = _flip_hand_pose(
...
class Lighting(object):
    ...
    def __init__(self, eigval, eigvec, alphastd=0.1, to_rgb=True):
        ...
        for vec in eigvec:
            assert isinstance(vec, list) and len(vec) == len(eigvec[0]), "eigvec must contains lists with equal length."
...
class MeshAffine:
    ...
    def __call__(self, results):
        ...
        inv_trans = get_affine_transform(c, s, 0.0, self.image_size, inv=True)
...
class Rotation:
    ...
    def __call__(self, results):
        ...
        if "keypoints2d" in results:
            keypoints2d = results["keypoints2d"].copy()
            keypoints2d[:, :2] = (np.dot(keypoints2d[:, :2], M[:2, :2].T) + M[:2, 2] + 1).astype(np.int)
            results["keypoints2d"] = keypoints2d
...
class BBoxCenterJitter(object):
    ...
    def __init__(self, factor=0.0, dist="normal"):
        ...
        self.factor = factor
        self.dist = dist
        assert self.dist in ["normal", "uniform"], f"Distribution must be normal or uniform, not {self.dist}"
...
class SimulateLowRes(object):
    ...
    def _sample_low_res(self, image: np.ndarray) -> np.ndarray:
        ...
        if self.dist == "uniform":
            ...
            if not downsample:
                return image
            factor = np.random.rand() * (self.factor_max - self.factor_min) + self.factor_min
        ...
        H, W, _ = image.shape
        downsampled_image = cv2.resize(image, (int(W // factor), int(H // factor)), cv2.INTER_NEAREST)
...
class GetBboxInfo:
    ...
    def __call__(self, results):
        ...
        bbox_info = np.stack([cx - img_w / 2.0, cy - img_h / 2.0, s])
