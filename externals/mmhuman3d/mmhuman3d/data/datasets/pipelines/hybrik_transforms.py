...
def heatmap2coord(pred_jts, pred_scores, hm_shape, bbox, output_3d=False, mean_bbox_scale=None):
...
def transform_preds(coords, center, scale, output_size):
    ...
    trans = get_affine_transform(center, scale, 0, output_size, inv=1, pixel_std=1)
...
class RandomDPG(object):
    ...
    def __call__(self, results):
        ...
        if PatchScale > 0.85:
            ...
            ymin = max(1, min(bbox[1] + np.random.normal(0.0043, 0.068) * ht, imgheight - 3))
            xmax = min(max(xmin + 2, bbox[2] + np.random.normal(0.0154, 0.1337) * width), imgwidth - 3)
            ymax = min(max(ymin + 2, bbox[3] + np.random.normal(-0.0013, 0.0711) * ht), imgheight - 3)
        bbox_xyxy = np.array([xmin, ymin, xmax, ymax])
        bbox_xywh = xyxy2xywh(bbox_xyxy)
        center, scale = box2cs(bbox_xywh, aspect_ratio=1.0, bbox_scale_factor=1.0)
...
class HybrIKRandomFlip:
    ...
    def __call__(self, results):
        ...
        keypoints3d, keypoints3d_vis = flip_joints_3d(keypoints3d, keypoints3d_vis, width, self.flip_pairs)
        ...
        if results["has_smpl"]:
            ...
            keypoints3d17, keypoints3d17_vis = flip_joints_3d(keypoints3d17, keypoints3d17_vis, width, self.flip_pairs)
            keypoints3d17_relative = flip_xyz_joints_3d(keypoints3d17_relative, self.flip_pairs)
            keypoints3d_relative = flip_xyz_joints_3d(keypoints3d_relative, self.flip_pairs)
            twist_phi, twist_weight = results["target_twist"], results["target_twist_weight"]
            results["target_twist"], results["target_twist_weight"] = flip_twist(
                twist_phi, twist_weight, smpl_flip_pairs
            )

            results["keypoints3d17_relative"] = keypoints3d17_relative.astype(np.float32)
            results["keypoints3d_relative"] = keypoints3d_relative.astype(np.float32)
...
class HybrIKAffine:
    ...
    def __call__(self, results):
        ...
        img = cv2.warpAffine(img, trans, (int(self.image_size[0]), int(self.image_size[1])), flags=cv2.INTER_LINEAR)

        for i in range(num_joints):
            if keypoints3d_vis[i, 0] > 0.0:
                keypoints3d[i, 0:2] = affine_transform(keypoints3d[i, 0:2], trans)
        ...
        if has_smpl:
            ...
            for i in range(17):
                if keypoints3d17_vis[i, 0] > 0.0:
                    keypoints3d17[i, 0:2] = affine_transform(keypoints3d17[i, 0:2], trans)
...
class GenerateHybrIKTarget:
    ...
    def _integral_uvd_target_generator(
        self, joints_3d, num_joints, patch_height, patch_width, depth_factor, test_mode=False
    ):
    ...
    def _integral_target_generator(self, joints_3d, num_joints, patch_height, patch_width, depth_factor):
        target_weight = np.ones((num_joints, 3), dtype=np.float32)
        target_weight[:, 0] = joints_3d[:, 0, 1]
        target_weight[:, 1] = joints_3d[:, 0, 1]
        target_weight[:, 2] = joints_3d[:, 0, 1]
    ...
    def _integral_xyz_target_generator(self, joints_3d, joints_3d_vis, num_joints, depth_factor):
        target_weight = np.ones((num_joints, 3), dtype=np.float32)
    ...
    def _integral_target_generator_coco(self, joints_3d, num_joints, patch_height, patch_width):
    ...
    def __call__(self, results):
        ...
        #  generate new keys
        trans_inv = get_affine_transform(c, s, r, self.image_size, inv=True, pixel_std=1).astype(np.float32)
        ...
        if has_smpl:
            theta = results["pose"]
            # aa to quat
            results["target_theta"] = aa_to_quat_numpy(theta).reshape(24 * 4).astype(np.float32)
            theta_24_weights = np.ones((24, 4))
            results["target_theta_weight"] = theta_24_weights.reshape(24 * 4).astype(np.float32)
            ...
            target_xyz_17, target_weight_17 = self._integral_xyz_target_generator(
                keypoints3d17_relative, keypoints3d17_vis, 17, depth_factor
            )
            target_xyz_24, target_weight_24 = self._integral_xyz_target_generator(
                joints24_relative_3d, keypoints3d_vis[:24, :], 24, depth_factor
            )
            ...
            if results["ann_info"]["dataset_name"] == "coco":
                target, target_weight = self._integral_target_generator_coco(gt_joints, num_joints, inp_h, inp_w)
                ...
                label_jts_mask_origin = label_jts_mask_origin.reshape(num_joints, 2)
                label_jts_origin[mask_idx] = label_jts_origin[mask_idx] * 0
                label_jts_mask_origin[mask_idx] = label_jts_origin[mask_idx] * 0
                label_uvd_29 = np.hstack([label_jts_origin, np.zeros([29, 1])])
                label_uvd_29_mask = np.hstack([label_jts_mask_origin, np.zeros([29, 1])])
                ...
                if not self.test_mode:
                    ...
                    label_jts_mask_origin = label_jts_mask_origin.reshape(num_joints, 3)
                    label_jts_origin[mask_idx] = label_jts_origin[mask_idx] * 0
                    label_jts_mask_origin[mask_idx] = label_jts_origin[mask_idx] * 0
            ...
            results["target_theta_weight"] = np.zeros(24 * 4).astype(np.float32)
        ...
        for map in self.maps:
            for keypoint in map["keypoints"]:
                ...
                if keypoint in results:
                    results[keypoint] = results[keypoint][..., keypoints_index, :]
        return results
