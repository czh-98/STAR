...
class HumanImageDataset(BaseDataset, metaclass=ABCMeta):
    ...
    # metric
    ALLOWED_METRICS = {"mpjpe", "pa-mpjpe", "pve", "3dpck", "pa-3dpck", "3dauc", "pa-3dauc", "ihmr"}
    ...
    def __init__(
        ...
        super(HumanImageDataset, self).__init__(data_prefix, pipeline, ann_file, test_mode, dataset_name)
    ...
    def load_annotations(self):
        ...
        if use_human_data:
            ...
            if "keypoints3d" in self.human_data:
                ...
                keypoints3d, keypoints3d_mask = convert_kps(
                    keypoints3d, src="human_data", dst=self.convention, mask=keypoints3d_mask
                )
            ...
            if "keypoints2d" in self.human_data:
                ...
                keypoints2d, keypoints2d_mask = convert_kps(
                    keypoints2d, src="human_data", dst=self.convention, mask=keypoints2d_mask
                )
        ...
        if self.cache_data_path is not None:
            ...
            if world_size > 1:
                dist.barrier()
            self.cache_reader = HumanDataCacheReader(npz_path=self.cache_data_path)
    ...
    def _parse_result(self, res, mode="keypoint", body_part=None):
        ...
        if mode == "vertice":
            ...
            gt_output = self.body_model(betas=gt_beta, body_pose=gt_pose, global_orient=gt_global_orient, gender=gender)
            ...
            if self.dataset_name == "pw3d":
                ...
                gt_output = self.body_model(
                    betas=betas, body_pose=body_pose, global_orient=global_orient, gender=gender
                )
                ...
                gt_keypoints3d = self.human_data["keypoints3d"][:, h36m_idxs, :3]
                ...
                body_pose = torch.FloatTensor(body_pose).view(-1, 69)
                global_orient = torch.FloatTensor(global_orient)
                gt_output = self.body_model(betas=betas, body_pose=body_pose, global_orient=global_orient)
                gt_keypoints3d = gt_output["joints"].detach().cpu().numpy()
                gt_keypoints3d_mask = np.ones((len(pred_keypoints3d), 24))
            else:
                raise NotImplementedError()
    ...
    def _report_mpjpe(self, res_file, metric="mpjpe", body_part=""):
        ...
        pred_keypoints3d, gt_keypoints3d, gt_keypoints3d_mask = self._parse_result(
            res_file, mode="keypoint", body_part=body_part
        )
    ...
    def _report_3d_pck(self, res_file, metric="3dpck"):
        ...
        pred_keypoints3d, gt_keypoints3d, gt_keypoints3d_mask = self._parse_result(res_file)
    ...
    def _report_pve(self, res_file, metric="pve", body_part=""):
        ...
        pred_verts, gt_verts, _ = self._parse_result(res_file, mode="vertice", body_part=body_part)
        err_name = metric.upper()
        if body_part != "":
            err_name = body_part.upper() + " " + err_name
    ...
    def _report_ihmr(self, res_file):
        ...
        mean_output = self.body_model(
            betas=mean_shape, body_pose=mean_pose[:, 1:], global_orient=mean_pose[:, :1], pose2rot=False
        )
        mean_verts = mean_output["vertices"].detach().cpu().numpy() * 1000.0
        ...
        def report_ihmr_idx(idx):
            ...
            mpjpe = keypoint_mpjpe(pred_keypoints3d[idx], gt_keypoints3d[idx], gt_keypoints3d_mask[idx], "none")
            pampjpe = keypoint_mpjpe(pred_keypoints3d[idx], gt_keypoints3d[idx], gt_keypoints3d_mask[idx], "procrustes")
        ...
        def report_ihmr_all(num_bin):
            num_per_bin = np.array([0 for _ in range(num_bin)]).astype(np.float32)
            sum_mpvpe = np.array([0 for _ in range(num_bin)]).astype(np.float32)
            sum_mpjpe = np.array([0 for _ in range(num_bin)]).astype(np.float32)
            sum_pampjpe = np.array([0 for _ in range(num_bin)]).astype(np.float32)
