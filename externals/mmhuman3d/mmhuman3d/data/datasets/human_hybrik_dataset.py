...
class HybrIKHumanImageDataset(BaseDataset, metaclass=ABCMeta):
    ...
    # metric
    ALLOWED_METRICS = {"mpjpe", "pa-mpjpe", "pve", "3dpck", "pa-3dpck", "3dauc", "pa-3dauc"}
    ...
    def __init__(
        ...
        super(HybrIKHumanImageDataset, self).__init__(data_prefix, pipeline, ann_file, test_mode)
    ...
    def load_annotations(self):
        ...
        try:
            self.keypoints3d, self.keypoints3d_vis = self.get_3d_keypoints_vis(data["keypoints2d"])
        except KeyError:
            self.keypoints3d, self.keypoints3d_vis = self.get_3d_keypoints_vis(data["keypoints3d"])
        ...
        try:
            ...
            self.keypoints3d_relative, _ = self.get_3d_keypoints_vis(data["keypoints3d_relative"])
            self.keypoints3d17, self.keypoints3d17_vis = self.get_3d_keypoints_vis(data["keypoints3d17"])
            self.keypoints3d17_relative, _ = self.get_3d_keypoints_vis(data["keypoints3d17_relative"])

            if self.test_mode:
                self.keypoints3d_cam, _ = self.get_3d_keypoints_vis(data["keypoints3d_cam"])
        except KeyError:
        ...
        for idx in range(self.num_data):
            ...
            info["image_path"] = os.path.join(self.data_prefix, "datasets", self.dataset_name, self.image_path[idx])
            ...
            info["target_twist"] = self.target_twist[idx].astype(np.float32)  # twist_phi
            info["target_twist_weight"] = self.target_twist_weight[idx].astype(np.float32)
            ...
            if info["has_smpl"]:
                ...
                info["keypoints3d17_relative"] = self.keypoints3d17_relative[idx]

                if self.test_mode:
                    info["joint_relative_17"] = self.keypoints3d17_relative[idx].astype(np.float32)

            else:
                if self.test_mode:
                    info["joint_relative_17"] = self.keypoints3d_cam[idx].astype(np.float32)

            self.data_infos.append(info)

    def evaluate(
        self, outputs: list, res_folder: str, metric: Optional[Union[str, List[str]]] = "pa-mpjpe", **kwargs: dict
    ):
    ...
    def _parse_result(self, res, mode="keypoint"):
        ...
        if mode == "vertice":
            ...
            pred_output = self.body_model(
                betas=pred_beta, body_pose=pred_pose[:, 1:], global_orient=pred_pose[:, 0].unsqueeze(1), pose2rot=False
            )
            pred_vertices = pred_output["vertices"].detach().cpu().numpy()

            gt_pose = torch.FloatTensor([gt["pose"] for gt in gts]).view(-1, 72)
            gt_beta = torch.FloatTensor([gt["beta"] for gt in gts])
            gt_output = self.body_model(betas=gt_beta, body_pose=gt_pose[:, 3:], global_orient=gt_pose[:, :3])
            gt_vertices = gt_output["vertices"].detach().cpu().numpy()
            gt_mask = np.ones(gt_vertices.shape[:-1])
            assert len(pred_vertices) == self.num_data

            return pred_vertices * 1000.0, gt_vertices * 1000.0, gt_mask
        elif mode == "keypoint":
            ...
            if self.dataset_name == "mpi_inf_3dhp":
                ...
                gt_keypoints3d = np.array([gt["joint_relative_17"][hp3d_idxs] for gt in gts])
                joint_mapper = [14, 11, 12, 13, 8, 9, 10, 15, 1, 16, 0, 5, 6, 7, 2, 3, 4]
                gt_keypoints3d_mask = np.ones((len(gt_keypoints3d), len(joint_mapper)))
            else:
                _, h36m_idxs, _ = get_mapping("human_data", "h36m")
                gt_keypoints3d = np.array([gt["joint_relative_17"][h36m_idxs] for gt in gts])
            ...
            pred_keypoints3d = (pred_keypoints3d - pred_keypoints3d[:, None, root_idx_17]) * factor
            gt_keypoints3d = (gt_keypoints3d - gt_keypoints3d[:, None, root_idx_17]) * factor
    ...
    def _report_mpjpe(self, res_file, metric="mpjpe"):
        ...
        """
        pred_keypoints3d, gt_keypoints3d, gt_keypoints3d_mask = self._parse_result(res_file, mode="keypoint")
        ...
        error = keypoint_mpjpe(pred_keypoints3d, gt_keypoints3d, gt_keypoints3d_mask, alignment)
    ...
    def _report_3d_pck(self, res_file, metric="3dpck"):
        ...
        error = keypoint_3d_pck(pred_keypoints3d, gt_keypoints3d, gt_keypoints3d_mask, alignment)
    ...
    def _report_3d_auc(self, res_file, metric="3dauc"):
        ...
        error = keypoint_3d_auc(pred_keypoints3d, gt_keypoints3d, gt_keypoints3d_mask, alignment)
        name_value_tuples = [(err_name, error)]
