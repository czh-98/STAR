...
class PyMAFXHumanImageDataset(BaseDataset, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        if self.has_keypoints:
            ...
            for j2d in self.joints2d:
                kp2d_valid = j2d[j2d[:, 2] > 0.0]
                bbox_xyxy = [min(kp2d_valid[:, 0]), min(kp2d_valid[:, 1]), max(kp2d_valid[:, 0]), max(kp2d_valid[:, 1])]
                center = [(bbox_xyxy[2] + bbox_xyxy[0]) / 2.0, (bbox_xyxy[3] + bbox_xyxy[1]) / 2.0]
                scale = self.scale_factor * 1.2 * max(bbox_xyxy[2] - bbox_xyxy[0], bbox_xyxy[3] - bbox_xyxy[1]) / 200.0
                ...
                center = [(ul[0] + br[0]) / 2.0, (ul[1] + br[1]) / 2.0]
        ...
        self.joints2d_part = {"lhand": joints2d_lhand, "rhand": joints2d_rhand, "face": joints2d_face}
    ...
    def process_kps2d(self, kps2d, t):
        ...
        for i in range(n_kps2d):
            kp = kps2d[i, 0:2]
            new_kp = np.array([kp[0], kp[1], 1.0]).T
            new_kp = np.dot(t, new_kp)
            kps2d[i, 0:2] = new_kp[:2]
        # convert to normalized coordinates
        kps2d[:, :-1] = 2.0 * kps2d[:, :-1] / self.img_res - 1.0
    ...
    def prepare_data(self, idx: int):
        ...
        bbox_xyxy = [min(kp2d_valid[:, 0]), min(kp2d_valid[:, 1]), max(kp2d_valid[:, 0]), max(kp2d_valid[:, 1])]
        center = [(bbox_xyxy[2] + bbox_xyxy[0]) / 2.0, (bbox_xyxy[3] + bbox_xyxy[1]) / 2.0]
        sc = 1.2 * max(bbox_xyxy[2] - bbox_xyxy[0], bbox_xyxy[3] - bbox_xyxy[1]) / 200.0

        img, _ = self.process_rgb(img_orig, center, sc * scale, [self.img_res, self.img_res])
        ...
        img_hr, img_crop = self.process_rgb(img_orig, center, sc * scale, [self.img_res * 8, self.img_res * 8])

        kps_transf = get_transform(center, sc * scale, [self.img_res, self.img_res])

        lhand_kp2d, rhand_kp2d, face_kp2d = (
            self.joints2d_part["lhand"][idx],
            self.joints2d_part["rhand"][idx],
            self.joints2d_part["face"][idx],
        )

        hand_kp2d = self.process_kps2d(np.concatenate([lhand_kp2d, rhand_kp2d]).copy(), kps_transf)
        ...
        for part in ["lhand", "rhand", "face"]:
            ...
            kp2d_valid = kp2d[kp2d[:, 2] > 0.0]
            if len(kp2d_valid) > 0:
                bbox_xyxy = [min(kp2d_valid[:, 0]), min(kp2d_valid[:, 1]), max(kp2d_valid[:, 0]), max(kp2d_valid[:, 1])]
                center_part = [(bbox_xyxy[2] + bbox_xyxy[0]) / 2.0, (bbox_xyxy[3] + bbox_xyxy[1]) / 2.0]
                scale_part = 2.0 * max(bbox_xyxy[2] - bbox_xyxy[0], bbox_xyxy[3] - bbox_xyxy[1]) / 2
            ...
            crop_hf_img_size = torch.Size([1, 3, self.hf_img_size, self.hf_img_size])
            grid = F.affine_grid(theta_part.detach(), crop_hf_img_size, align_corners=False)
            img_part = F.grid_sample(
                torch.from_numpy(img_crop.transpose(2, 0, 1)[None]), grid.cpu(), align_corners=False
            ).squeeze(0)

            item[f"img_{part}"] = self.pipeline({"img": img_part.numpy().transpose(1, 2, 0)})["img"].float()
            theta_i_inv = torch.zeros_like(theta_part)
            theta_i_inv[:, 0, 0] = 1.0 / theta_part[:, 0, 0]
            theta_i_inv[:, 1, 1] = 1.0 / theta_part[:, 1, 1]
