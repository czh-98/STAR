...
class MpiInf3dhpConverter(BaseModeConverter):
    ...
    def extract_keypoints(
        self, keypoints2d: np.ndarray, keypoints3d: np.ndarray, num_keypoints: int
    ) -> Tuple[bool, np.ndarray, np.ndarray, List[float]]:
        """Check keypoints validiy and add confidence and bbox."""

        bbox_xyxy = [min(keypoints2d[:, 0]), min(keypoints2d[:, 1]), max(keypoints2d[:, 0]), max(keypoints2d[:, 1])]
    ...
    def convert_by_mode(self, dataset_path: str, out_path: str, mode: str) -> dict:
        ...
        if mode == "train":
                    ...
                    for j, vid_i in tqdm(enumerate(vid_list), desc="vid list"):
                        ...
                        if self.extract_img:
                            ...
                            while 1:
                                ...
                                frame += 1
                                # image name
                                imgname = os.path.join(imgs_path, "%06d.jpg" % frame)
                                # save image
                        ...
                        for i, img_i in enumerate(sorted(img_list)):
                            ...
                            image_path = os.path.join(
                                "S" + str(user_i), "Seq" + str(seq_i), "video_" + str(vid_i), img_name
                            )
                            ...
                            # 3D keypoints
                            keypoints3d = np.reshape(annot3[vid_i][0][i], (28, 3)) / 1000
                            keypoints3d = keypoints3d - keypoints3d[4]  # 4 is the root
                            valid, keypoints2d, keypoints3d, bbox_xywh = self.extract_keypoints(
            ...
            keypoints2d_, mask = convert_kps(keypoints2d_, "mpi_inf_3dhp", "human_data")
            keypoints3d_ = np.array(keypoints3d_).reshape((-1, 28, 4))
            keypoints3d_, _ = convert_kps(keypoints3d_, "mpi_inf_3dhp", "human_data")
            ...
            for user_i in tqdm(user_list, desc="user"):
                ...
                for frame_i, valid_i in tqdm(enumerate(valid), desc="frame"):
                    ...
                    valid, keypoints2d, keypoints3d, bbox_xywh = self.extract_keypoints(keypoints2d, keypoints3d, 17)
            ...
            keypoints2d_, mask = convert_kps(keypoints2d_, "mpi_inf_3dhp_test", "human_data")
            keypoints3d_ = np.array(keypoints3d_).reshape((-1, 17, 4))
            keypoints3d_, _ = convert_kps(keypoints3d_, "mpi_inf_3dhp_test", "human_data")
