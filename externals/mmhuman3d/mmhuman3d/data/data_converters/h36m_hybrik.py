...
class H36mHybrIKConverter(BaseModeConverter):
    ...
    def convert_by_mode(self, dataset_path: str, out_path: str, mode: str) -> dict:
        ...
        """
        if mode == "train":
            ann_file = os.path.join(dataset_path, "Sample_5_train_Human36M_smpl_leaf_twist_protocol_2.json")
        elif mode == "test":
            ann_file = os.path.join(dataset_path, "Sample_20_test_Human36M_smpl_protocol_2.json")
        ...
        for ann_image, ann_annotations in tqdm(zip(database["images"], database["annotations"]), total=num_datapoints):
            ann = dict()
            ...
            f, c = np.array(ann["cam_param"]["f"], dtype=np.float32), np.array(ann["cam_param"]["c"], dtype=np.float32)
            ...
            fname = "S{}_{}.{}".format(subject, seq_name, CAMERA_IDX_TO_NAME[camera])
            ...
            joint_img_17[:, 2] = joint_img_17[:, 2] - joint_cam_17[0, 2]
            joint_img_17 = np.hstack([joint_img_17, np.ones([17, 1])])
            joint_cam_17 = np.hstack([joint_cam_17, np.ones([17, 1])])
            joint_relative_17 = np.hstack([joint_relative_17, np.ones([17, 1])])
            ...
            if smpl_joints.size == 24 * 3:
                ...
                joint_img_29[:, 2] = joint_img_29[:, 2] - joint_cam_29[0, 2]

                joint_cam_29 = np.hstack([joint_cam_29, np.ones([29, 1])])
                joint_img_29 = np.hstack([joint_img_29, np.ones([29, 1])])
                joint_relative_29 = np.hstack([joint_relative_29, np.ones([29, 1])])
        ...
        cam_param["intrinsic"] = np.array(cam_param["intrinsic"]).reshape((-1, 3, 3))
        ...
        keypoints3d17_, keypoints3d17_mask = convert_kps(joint_img_, "h36m", "human_data")
        keypoints3d17_cam_, _ = convert_kps(joint_cam_, "h36m", "human_data")
        keypoints3d17_relative_, _ = convert_kps(joint_relative_, "h36m", "human_data")

        keypoints3d_, keypoints3d_mask = convert_kps(joint29_img_, "hybrik_29", "human_data")
        keypoints3d_cam_, _ = convert_kps(joint29_cam_, "hybrik_29", "human_data")
        keypoints3d_relative_, _ = convert_kps(joint29_relative_, "hybrik_29", "human_data")
        ...
        if mode == "train":
            ...
            out_file = os.path.join(out_path, "hybrik_h36m_valid_protocol2.npz")
