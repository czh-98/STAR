...
class EHFConverter(BaseModeConverter):
    ...
    def convert_by_mode(self, dataset_path: str, out_path: str, mode: str) -> dict:
        ...
        keypoints2d_ = []
        vertices_ = []
        camera_pose = np.array([-2.9874789618512025, 0.011724572107320893, -0.05704686818955933])
        ...
        for dataname in image_path_:
            ...
            body_kps2d = np.array(json_data["people"][0]["pose_keypoints_2d"], dtype=np.float32).reshape(-1, 3)
            face_kps2d = np.array(json_data["people"][0]["face_keypoints_2d"], dtype=np.float32).reshape(-1, 3)
            lhand_kps2d = np.array(json_data["people"][0]["hand_left_keypoints_2d"], dtype=np.float32).reshape(-1, 3)
            ...
            bbox = self._keypoints_to_scaled_bbox(keypoints2d[:, :2][conf > 0], scale=1.2)
            ...
            vertices = np.concatenate(
                (
                    np.array(aligndata["vertex"]["x"]).reshape(-1, 1),
                    np.array(aligndata["vertex"]["y"]).reshape(-1, 1),
