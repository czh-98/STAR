...
class PennActionConverter(BaseConverter):
    ...
    def convert(self, dataset_path: str, out_path: str) -> dict:
        ...
        for fname in tqdm(annot_files):
            ...
            imgs = sorted(glob.glob(dataset_path + "/frames/" + fname.strip().split("/")[-1].split(".")[0] + "/*.jpg"))
            ...
            for kp, img_path in zip(kp_2d, imgs):
                ...
                bbox_xyxy = [min(kp[:, 0]), min(kp[:, 1]), max(kp[:, 0]), max(kp[:, 1])]
        ...
        keypoints2d_, mask = convert_kps(keypoints2d_, "penn_action", "human_data")
