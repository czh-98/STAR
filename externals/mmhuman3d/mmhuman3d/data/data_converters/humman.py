...
class HuMManConverter(BaseModeConverter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.num_keypoints_humman = get_keypoint_num(self.keypoint_convention_humman)

        self.keypoint_convention_smpl = "smpl_54"
        self.num_keypoints_smpl = get_keypoint_num(self.keypoint_convention_smpl)

        self.left_hip_idx_humman = get_keypoint_idx("left_hip", convention=self.keypoint_convention_humman)
        self.right_hip_idx_humman = get_keypoint_idx("right_hip", convention=self.keypoint_convention_humman)
        self.root_idx_smpl = get_keypoint_idx("pelvis_extra", convention=self.keypoint_convention_smpl)
    ...
    def _derive_keypoints(self, global_orient, body_pose, betas, transl, focal_length, image_size, camera_center):
        ...
        camera = build_cameras(
            dict(
                ...
                convention="opencv",
                in_ndc=False,
                focal_length=torch.tensor(focal_length, dtype=torch.float).reshape(1, 2),
                image_size=torch.tensor(image_size, dtype=torch.float).reshape(1, 2),
                principal_point=torch.tensor(camera_center, dtype=torch.float).reshape(1, 2),
            )
        ).to(self.device)
    ...
    def _make_human_data(
        ...
        smpl["global_orient"] = np.concatenate(smpl["global_orient"], axis=0).reshape(-1, 3)[selected_inds]
        smpl["body_pose"] = np.concatenate(smpl["body_pose"], axis=0).reshape(-1, 23, 3)[selected_inds]
        smpl["betas"] = np.concatenate(smpl["betas"], axis=0).reshape(-1, 10)[selected_inds]
        smpl["transl"] = np.concatenate(smpl["transl"], axis=0).reshape(-1, 3)[selected_inds]

        human_data["smpl"] = smpl

        # Save derived keypoints as ground truth
        # 2D SMPL keypoints
        keypoints2d_smpl = np.concatenate(keypoints2d_smpl, axis=0).reshape(num_frames, self.num_keypoints_smpl, 2)
        keypoints2d_smpl, keypoints2d_smpl_mask = convert_kps(
            keypoints2d_smpl, src=self.keypoint_convention_smpl, dst="human_data"
        )
        keypoints2d_smpl = np.concatenate([keypoints2d_smpl, np.ones([*keypoints2d_smpl.shape[:2], 1])], axis=-1)
        human_data["keypoints2d"] = keypoints2d_smpl[selected_inds]
        human_data["keypoints2d_mask"] = keypoints2d_smpl_mask

        # 3D SMPL keypoints
        keypoints3d_smpl = np.concatenate(keypoints3d_smpl, axis=0).reshape(num_frames, self.num_keypoints_smpl, 3)
        keypoints3d_smpl, keypoints3d_smpl_mask = convert_kps(
            keypoints3d_smpl, src=self.keypoint_convention_smpl, dst="human_data"
        )
        keypoints3d_smpl = np.concatenate([keypoints3d_smpl, np.ones([*keypoints3d_smpl.shape[:2], 1])], axis=-1)
        ...
        # 2D HuMMan Keypoints
        keypoints2d_humman = np.concatenate(keypoints2d_humman, axis=0).reshape(
            num_frames, self.num_keypoints_humman, 3
        )
        keypoints2d_humman, keypoints2d_humman_mask = convert_kps(
            keypoints2d_humman, mask=self.keypoints2d_humman_mask, src=self.keypoint_convention_humman, dst="human_data"
        )
        ...
        # 3D HuMMan Keypoints
        keypoints3d_humman = np.concatenate(keypoints3d_humman, axis=0).reshape(
            num_frames, self.num_keypoints_humman, 4
        )
        keypoints3d_humman, keypoints3d_humman_mask = convert_kps(
            keypoints3d_humman, mask=self.keypoints3d_humman_mask, src=self.keypoint_convention_humman, dst="human_data"
        )
        ...
        bbox_xywh = np.concatenate([bbox_xywh, np.ones([num_frames, 1])], axis=-1)
    ...
    def convert_by_mode(self, dataset_path: str, out_path: str, mode: str) -> dict:
        ...
        for ann_path in tqdm(ann_paths):
            ...
            device_list = [("Kinect", i) for i in range(num_kinect)] + [("iPhone", i) for i in range(num_iphone)]
            assert len(device_list) == num_kinect + num_iphone

            for device, device_id in device_list:
                assert device in {"Kinect", "iPhone"}, f"Undefined device: {device}, " f'should be "Kinect" or "iPhone"'
                ...
                if device == "Kinect":
                    ...
                    smpl_ = kinect_smpl
                    width, height = smc_reader.get_kinect_color_resolution(device_id)
                ...
                keypoints2d_humman, keypoints2d_humman_mask = smc_reader.get_keypoints2d(device, device_id)

                if self.keypoints2d_humman_mask is None:
                    self.keypoints2d_humman_mask = keypoints2d_humman_mask
                assert np.allclose(self.keypoints2d_humman_mask, keypoints2d_humman_mask)

                keypoints2d_humman_.append(keypoints2d_humman)

                # get keypoints3d (all frames)
                keypoints3d_humman, keypoints3d_humman_mask = smc_reader.get_keypoints3d(device, device_id)

                if self.keypoints3d_humman_mask is None:
                    self.keypoints3d_humman_mask = keypoints3d_humman_mask
                assert np.allclose(self.keypoints3d_humman_mask, keypoints3d_humman_mask)

                # root-align keypoints3d
                left_hip_keypoints = keypoints3d_humman[:, [self.left_hip_idx_humman], :3]
                right_hip_keypoints = keypoints3d_humman[:, [self.right_hip_idx_humman], :3]
                root_keypoints = (left_hip_keypoints + right_hip_keypoints) / 2.0
                keypoints3d_humman[..., :3] = keypoints3d_humman[..., :3] - root_keypoints
                ...
                # expand betas
                betas_expanded = np.tile(smpl_dict["betas"], num_frames).reshape(-1, 10)
                ...
                smpl_keypoints = self._derive_keypoints(
                    **smpl_dict, focal_length=focal_length, image_size=image_size, camera_center=camera_center
                )
