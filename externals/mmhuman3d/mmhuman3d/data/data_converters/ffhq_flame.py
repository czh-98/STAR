...
class FFHQFlameConverter(BaseModeConverter):
    ...
    def convert_by_mode(self, dataset_path: str, out_path: str, mode: str) -> dict:
        ...
        keypoints2d_flame = np.concatenate([keypoints2d_flame, np.ones([keypoints2d_flame.shape[0], 73, 1])], axis=-1)
        keypoints2d, keypoints2d_mask = convert_kps(keypoints2d_flame, src="flame", dst="human_data")
        human_data["keypoints2d"] = keypoints2d
        human_data["keypoints2d_mask"] = keypoints2d_mask

        img_fnames = data["img_fnames"]
        image_path_ = np.array([os.path.join("ffhq_global_images_1024", img_fname) for img_fname in img_fnames])
