...
class VibeConverter(BaseModeConverter):
    ...
    def __init__(self, modes: List = [], pretrained_ckpt: dict = None) -> None:
        ...
        self.mapping_dict = {"mpi_inf_3dhp": "mpi_inf_3dhp_train.npz", "pw3d": "3dpw_test.npz"}
        self.img_norm_cfg = dict(mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375], to_rgb=True)
    ...
    def convert_by_mode(self, dataset_path: str, out_path: str, mode: str) -> dict:
        ...
        if mode == "mpi_inf_3dhp":
            image_path_ = np.array([s.replace("imageFrames/", "").replace("frame_", "") for s in data["imgname"]])
        ...
        if "openpose" in data:
            ...
            keypoints2d_, keypoints2d_mask = convert_kps(keypoints2d_, "smpl_49", "human_data")
        ...
        if "S" in data:
            ...
            keypoints3d_, keypoints3d_mask = convert_kps(keypoints3d_, "smpl_24", "human_data")
            human_data["keypoints3d_mask"] = keypoints3d_mask
            human_data["keypoints3d"] = keypoints3d_

        if "pose" in data:
            smpl = {}
            smpl["body_pose"] = np.array(data["pose"][:, 3:]).reshape((-1, 23, 3))
            smpl["global_orient"] = np.array(data["pose"][:, :3]).reshape((-1, 3))
        ...
        model = self.init_model(model_config, self.pretrained_ckpt, self.device)
        model.eval()
        img_root_path = os.path.join(dataset_path, "..", mode)
        video = []
        for center, scale, img_name in tqdm(zip(data["center"], data["scale"], image_path_), total=num_data):
            results = {}
