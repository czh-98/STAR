...
class CrowdposeConverter(BaseModeConverter):
    ...
    def convert_by_mode(
        ...
        json_path = os.path.join(dataset_path, "crowdpose_{}.json".format(mode))
        ...
        for annot in tqdm(json_data["annotations"]):
            ...
            x_in = np.logical_and(keypoints2d[:, 0] < width, keypoints2d[:, 0] >= 0)
            y_in = np.logical_and(keypoints2d[:, 1] < height, keypoints2d[:, 1] >= 0)
        ...
        if enable_multi_human_data:
            ...
            for image_path in sorted(set(image_path_), key=image_path_.index):
                frame_end = frame_start + image_path_.count(image_path)
        ...
        keypoints2d_, mask = convert_kps(keypoints2d_, "crowdpose", "human_data")
