...
class ExposeSPINSMPLXConverter(BaseModeConverter):
    ...
    def convert_by_mode(self, dataset_path: str, out_path: str, mode: str) -> dict:
        ...
        for dset_name in dset_names:
            data = np.load(os.path.join(dataset_path, "{}.npz".format(dset_name)))
        ...
        pose = np.concatenate([spin_data[dset]["pose"] for dset in spin_data], axis=0).astype(np.float32)
        keypoints2d = np.concatenate([spin_data[dset]["part"] for dset in spin_data], axis=0).astype(np.float32)
        imgnames = np.concatenate([spin_data[dset]["imgname"] for dset in spin_data], axis=0)
        ...
        for index, kps in enumerate(keypoints2d):
            ...
            right_hand_conf = (right_hand_conf >= HAND_THRESH).astype(np.float32)
        ...
        for index in range(len(imgnames)):
            if dsets[index] == "coco":
                image_path_.append(os.path.join("coco/train2014", imgnames[index]))
            elif dsets[index] == "lspet":
                image_path_.append(os.path.join("lsp/lspet/images", imgnames[index]))
            elif dsets[index] == "lsp":
                image_path_.append(os.path.join("lsp/lsp_dataset_original/images", imgnames[index]))
            elif dsets[index] == "mpii":
                image_path_.append(os.path.join("mpii/images", imgnames[index]))
