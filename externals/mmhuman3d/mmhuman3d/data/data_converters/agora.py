...
class AgoraConverter(BaseModeConverter):
    ...
    def __init__(
        ...
        if res not in accepted_res:
            raise ValueError(
                "Input resolution not in accepted resolution. \
                Use either (1280, 720) or (3840, 2160)"
            )
        self.res = res

    def get_global_orient(self, imgPath, df, i, pNum, globalOrient=None, meanPose=False):
        ...
        camera_rotationMatrix, _ = cv2.Rodrigues(np.array([0, ((-camYaw) / 180) * np.pi, 0]).reshape(3, 1))
        camera_rotationMatrix2, _ = cv2.Rodrigues(np.array([camPitch / 180 * np.pi, 0, 0]).reshape(3, 1))
        ...
        globalOrient = self.rotate_global_orient(camera_rotationMatrix, globalOrient)
        globalOrient = self.rotate_global_orient(camera_rotationMatrix2, globalOrient)
    ...
    def rotate_global_orient(rotMat, global_orient):
        ...
        """
        new_global_orient = cv2.Rodrigues(np.dot(rotMat, cv2.Rodrigues(global_orient.reshape(-1))[0]))[0].T[0]
    ...
    def convert_by_mode(self, dataset_path: str, out_path: str, mode: str) -> dict:
        ...
        annot_dataframes = [
            os.path.join(annot_path, f) for f in os.listdir(annot_path) if f.endswith(".pkl") and "{}".format(mode) in f
        ]
        ...
        for filename in tqdm(sorted(annot_dataframes)):
            ...
            for idx in tqdm(range(len(df))):
                ...
                for pidx in valid_pers_idx:
                    ...
                    gt_bodymodel_path = os.path.join(dataset_path, df.iloc[idx][f"gt_path_{self.fit}"][pidx])
                    gt_bodymodel_path = gt_bodymodel_path.replace(".obj", ".pkl")
                    ...
                    if self.fit == "smplx":
                        ...
                        body_model["betas"].append(ann["betas"].reshape(-1)[:10])
                        body_model["transl"].append(ann["transl"])
                        body_model["left_hand_pose"].append(ann["left_hand_pose"])
                        body_model["right_hand_pose"].append(ann["right_hand_pose"])
                        ...
                        # obtain smpl data
                        body_model["body_pose"].append(ann["body_pose"].cpu().detach().numpy())
                        global_orient = ann["root_pose"].cpu().detach().numpy()
                        body_model["betas"].append(ann["betas"].cpu().detach().numpy().reshape(-1)[:10])
                        body_model["transl"].append(ann["translation"].cpu().detach().numpy())

                    global_orient = self.get_global_orient(img_path, df, idx, pidx, global_orient.reshape(-1))
        ...
        if self.fit == "smplx":
            body_model["left_hand_pose"] = np.array(body_model["left_hand_pose"]).reshape((-1, 15, 3))
            body_model["right_hand_pose"] = np.array(body_model["right_hand_pose"]).reshape((-1, 15, 3))
            body_model["expression"] = np.array(body_model["expression"]).reshape((-1, 10))
            body_model["leye_pose"] = np.array(body_model["leye_pose"]).reshape((-1, 3))
            body_model["reye_pose"] = np.array(body_model["reye_pose"]).reshape((-1, 3))
            body_model["jaw_pose"] = np.array(body_model["jaw_pose"]).reshape((-1, 3))

        body_model["body_pose"] = np.array(body_model["body_pose"]).reshape((-1, num_body_pose, 3))
        body_model["global_orient"] = np.array(body_model["global_orient"]).reshape((-1, 3))
        ...
        keypoints2d_, mask = convert_kps(keypoints2d_, keypoints_convention, "human_data")
        keypoints3d_ = np.array(keypoints3d_).reshape((-1, num_keypoints, 4))
        keypoints3d_, _ = convert_kps(keypoints3d_, keypoints_convention, "human_data")
