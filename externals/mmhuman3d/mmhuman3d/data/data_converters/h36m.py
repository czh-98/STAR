...
class H36mCamera:
    ...
    @staticmethod
    def get_intrinsic_matrix(f: List[float], c: List[float], inv: bool = False) -> np.ndarray:
        ...
        if inv:
            intrinsic_matrix = np.linalg.inv(intrinsic_matrix).astype(np.float32)
...
class H36mConverter(BaseModeConverter):
    ...
    def __init__(self, modes: List = [], protocol: int = 1, extract_img: bool = False, mosh_dir=None) -> None:
        super(H36mConverter, self).__init__(modes)
        accepted_protocol = [1, 2]
        if protocol not in accepted_protocol:
            raise ValueError(
                "Input protocol not in accepted protocol. \
                Use either 1 or 2"
            )
    ...
    def convert_by_mode(
        self, dataset_path: str, out_path: str, mode: str, enable_multi_human_data: bool = False
    ) -> dict:
        """
        Args:
        ...
        for user_i in tqdm(user_list, desc="user id"):
            ...
            for seq_i in tqdm(seq_list, desc="sequence id"):
                ...
                # 3D mosh file
                if self.get_mosh:
                    mosh_name = "%s_cam%s_aligned.pkl" % (action_raw, self.camera_name_to_idx[camera])
                ...
                for frame_i in tqdm(range(poses_3d.shape[0]), desc="frame id"):
                    ...
                    if frame_i % 5 == 0 and (self.protocol == 1 or camera == "60457274"):
                        # image name
                        seq_id = f"{user_name}_{action}"
                        image_name = f"{seq_id}.{camera}_{frame_i + 1:06d}.jpg"
                        img_folder_name = f"{user_name}_{action}.{camera}"
                        image_path = os.path.join(user_name, "images", img_folder_name, image_name)
                        ...
                        bbox_xyxy = np.array([np.min(xs), np.min(ys), np.max(xs) + 1, np.max(ys) + 1])
                        ...
                        keypoints2d17 = np.concatenate([keypoints2d17, np.ones((17, 1))], axis=1)

                        # read GT 3D pose
                        keypoints3dall = np.reshape(poses_3d[frame_i, :], [-1, 3]) / 1000.0
                        keypoints3d17 = keypoints3dall[h36m_idx]
                        keypoints3d17 -= keypoints3d17[0]  # root-centered
                        keypoints3d17 = np.concatenate([keypoints3d17, np.ones((17, 1))], axis=1)
        ...
        if enable_multi_human_data:
            frame_range = np.array([[i, i + 1] for i in range(len(image_path_))])
        ...
        if mode == "train":
            ...
            out_file = os.path.join(out_path, f"h36m_valid_protocol{self.protocol}.npz")
