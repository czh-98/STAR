...
class GTAHumanConverter(BaseConverter):
    ...
    def convert(self, dataset_path: str, out_path: str) -> dict:
        ...
        # structs we use
        image_path_, bbox_xywh_, keypoints_2d_gta_, keypoints_3d_gta_, keypoints_2d_, keypoints_3d_ = (
            [],
            [],
            [],
            [],
        ...
        ann_paths = sorted(glob.glob(os.path.join(dataset_path, "annotations", "*.pkl")))
        ...
        for ann_path in tqdm(ann_paths):
            ...
            keypoints_2d_gta, keypoints_2d_gta_mask = convert_kps(ann["keypoints_2d"], src="gta", dst="smpl_49")
            keypoints_3d_gta, keypoints_3d_gta_mask = convert_kps(ann["keypoints_3d"], src="gta", dst="smpl_49")
            ...
            keypoints_3d_gta = keypoints_3d_gta - keypoints_3d_gta[:, [root_idx], :]
            keypoints_3d = keypoints_3d - keypoints_3d[:, [root_idx], :]

            for frame_idx in range(num_frames):

                image_path = os.path.join("images", seq_idx, "{:08d}.jpeg".format(frame_idx))
                ...
                if not (0 <= x < 1920 and 0 <= y < 1080 and 0 < w < 1920 and 0 < h < 1080):
                    continue
        ...
        keypoints2d = np.concatenate([keypoints2d, np.ones([keypoints2d.shape[0], 49, 1])], axis=-1)
        keypoints2d, keypoints2d_mask = convert_kps(keypoints2d, src="smpl_49", dst="human_data")
        human_data["keypoints2d"] = keypoints2d
        human_data["keypoints2d_mask"] = keypoints2d_mask

        keypoints3d = np.array(keypoints_3d_).reshape(-1, 49, 3)
        keypoints3d = np.concatenate([keypoints3d, np.ones([keypoints3d.shape[0], 49, 1])], axis=-1)
        keypoints3d, keypoints3d_mask = convert_kps(keypoints3d, src="smpl_49", dst="human_data")
        human_data["keypoints3d"] = keypoints3d
        human_data["keypoints3d_mask"] = keypoints3d_mask

        keypoints2d_gta = np.array(keypoints_2d_gta_).reshape(-1, 49, 3)
        keypoints2d_gta, keypoints2d_gta_mask = convert_kps(keypoints2d_gta, src="smpl_49", dst="human_data")
        ...
        keypoints3d_gta, keypoints3d_gta_mask = convert_kps(keypoints3d_gta, src="smpl_49", dst="human_data")
