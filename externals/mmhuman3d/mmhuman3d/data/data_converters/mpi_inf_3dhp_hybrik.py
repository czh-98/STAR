...
class MpiInf3dhpHybrIKConverter(BaseModeConverter):
    ...
    @staticmethod
    def cam2pixel_matrix(cam_coord: np.ndarray, intrinsic_param: np.ndarray) -> np.ndarray:
    ...
    def convert_by_mode(self, dataset_path: str, out_path: str, mode: str) -> dict:
        ...
        """
        if mode == "train":
            ann_file = os.path.join(dataset_path, "annotation_mpi_inf_3dhp_train_v2.json")
        elif mode == "test":
            ann_file = os.path.join(dataset_path, "annotation_mpi_inf_3dhp_test.json")
        ...
        # structs we use
        image_path_, bbox_xywh_, root_cam_, image_width_, image_height_, joint_cam_, joint_img_, depth_factor_ = (
            [],
            [],
            [],
            [],
        ...
        for ann_image, ann_annotations in tqdm(zip(database["images"], database["annotations"]), total=num_datapoints):
            ...
            if mode == "train":
                root_idx = 4
                _, sub, seq, vid, im = ann["file_name"].split("/")[-1].split("_")
                fname = "{}/{}/{}/{}".format(sub, seq, vid.replace("V", "video_"), im)
        ...
        if mode == "train":
            ...
            keypoints3d_, keypoints3d_mask = convert_kps(keypoints3d_, "hybrik_hp3d", "human_data")
            keypoints3d_cam_, keypoints3d_cam_mask = convert_kps(keypoints3d_cam_, "hybrik_hp3d", "human_data")
        elif mode == "test":
            keypoints3d_ = np.array(joint_img_).reshape((-1, 17, 4))
            keypoints3d_cam_ = np.array(joint_cam_).reshape((-1, 17, 4))
            keypoints3d_, keypoints3d_mask = convert_kps(keypoints3d_, "mpi_inf_3dhp_test", "human_data")
            keypoints3d_cam_, _ = convert_kps(keypoints3d_cam_, "mpi_inf_3dhp_test", "human_data")
