...
class AmassConverter(BaseConverter):
    ...
    def convert(self, dataset_path: str, out_path: str) -> dict:
        ...
        for seq_name in tqdm(all_sequences):
            ...
            for subject in tqdm(subjects):
                ...
                for action_file in action_list:
                    ...
                    vid_id = os.path.join(seq_name, subject, action_name + ".mp4")
        ...
        smplh["global_orient"] = np.array(smplh["global_orient"]).reshape((-1, 3))
        smplh["betas"] = np.array(smplh["betas"]).reshape((-1, 10))
        smplh["transl"] = np.array(smplh["transl"]).reshape((-1, 3))
        smplh["left_hand_pose"] = np.array(smplh["left_hand_pose"]).reshape((-1, 15, 3))
        smplh["right_hand_pose"] = np.array(smplh["right_hand_pose"]).reshape((-1, 15, 3))
