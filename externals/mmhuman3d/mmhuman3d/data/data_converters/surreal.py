...
class SurrealConverter(BaseModeConverter):
    ...
    def __init__(self, modes: List = [], run: int = 0, extract_img: bool = False) -> None:
        super(SurrealConverter, self).__init__(modes)
        accepted_runs = [0, 1, 2]
        if run not in accepted_runs:
            raise ValueError(
                "Input run not in accepted runs. \
    ...
    def convert_by_mode(self, dataset_path: str, out_path: str, mode: str) -> dict:
        ...
        # structs we use
        image_path_, bbox_xywh_, video_path_, keypoints2d_, keypoints3d_, cam_param_ = [], [], [], [], [], []
        ...
        data_path = os.path.join(dataset_path, "{}/run{}".format(mode, self.run))
        ...
        for seq_name in tqdm(os.listdir(data_path)):
            ...
            for ann_file in seq_files:
                ...
                camera = CameraParameter(H=self.image_height, W=self.image_width)
                ...
                if self.extract_img:
                    ...
                    while 1:
                        ...
                        if not success:
                            break
                        # image name
                        imgname = os.path.join(img_dir, "frame_%06d.jpg" % frame)
                        frame += 1
                        # save image
                ...
                for idx in range(num_frames):
                    ...
                    image_path = os.path.join(rel_img_dir, "frame_%06d.jpg" % idx)
