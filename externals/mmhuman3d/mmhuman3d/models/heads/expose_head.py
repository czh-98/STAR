...
class IterativeRegression(nn.Module):
    ...
    def __init__(
        self, module, mean_param, num_stages=1, append_params=True, learn_mean=False, detach_mean=False, dim=1, **kwargs
    ):
    ...
    def forward(self, features: torch.Tensor, cond: Optional[torch.Tensor] = None):
        ...
        if self.append_params:
            assert features is not None, "Features are none even though append_params is True"
    ...
    def __init__(
        ...
        for layer_idx, layer_dim in enumerate(layers):
            if activ_type == "none":
                ...
                active = build_activation_layer(cfg=dict(type=activ_type, inplace=True))
        ...
        initialize(self.output_layer, init_cfg=dict(type="Xavier", gain=gain, distribution="uniform"))
        ...
        for pose_param in self.pose_param_conf:
            ...
            if pose_param["use_mean"]:
                pose_decoder = ContinuousRotReprDecoder(
                    num_angles, dtype=torch.float32, mean=mean_poses_dict.get(pose_name, None)
                )
            else:
...
class ExPoseBodyHead(ExPoseHead):
    ...
    def forward(self, features):
        ...
        return {"pred_param": params_dict, "pred_cam": camera_params, "pred_raw": raw_dict}
