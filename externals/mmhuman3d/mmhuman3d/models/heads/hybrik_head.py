    ...
    def forward(
        ...
        hm_x = (
            ...
            * torch.cuda.comm.broadcast(
                torch.arange(hm_x.shape[-1]).type(torch.cuda.FloatTensor), devices=[hm_x.device.index]
            )[0]
        )
        hm_y = (
            hm_y
            * torch.cuda.comm.broadcast(
                torch.arange(hm_y.shape[-1]).type(torch.cuda.FloatTensor), devices=[hm_y.device.index]
            )[0]
        )
        hm_z = (
            hm_z
            * torch.cuda.comm.broadcast(
                torch.arange(hm_z.shape[-1]).type(torch.cuda.FloatTensor), devices=[hm_z.device.index]
            )[0]
        )
        ...
        pred_poses = hybrik_output["poses"].float().reshape(batch_size, 24, 3, 3)
