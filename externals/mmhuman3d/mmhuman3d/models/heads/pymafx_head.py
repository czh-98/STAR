...
class Mesh_Sampler(nn.Module):
    ...
    def __init__(self, type="smpl", level=2, device=torch.device("cuda"), option=None):
        ...
        if level == 2:
            Dmap = torch.matmul(point_downsample[1].to_dense(), point_downsample[0].to_dense())  # 6890 -> 431
        elif level == 1:
...
class MAF_Extractor(nn.Module):
    ...
    def __init__(self, filter_channels, device=torch.device("cuda"), iwp_cam_mode=True, option=None):
        ...
        for ll in range(0, len(filter_channels) - 1):
            if 0 != ll:
                self.filters.append(nn.Conv1d(filter_channels[ll] + filter_channels[0], filter_channels[ll + 1], 1))
            else:
                self.filters.append(nn.Conv1d(filter_channels[ll], filter_channels[ll + 1], 1))
    ...
    def sampling(
        self, points: torch.Tensor, im_feat: torch.Tensor = None, add_att: bool = False, reduce_dim: bool = True
    ):
    ...
    def forward(
        self, p: torch.Tensor, im_feat: torch.Tensor, cam: torch.Tensor = None, add_att=False, reduce_dim=True, **kwargs
    ):
...
class Regressor(nn.Module):
    ...
    def __init__(
        ...
        if not self.smpl_mode:
            lhand_mean_rotmat = aa_to_rotmat(self.smpl.model_neutral.left_hand_mean.view(-1, 3))
            bs = lhand_mean_rotmat.shape[0]
            lhand_mean_rot6d = lhand_mean_rotmat[:, :, :2].reshape(bs, 6)

            rhand_mean_rotmat = aa_to_rotmat(self.smpl.model_neutral.right_hand_mean.view(-1, 3))
            ...
            init_face = init_face_rotmat[:, :, :2].reshape(3, 6).reshape(-1).unsqueeze(0)
    ...
    def forward(self, x: torch.Tensor = None, n_iter: int = 1, rw_cam={}, init_mode=False, global_iter=-1, **kwargs):
        """Forward function.
        ...
        if self.smplx_mode or "hand" in self.bhf_names:
            ...
            if self.mesh_model["name"] == "mano" or self.opt_wrist:
                ...
                if self.mesh_model["name"] == "mano":
                    pred_cam = torch.cat([pred_cam_rh[:, 0:1] * 10.0, pred_cam_rh[:, 1:]], dim=1)
        ...
        if self.smplx_mode or "face" in self.bhf_names:
            ...
            if self.mesh_model["name"] == "flame" or self.opt_wrist:
                ...
                if self.mesh_model["name"] == "flame":
                    pred_cam = torch.cat([pred_cam_fa[:, 0:1] * 10.0, pred_cam_fa[:, 1:]], dim=1)
                ...
                if not self.smpl_mode:
                    ...
                    if "rhand" in self.part_names:
                        ...
                        if self.mesh_model["name"] == "mano" or self.opt_wrist:
                            ...
                            if self.mesh_model["name"] == "mano":
                                pred_cam = torch.cat([pred_cam_rh[:, 0:1] * 10.0, pred_cam_rh[:, 1:] / 10.0], dim=1)
                    ...
                    if "face" in self.part_names:
                        ...
                        if self.mesh_model["name"] == "flame":
                            ...
                            if self.mesh_model["name"] == "flame":
                                pred_cam = torch.cat([pred_cam_fa[:, 0:1] * 10.0, pred_cam_fa[:, 1:] / 10.0], dim=1)
                    ...
                    if self.full_body_mode or self.body_hand_mode:
                        if self.pred_vis_h:
                            ...
                            pred_vis_lhand = pred_vis_hands[:, 0] > self.hand_vis_th
                            pred_vis_rhand = pred_vis_hands[:, 1] > self.hand_vis_th
                        ...
                        if self.opt_wrist:
                            ...
                            if self.adapt_integr:
                                ...
                                lelbow_twist_angle[
                                    lelbow_twist_angle == torch.clamp(lelbow_twist_angle, min_angle, max_angle)
                                ] = 0
                                relbow_twist_angle[
                                    relbow_twist_angle == torch.clamp(relbow_twist_angle, min_angle, max_angle)
                                ] = 0
        ...
        if self.smplx_mode:
            if self.pred_vis_h and global_iter == (self.n_iter - 1):
                ...
                pred_hf6d = torch.cat([pred_lhand_filtered, pred_rhand_filtered, pred_face], dim=1).reshape(
                    batch_size, -1, 6
                )
            else:
        ...
        if self.smplx_mode:
            pred_joints_full = torch.cat(
                [
                    ...
                    pred_output["lfoot_joints"],
                    pred_output["rfoot_joints"],
                ],
                dim=1,
            )
        else:
            pred_joints_full = pred_joints
        pred_keypoints_2d = projection(pred_joints_full, {**rw_cam, "cam_sxy": pred_cam}, iwp_mode=self.use_iwp_cam)
        if self.use_iwp_cam:
            # Normalize keypoints to [-1,1]
            pred_keypoints_2d = pred_keypoints_2d / (224.0 / 2.0)
        else:
            pred_keypoints_2d = transform_kps2d(pred_keypoints_2d, rw_cam["kps_transf"])
                ...
                output.update(
                    {
                        ...
                        "pred_feet_kp2d": pred_keypoints_2d[
                            :, len_b_kp + len_h_kp * 2 + len_f_kp : len_b_kp + len_h_kp * 2 + len_f_kp + len_feet_kp
                        ],
                    }
                )
                if self.opt_wrist:
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: torch.Tensor, head_mask=None, history_state=None):
        """Forward function."""
...
class PyMAFXHead(BaseModule):
    ...
    def init_mesh(self, regressor, batch_size, rw_cam={}):
        ...
        if self.init_mesh_output is None or self.batch_size != batch_size:
            self.init_mesh_output = regressor[0](torch.zeros(batch_size), rw_cam=rw_cam, init_mode=True)
    ...
    def forward(
        ...
        fuse_grid_align = self.grid_align["use_att"] or self.grid_align["use_fc"]
        if fuse_grid_align:
        ...
        for rf_i in range(self.n_iter):
            ...
            if self.bhf_mode == "body_hand":
                ...
                pred_hand_proj = projection(pred_hand_v, {**rw_cam, "cam_sxy": pred_cam}, iwp_mode=self.use_iwp_cam)
                if self.use_iwp_cam:
                    pred_hand_proj = pred_hand_proj / (224.0 / 2.0)
                else:
                    pred_hand_proj = transform_kps2d(pred_hand_proj, rw_cam["kps_transf"])
            ...
            if "hand" in self.bhf_names or "face" in self.bhf_names:
                ...
                for hf_i, part_name in enumerate(part_names):
                    ...
                    if self.maf_on:
                        ...
                        limb_reduce_dim = (not fuse_grid_align) or (rf_i < att_starts)
                        ...
                        if limb_rf_i == 0 or self.grid_feat:
                            ...
                            if self.bhf_mode == "full_body" or self.bhf_mode == "body_hand":
                                ...
                                proj_hf_pts_crop = torch.bmm(
                                    theta_i_inv, homo_vector(proj_hf_pts[part_name][:, :, :2]).permute(0, 2, 1)
                                ).permute(0, 2, 1)
                                ...
                                if self.hf_box_center:
                                    ...
                                    part_box_center = (part_box_ul + part_box_br) / 2.0
                        ...
                        if fuse_grid_align and limb_rf_i >= att_starts:
                            ...
                            if self.grid_align["use_att"]:
                                att_ref_feat_ctd = align_attention[hf_key][limb_rf_i - att_starts](
                                    limb_grid_ref_feat_ctd
                                )[0]
                            elif self.grid_align["use_fc"]:
                                att_ref_feat_ctd = limb_grid_ref_feat_ctd

                            att_ref_feat_ctd = (
                                self.maf_extractor[hf_key][limb_rf_i]
                                .reduce_dim(att_ref_feat_ctd.permute(0, 2, 1))
                                .view(batch_size, -1)
                            )
                            limb_ref_feat_ctd = att_feat_reduce[hf_key][limb_rf_i - att_starts](att_ref_feat_ctd)
            ...
            if "body" in self.bhf_names:
                if self.maf_on:
                    ...
                    if fuse_grid_align and rf_i >= att_starts:
                        ...
                        if self.grid_align["use_att"]:
                            att_ref_feat = align_attention["body"][rf_i - att_starts](grid_ref_feat)[0]
                        elif self.grid_align["use_fc"]:
                        ...
                        ref_feature = att_feat_reduce["body"][rf_i - att_starts](att_ref_feat)
                    else:
            ...
            mesh_output = regressor[rf_i](ref_feature, n_iter=1, rw_cam=rw_cam, global_iter=rf_i, **current_states)
