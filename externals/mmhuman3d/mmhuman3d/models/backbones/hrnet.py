    ...
    def forward(self, x):
        ...
        if self.extra["return_list"]:
            ...
            if self.extra["use_conv"]:
                ...
                x1 = F.interpolate(y_list[0], size=(x0_h, x0_w), mode="bilinear", align_corners=True)
                x2 = F.interpolate(y_list[1], size=(x0_h, x0_w), mode="bilinear", align_corners=True)
                x3 = F.interpolate(y_list[2], size=(x0_h, x0_w), mode="bilinear", align_corners=True)
                x = torch.cat([x1, x2, x3, y_list[3]], 1)
        else:
            if self.extra["use_conv"]:
...
class PoseHighResolutionNetExpose(PoseHighResolutionNet):
    ...
    def __init__(
        ...
        in_dims = (
            2**2 * self.stage2_cfg["num_channels"][-1]
        ...
        self.subsample_3 = self._make_subsample_layer(in_channels=self.stage2_cfg["num_channels"][-1], num_layers=2)
        self.subsample_2 = self._make_subsample_layer(in_channels=self.stage3_cfg["num_channels"][-1], num_layers=1)
...
class PoseHighResolutionNetPyMAFX(PoseHighResolutionNet):
    ...
    def __init__(
        ...
        # stem net
        self.conv1 = build_conv_layer(self.conv_cfg, 3, 64, kernel_size=3, stride=2, padding=1, bias=False)
        ...
        num_channels = [num_channels[i] * block.expansion for i in range(len(num_channels))]
        self.transition3 = self._make_transition_layer(pre_stage_channels, num_channels)
        self.stage4, pre_stage_channels = self._make_stage(self.stage4_cfg, num_channels, multiscale_output=True)
    ...
    def _make_head(self, pre_stage_channels):
        ...
        final_layer = nn.Sequential(
            ...
            build_norm_layer(self.norm_cfg, 2048)[1],
            nn.ReLU(inplace=True),
        )
