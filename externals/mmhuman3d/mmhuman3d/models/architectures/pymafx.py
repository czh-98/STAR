...
class PyMAFX(BaseArchitecture, metaclass=ABCMeta):
    ...
    def _create_encoder(self):
        ...
        if "body" in self.bhf_names and self.backbone is not None:
            ...
            if not self.grid_feat:
                self.ma_feat_dim = self.mesh_sampler.Dmap.shape[0] * self.mlp_dim[-1]
            else:
                self.ma_feat_dim = 0
        ...
        if "hand" in self.bhf_names or "face" in self.bhf_names and self.hf_model_cfg is not None:
            for hf in ["hand", "face"]:
                if hf in self.bhf_names:
                    self.encoders[hf] = build_backbone(self.hf_model_cfg["backbone"])
            if self.hf_aux_supv_on:
                assert self.maf_on
                self.dp_head_hf = nn.ModuleDict()
                if "hand" in self.bhf_names:
                    self.dp_head_hf["hand"] = IUV_predict_layer(feat_dim=self.hf_sfeat_dim[-1], mode="pncc")
                if "face" in self.bhf_names:
                    self.dp_head_hf["face"] = IUV_predict_layer(feat_dim=self.hf_sfeat_dim[-1], mode="pncc")
    ...
    def _create_attention_modules(self, attention_config):
        ...
        if "hand" in self.bhf_names:
            ...
            self.bhf_ma_feat_dim.update({"hand": self.mano_ds_len * self.hf_mlp_dim[-1]})

            if self.fuse_grid_align:
                self.bhf_att_feat_dim.update({"hand": 1024})

        if "face" in self.bhf_names:
            self.bhf_ma_feat_dim.update({"face": len(FACIAL_LANDMARKS) * self.hf_mlp_dim[-1]})
            if self.fuse_grid_align:
        ...
        if self.grid_align["use_att"]:
            ...
            if "hand" in self.bhf_names or "face" in self.bhf_names:
                if "hand" in self.bhf_names:
                    hfimg_feat_dim_list["hand"] = self.hf_sfeat_dim[-n_iter_att:]
                if "face" in self.bhf_names:
                    hfimg_feat_dim_list["face"] = self.hf_sfeat_dim[-n_iter_att:]
    ...
    def _create_regressor(self, regressor: Optional[Union[dict, None]]):
        ...
        for i in range(self.n_iter):
            ...
            if self.maf_on:
                if "hand" in self.bhf_names or "face" in self.bhf_names:
                    if i == 0:
                        feat_dim_hand = self.grid_feat_dim if "hand" in self.bhf_names else None
                        feat_dim_face = self.grid_feat_dim if "face" in self.bhf_names else None
                    else:
                        if self.fuse_grid_align:
                            feat_dim_hand = self.bhf_att_feat_dim["hand"] if "hand" in self.bhf_names else None
                            feat_dim_face = self.bhf_att_feat_dim["face"] if "face" in self.bhf_names else None
                        else:
                            feat_dim_hand = self.bhf_ma_feat_dim["hand"] if "hand" in self.bhf_names else None
                            feat_dim_face = self.bhf_ma_feat_dim["face"] if "face" in self.bhf_names else None
                else:
    ...
    def _create_maf_extractor(self):
        ...
        for part in self.bhf_names:
            ...
            filter_channels_default = self.mlp_dim if part == "body" else self.hf_mlp_dim
            sfeat_dim = self.body_sfeat_dim if part == "body" else self.hf_sfeat_dim
            for i in range(self.n_iter):
                ...
                if self.grid_align["use_att"] and i >= self.grid_align["att_starts"]:
                    self.maf_extractor[part].append(
                        MAF_Extractor(
                            filter_channels=filter_channels_default[self.att_feat_dim_idx :],
                            iwp_cam_mode=self.use_iwp_cam,
                        )
                    )
                else:
                    self.maf_extractor[part].append(
                        MAF_Extractor(filter_channels=filter_channels, iwp_cam_mode=self.use_iwp_cam)
                    )
    ...
    def forward_test(self, batch={}, rw_cam={}, **kwargs):
        ...
        # grid-pattern points
        grid_points = torch.transpose(grid_points.expand(batch_size, -1, -1), 1, 2).to(self.device)
        ...
        out_dict = self.head(
            ...
            g_feat,
            grid_points,
            self.att_feat_reduce,
            self.align_attention,
            self.maf_extractor,
            self.regressor,
            batch_size,
            limb_gfeat_dict,
