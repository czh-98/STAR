...
class BodyModelEstimator(BaseArchitecture, metaclass=ABCMeta):
    ...
    def run_registration(
        ...
        dataset_name = [meta["dataset_name"] for meta in img_metas]  # name of the dataset the image comes from

        indices = targets["sample_idx"].squeeze()
        is_flipped = targets["is_flipped"].squeeze().bool()  # flag that indicates whether image was flipped
        # during data augmentation
        rot_angle = targets["rotation"].squeeze()  # rotation angle used for data augmentation Q
        ...
        pred_cam_t = torch.stack(
            [pred_cam[:, 1], pred_cam[:, 2], 2 * focal_length / (img_res * pred_cam[:, 0] + 1e-9)], dim=-1
        )

        gt_keypoints_2d = targets["keypoints2d"].float()
        num_keypoints = gt_keypoints_2d.shape[1]

        has_smpl = targets["has_smpl"].view(-1).bool()  # flag that indicates whether SMPL parameters are valid
        ...
        # it comes from SMPL
        gt_out = self.body_model_train(betas=gt_betas, body_pose=gt_pose, global_orient=gt_global_orient)
        ...
        opt_output = self.body_model_train(betas=opt_betas, body_pose=opt_pose[:, 3:], global_orient=opt_pose[:, :3])
        ...
        gt_cam_t = estimate_translation(
            gt_model_joints, gt_keypoints_2d_orig, focal_length=focal_length, img_size=img_res
        )
        ...
        if self.registration_mode == "in_the_loop":
            ...
            new_opt_vertices = registrant_output["vertices"] - pred_cam_t.unsqueeze(1)
            new_opt_joints = registrant_output["joints"] - pred_cam_t.unsqueeze(1)
            ...
            new_opt_joint_loss = registrant_output["keypoint2d_loss"].sum(dim=-1).sum(dim=-1)
    ...
    def optimize_discrinimator(self, predictions: dict, data_batch: dict, optimizer: dict):
        ...
        disc_losses = {}
        disc_losses["real_loss"] = self.loss_adv(real_score, target_is_real=True, is_disc=True)
        disc_losses["fake_loss"] = self.loss_adv(fake_score, target_is_real=False, is_disc=True)
    ...
    def compute_keypoints3d_loss(
        ...
        gt_pelvis = (gt_keypoints3d[:, right_hip_idx, :] + gt_keypoints3d[:, left_hip_idx, :]) / 2
        pred_pelvis = (pred_keypoints3d[:, right_hip_idx, :] + pred_keypoints3d[:, left_hip_idx, :]) / 2

        gt_keypoints3d = gt_keypoints3d - gt_pelvis[:, None, :]
        pred_keypoints3d = pred_keypoints3d - pred_pelvis[:, None, :]
        loss = self.loss_keypoints3d(pred_keypoints3d, gt_keypoints3d, reduction_override="none")
    ...
    def compute_keypoints2d_loss(
        ...
        pred_keypoints2d = project_points(pred_keypoints3d, pred_cam, focal_length=focal_length, img_res=img_res)
        ...
        loss = self.loss_keypoints2d(pred_keypoints2d, gt_keypoints2d, reduction_override="none")
    ...
    def compute_keypoints2d_loss_cliff(
        ...
        pred_keypoints2d = torch.cat((pred_keypoints2d, torch.ones(batch_size, num_keypoints, 1).to(device)), dim=2)
        # trans @ pred_keypoints2d2
        pred_keypoints2d = torch.einsum("bij,bkj->bki", trans, pred_keypoints2d)
    ...
    def compute_vertex_loss(self, pred_vertices: torch.Tensor, gt_vertices: torch.Tensor, has_smpl: torch.Tensor):
    ...
    def compute_smpl_pose_loss(self, pred_rotmat: torch.Tensor, gt_pose: torch.Tensor, has_smpl: torch.Tensor):
    ...
    def compute_smpl_betas_loss(self, pred_betas: torch.Tensor, gt_betas: torch.Tensor, has_smpl: torch.Tensor):
    ...
    def compute_part_segmentation_loss(
        ...
        if ph != h or pw != w:
            pred_heatmap_valid = F.interpolate(input=pred_heatmap_valid, size=(h, w), mode="bilinear")
    ...
    def compute_losses(self, predictions: dict, targets: dict):
        ...
        center, scale, focal_length = targets["center"], targets["scale"][:, 0], targets["focal_length"].squeeze(dim=1)
        full_img_shape = torch.hstack((img_h, img_w))
        pred_cam = cam_crop2full(pred_cam_crop, center, scale, full_img_shape, focal_length).to(torch.float32)
        ...
        if self.loss_vertex is not None:
            losses["vertex_loss"] = self.compute_vertex_loss(pred_vertices, gt_vertices, has_smpl)
        if self.loss_smpl_pose is not None:
            losses["smpl_pose_loss"] = self.compute_smpl_pose_loss(pred_pose, gt_pose, has_smpl)
        if self.loss_smpl_betas is not None:
            losses["smpl_betas_loss"] = self.compute_smpl_betas_loss(pred_betas, gt_betas, has_smpl)
        ...
        if self.loss_segm_mask is not None:
            losses["loss_segm_mask"] = self.compute_part_segmentation_loss(
                pred_segm_mask, gt_vertices, gt_keypoints2d, gt_model_joints, has_smpl
            )
...
class CliffImageBodyModelEstimator(BodyModelEstimator):
    ...
    def forward_test(self, img: torch.Tensor, img_metas: dict, **kwargs):
        ...
        center, scale, focal_length = kwargs["center"], kwargs["scale"][:, 0], kwargs["focal_length"].squeeze(dim=1)
        full_img_shape = torch.hstack((img_h, img_w))

        pred_cam = cam_crop2full(pred_cam_crop, center, scale, full_img_shape, focal_length).to(torch.float32)

        pred_output = self.body_model_test(
            betas=pred_betas, body_pose=pred_pose[:, 1:], global_orient=pred_pose[:, 0].unsqueeze(1), pose2rot=False
        )
