...
class SMPLXBodyModelEstimator(BaseArchitecture, metaclass=ABCMeta):
        ...
        if extra_hand_model_cfg is not None:
            ...
            if crop_cfg is not None:
                self.crop_hand_func = SMPLXHandCropFunc(
                    self.hand_head, self.body_model_train, convention=self.convention, **crop_cfg
                )
        ...
        if extra_face_model_cfg is not None:
            ...
            if crop_cfg is not None:
                self.crop_face_func = SMPLXFaceCropFunc(
                    self.face_head, self.body_model_train, convention=self.convention, **crop_cfg
                )
    ...
    def compute_keypoints3d_loss(
        ...
        loss = self.loss_keypoints3d(pred_keypoints3d, gt_keypoints3d, weight=keypoints3d_conf[target_idxs])
    ...
    def compute_keypoints2d_loss(
        ...
        loss = self.loss_keypoints2d(pred_keypoints2d, gt_keypoints2d, weight=keypoints2d_conf[target_idxs])
        loss /= gt_keypoints2d.shape[0]
        return loss

    def compute_smplx_body_pose_loss(
        self, pred_rotmat: torch.Tensor, gt_pose: torch.Tensor, has_smplx_body_pose: torch.Tensor
    ):
        ...
        loss = self.loss_smplx_body_pose(pred_rotmat[target_idxs], gt_rotmat[target_idxs])
        return loss

    def compute_smplx_global_orient_loss(
        self, pred_rotmat: torch.Tensor, gt_global_orient: torch.Tensor, has_smplx_global_orient: torch.Tensor
    ):
        ...
        loss = self.loss_smplx_global_orient(pred_rotmat[target_idxs], gt_rotmat[target_idxs])
        return loss

    def compute_smplx_jaw_pose_loss(
        self,
        ...
        loss = self.loss_smplx_jaw_pose(pred_rotmat[target_idxs], gt_rotmat[target_idxs], weight=conf[target_idxs])
        return loss

    def compute_smplx_hand_pose_loss(
        self,
        pred_rotmat: torch.Tensor,
        ...
        loss = self.loss_smplx_hand_pose(pred_rotmat[target_idxs], gt_rotmat[target_idxs], weight=conf[target_idxs])
        return loss

    def compute_smplx_betas_loss(self, pred_betas: torch.Tensor, gt_betas: torch.Tensor, has_smplx_betas: torch.Tensor):
        ...
        loss = self.loss_smplx_betas(pred_betas[target_idxs], gt_betas[target_idxs])
    ...
    def compute_smplx_expression_loss(
        self,
    ...
    def compute_face_crop_loss(
        ...
        pred_keypoints2d_hd = torch.einsum(
            "bij,bkj->bki", [face_inv_crop_transforms[:, :2, :2], pred_keypoints2d]
        ) + face_inv_crop_transforms[:, :2, 2].unsqueeze(dim=1)
        gt_keypoints2d_hd = torch.einsum(
            "bij,bkj->bki", [face_inv_crop_transforms[:, :2, :2], gt_keypoints2d]
        ) + face_inv_crop_transforms[:, :2, 2].unsqueeze(dim=1)
        ...
        face_img_keypoints = torch.einsum(
            "bij,bkj->bki", [inv_face_crop_transf[:, :2, :2], pred_face_keypoints_hd]
        ) + inv_face_crop_transf[:, :2, 2].unsqueeze(dim=1)
        gt_face_keypoints_hd = gt_keypoints2d_hd[:, self.face_idxs]
        gt_face_keypoints = torch.einsum(
            "bij,bkj->bki", [inv_face_crop_transf[:, :2, :2], gt_face_keypoints_hd]
        ) + inv_face_crop_transf[:, :2, 2].unsqueeze(dim=1)
    ...
    def compute_hand_crop_loss(
        ...
        pred_keypoints2d_hd = torch.einsum(
            "bij,bkj->bki", [hand_inv_crop_transforms[:, :2, :2], pred_keypoints2d]
        ) + hand_inv_crop_transforms[:, :2, 2].unsqueeze(dim=1)
        gt_keypoints2d_hd = torch.einsum(
            "bij,bkj->bki", [hand_inv_crop_transforms[:, :2, :2], gt_keypoints2d]
        ) + hand_inv_crop_transforms[:, :2, 2].unsqueeze(dim=1)
        ...
        left_hand_img_keypoints = torch.einsum(
            "bij,bkj->bki", [inv_left_hand_crop_transf[:, :2, :2], pred_left_hand_keypoints_hd]
        ) + inv_left_hand_crop_transf[:, :2, 2].unsqueeze(dim=1)
        gt_left_hand_keypoints_hd = gt_keypoints2d_hd[:, self.left_hand_idxs]
        gt_left_hand_keypoints = torch.einsum(
            "bij,bkj->bki", [inv_left_hand_crop_transf[:, :2, :2], gt_left_hand_keypoints_hd]
        ) + inv_left_hand_crop_transf[:, :2, 2].unsqueeze(dim=1)
        ...
        right_hand_img_keypoints = torch.einsum(
            "bij,bkj->bki", [inv_right_hand_crop_transf[:, :2, :2], pred_right_hand_keypoints_hd]
        ) + inv_right_hand_crop_transf[:, :2, 2].unsqueeze(dim=1)
        gt_right_hand_keypoints_hd = gt_keypoints2d_hd[:, self.right_hand_idxs]
        gt_right_hand_keypoints = torch.einsum(
            "bij,bkj->bki", [inv_right_hand_crop_transf[:, :2, :2], gt_right_hand_keypoints_hd]
        ) + inv_right_hand_crop_transf[:, :2, 2].unsqueeze(dim=1)
    ...
    def compute_losses(self, predictions: dict, targets: dict):
        ...
        if self.loss_smplx_body_pose is not None:
            ...
            losses["smplx_body_pose_loss"] = self.compute_smplx_body_pose_loss(pred_pose, gt_pose, has_smplx_body_pose)
        ...
        if self.loss_smplx_betas is not None:
            ...
            losses["smplx_betas_loss"] = self.compute_smplx_betas_loss(pred_betas, gt_betas, has_smplx_betas)
        ...
        if self.loss_smplx_betas_piror is not None:
            pred_betas = pred_param["betas"]
            losses["smplx_betas_prior_loss"] = self.compute_smplx_betas_prior_loss(pred_betas)
...
class FrozenBatchNorm2d(nn.Module):
    ...
    def from_bn(module: nn.BatchNorm2d):
        ...
        missing, not_found = frozen_module.load_state_dict(module.state_dict(), strict=False)
