...
class HybrIK_trainer(BaseArchitecture, metaclass=ABCMeta):
    ...
    def forward_train(self, img, img_metas, **kwargs):
        ...
        predictions = self.head(features, trans_inv, intrinsic_param, joint_root, depth_factor, self.smpl)
    ...
    def compute_losses(self, predictions, targets):
        ...
        if self.loss_beta is not None:
            losses["loss_beta"] = self.loss_beta(
                predictions["pred_shape"] * smpl_weight, targets["target_beta"] * smpl_weight
            )
        if self.loss_theta is not None:
            pred_pose = rotmat_to_quat(predictions["pred_pose"]).reshape(-1, 96)
            losses["loss_theta"] = self.loss_theta(
                pred_pose * smpl_weight * targets["target_theta_weight"],
                targets["target_theta"] * smpl_weight * targets["target_theta_weight"],
            )
        ...
        if self.loss_uvd is not None:
            ...
            target_uvd_weight = targets["target_weight_29"][:, : pred_uvd.shape[1]]
            losses["loss_uvd"] = self.loss_uvd(
                64 * predictions["pred_uvd_jts"], 64 * target_uvd, target_uvd_weight, avg_factor=target_uvd_weight.sum()
            )
    ...
    def forward_test(self, img, img_metas, **kwargs):
        ...
        output = self.head(features, trans_inv, intrinsic_param, joint_root, depth_factor, self.smpl)

        pred_uvd_jts = output["pred_uvd_jts"]
        batch_num = pred_uvd_jts.shape[0]
        pred_xyz_jts_24 = output["pred_xyz_jts_24"].reshape(batch_num, -1, 3)[:, :24, :]
        pred_xyz_jts_24_struct = output["pred_xyz_jts_24_struct"].reshape(batch_num, 24, 3)
        ...
        for i in range(pred_xyz_jts_17.shape[0]):
            ...
            pose_coords, _ = heatmap2coord(pred_uvd_jts[i], pred_scores[i], hm_shape, bbox, mean_bbox_scale=None)
