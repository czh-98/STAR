...
class KeypointMSELoss(nn.Module):
    ...
    def __init__(self, reduction="mean", loss_weight=1.0, sigma=1.0, keypoint_weight=None):
        super().__init__()
        assert reduction in (None, "none", "mean", "sum")
        reduction = "none" if reduction is None else reduction
        self.reduction = reduction
    ...
    def forward(
        ...
        B, K, D = pred.shape
        pred_conf = pred_conf.view((B, K, 1)) if pred_conf is not None else 1.0
        target_conf = target_conf.view((B, K, 1)) if target_conf is not None else 1.0
        keypoint_weight = (
            keypoint_weight.view((1, K, 1))
            if keypoint_weight is not None
            else self.keypoint_weight.view((1, K, 1)).type_as(pred) if self.keypoint_weight is not None else 1.0
        )

        weight = keypoint_weight * pred_conf * target_conf
        assert isinstance(weight, float) or weight.shape == (B, K, 1) or weight.shape == (1, K, 1)
        ...
        loss = loss_weight * mse_loss_with_gmof(
            pred, target, weight, reduction=reduction, avg_factor=avg_factor, sigma=self.sigma
        )
