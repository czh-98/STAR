...
class SMPLX(_SMPLX):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if extra_joints_regressor is not None:
            joints_regressor_extra = torch.tensor(np.load(extra_joints_regressor), dtype=torch.float)
            self.register_buffer("joints_regressor_extra", joints_regressor_extra)
    ...
    def forward(self, *args, return_verts: bool = True, return_full_pose: bool = False, **kwargs) -> dict:
        """Forward function.
        ...
        if not hasattr(self, "joints_regressor"):
            ...
            joints = vertices2joints(self.joints_regressor, smplx_output.vertices)

        if hasattr(self, "joints_regressor_extra"):
            extra_joints = vertices2joints(self.joints_regressor_extra, smplx_output.vertices)
            joints = torch.cat([joints, extra_joints], dim=1)

        joints, joint_mask = convert_kps(
            joints, src=self.keypoint_src, dst=self.keypoint_dst, approximate=self.keypoint_approximate
        )
        if isinstance(joint_mask, np.ndarray):
            joint_mask = torch.tensor(joint_mask, dtype=torch.uint8, device=joints.device)
    ...
    def tensor2dict(
        ...
        expression = (
            expression.view(batch_size, -1)
            if expression is not None
            else torch.zeros(batch_size, 10).to(body_pose.device)
        )
        return {
            "betas": betas,
            ...
            "right_hand_pose": right_hand_pose.view(batch_size, NUM_HAND_JOINTS * 3),
            "transl": transl,
    ...
    def dict2tensor(cls, smplx_dict: dict) -> torch.Tensor:
        ...
        left_hand_pose = smplx_dict.get("left_hand_pose", torch.zeros((batch_size, NUM_HAND_JOINTS, 3)))
        right_hand_pose = smplx_dict.get("right_hand_pose", torch.zeros((batch_size, NUM_HAND_JOINTS, 3)))
        full_pose = torch.cat(
            [
                global_orient,
                body_pose,
...
class GenderedSMPLX(nn.Module):
        ...
        **kwargs,
        ...
        if extra_joints_regressor is not None:
            self.register_buffer(
                "J_regressor_extra", torch.tensor(np.load(extra_joints_regressor), dtype=torch.float32)
            )
        ...
        smplx2lhand_joints = [SMPLX_JOINT_IDS[name] for name in MANO_LEFT_REORDER_KEYPOINTS]
        smplx2rhand_joints = [SMPLX_JOINT_IDS[name] for name in MANO_RIGHT_REORDER_KEYPOINTS]
        ...
        smplx2lfoot_joints = [SMPLX_JOINT_IDS[f"left_{name}"] for name in FOOT_NAMES]
        smplx2rfoot_joints = [SMPLX_JOINT_IDS[f"right_{name}"] for name in FOOT_NAMES]
    ...
    def forward(
        ...
        **kwargs,
        ...
        smplx_kwargs = dict(betas=betas, body_pose=body_pose, global_orient=global_orient)
        for k in param_keys:
            if k in kwargs:
                smplx_kwargs[k] = kwargs[k]

        if "gender" not in kwargs:
            smplx_kwargs["gender"] = 2 * torch.ones(batch_size).to(body_pose.device)
        ...
        for gi, g in enumerate(["male", "female", "neutral"]):
            gender_idx = (smplx_kwargs["gender"] == gi).nonzero(as_tuple=True)[0]
            if len(gender_idx) == 0:
                continue
            gender_idx_list.extend([int(idx) for idx in gender_idx])
            gender_kwargs = {k: smplx_kwargs[k][gender_idx] for k in param_keys if k in smplx_kwargs}
        ...
        joints, _ = convert_kps(
            smpl_54, src=self.keypoint_src, dst=self.keypoint_dst, approximate=self.keypoint_approximate
        )
