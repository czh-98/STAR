...
class SMPL(_SMPL):
    ...
    def forward(self, *args, return_verts: bool = True, return_full_pose: bool = False, **kwargs) -> dict:
        ...
        if not hasattr(self, "joints_regressor"):
            ...
            joints = vertices2joints(self.joints_regressor, smpl_output.vertices)

        if hasattr(self, "joints_regressor_extra"):
            extra_joints = vertices2joints(self.joints_regressor_extra, smpl_output.vertices)
    ...
    @classmethod
    def tensor2dict(
        cls, full_pose: torch.Tensor, betas: Optional[torch.Tensor] = None, transl: Optional[torch.Tensor] = None
    ):
    ...
    def forward(
        ...
        **kwargs
        ...
        if gender is not None:
            output = {
                "vertices": torch.zeros([batch_size, self.num_verts, 3], device=device),
                "joints": torch.zeros([batch_size, self.num_joints, 3], device=device),
                "joint_mask": torch.zeros([batch_size, self.num_joints], dtype=torch.uint8, device=device),
            }

            for body_model, gender_label in [(self.smpl_neutral, -1), (self.smpl_male, 0), (self.smpl_female, 1)]:
                ...
                output_model = body_model(
                    ...
                    body_pose=body_pose[gender_idxs] if body_pose is not None else None,
                    global_orient=global_orient[gender_idxs] if global_orient is not None else None,
                ...
                if "joint_mask" in output_model:
                    output["joint_mask"][gender_idxs] = output_model["joint_mask"]

                if return_verts:
                    output["vertices"][gender_idxs] = output_model["vertices"]
                if return_full_pose:
                    output["full_pose"][gender_idxs] = output_model["full_pose"]
        else:
            output = self.smpl_neutral(
                betas=betas, body_pose=body_pose, global_orient=global_orient, transl=transl, **kwargs
            )
...
class HybrIKSMPL(SMPL):
    ...
    def forward(self, pose_skeleton, betas, phis, global_orient, transl=None, return_verts=True, leaf_thetas=None):
        ...
        test_joints = True
        if test_joints:
            new_joints, A = batch_rigid_transform(rot_mats, rest_J[:, :24].clone(), self.parents[:24], dtype=self.dtype)
        else:
            new_joints = None
