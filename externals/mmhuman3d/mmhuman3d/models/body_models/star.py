...
class STAR(nn.Module):
    ...
    def __init__(
        ...
        if gender not in ["male", "female", "neutral"]:
            raise RuntimeError("Invalid gender! Should be one of " "['male', 'female', or 'neutral']!")
        self.gender = gender
        ...
        if not os.path.exists(model_path):
            ...
            if os.path.split(model_path)[-1] != model_fname:
                raise RuntimeError(f"Model filename ({model_fname}) and gender " f"({gender}) are incompatible!")
        ...
        self.register_buffer("J_regressor", torch.tensor(J_regressor, dtype=torch.float))

        # Model skinning weights
        self.register_buffer("weights", torch.tensor(star_model["weights"], dtype=torch.float))

        # Model pose corrective blend shapes
        self.register_buffer("posedirs", torch.tensor(star_model["posedirs"].reshape((-1, 93)), dtype=torch.float))

        # Mean Shape
        self.register_buffer("v_template", torch.tensor(star_model["v_template"], dtype=torch.float))

        # Shape corrective blend shapes
        self.register_buffer("shapedirs", torch.tensor(star_model["shapedirs"][:, :, :num_betas], dtype=torch.float))

        # Mesh traingles
        self.register_buffer("faces", torch.from_numpy(star_model["f"].astype(np.int64)))
        self.f = star_model["f"]
        self.register_buffer(
            "faces_tensor", torch.from_numpy(star_model["f"].astype(np.int64))
        )  # alias for face tensor in render
        ...
        if create_body_pose:
            if body_pose is None:
                default_body_pose = torch.zeros([batch_size, self.NUM_JOINTS, 3, 3], dtype=dtype)
            else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        if len(body_pose.shape) == 4:
            ...
            body_pose = rotation_matrix_to_angle_axis(body_pose.view(-1, 3, 3)).view(batch_size, -1, 3)
        ...
        root_transform = self.with_zeros(torch.cat((R[:, 0], J[:, 0][:, :, None]), 2))
        ...
        joints, joint_mask = convert_kps(
            v.J_transformed, src=self.keypoint_src, dst=self.keypoint_dst, approximate=self.keypoint_approximate
        )
