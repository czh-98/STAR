...
def batch_inverse_kinematics_transform(
    pose_skeleton, global_orient, phis, rest_pose, children, parents, dtype=torch.float32, train=False, leaf_thetas=None
):
    ...
    if train:
        ...
        final_pose_skeleton += rel_rest_pose[:, 0:1] - final_pose_skeleton[:, 0:1]
...
def batch_get_pelvis_orient(rel_pose_skeleton, rel_rest_pose, parents, children, dtype):
    ...
    axis = axis / (axis_norm + 1e-8)
    angle = torch.arccos(
        torch.sum(spine_rest_loc * spine_final_loc, dim=1, keepdim=True) / (vec_rest_norm * vec_final_norm + 1e-8)
    )
