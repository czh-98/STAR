...
class SMPLXHandMergeFunc:
    ...
    def __init__(self, body_model, convention="smplx"):
        ...
        self.left_wrist_kin_chain = find_joint_kin_chain(self.left_wrist_idx, self.body_model.parents)
        ...
        self.right_wrist_kin_chain = find_joint_kin_chain(self.right_wrist_idx, self.body_model.parents)
    ...
    def __call__(self, body_predictions, hand_predictions):
        ...
        hands_from_body_idxs = torch.arange(0, 2 * batch_size, dtype=torch.long, device=device)
        ...
        wrist_pose_from_hand = torch.matmul(
            parent_rots.reshape(-1, 3, 3).transpose(1, 2), wrist_pose_from_hand.reshape(-1, 3, 3)
        )
        ...
        body_predictions["pred_param"]["body_pose"][:, self.right_wrist_idx - 1] = right_hand_wrist
        body_predictions["pred_param"]["body_pose"][:, self.left_wrist_idx - 1] = left_hand_wrist
...
class SMPLXFaceMergeFunc:
    ...
    def __call__(self, body_predictions, face_predictions):
        ...
        Returns:
            dict: Merged prediction.
        """
        body_predictions["pred_param"]["jaw_pose"] = face_predictions["pred_param"]["jaw_pose"]
        body_predictions["pred_param"]["expression"] = face_predictions["pred_param"]["expression"][
...
def get_crop_info(points, img_metas, scale_factor: float = 1.0, crop_size: int = 256):
    ...
    crop_transforms = torch.cat(
        [crop_transforms, torch.tensor([0.0, 0.0, 1.0], dtype=dtype, device=device).expand([batch_size, 1, 3])], dim=1
    )

    inv_crop_transforms = torch.inverse(crop_transforms)

    # center on the cropped body image
    center_body_crop, bbox_size = points_to_bbox(points, bbox_scale_factor=scale_factor)

    orig_bbox_size = bbox_size / crop_size * img_bbox_sizes

    # Compute the center of the crop in the original image
    center = (
        torch.einsum("bij,bj->bi", [inv_crop_transforms[:, :2, :2], center_body_crop]) + inv_crop_transforms[:, :2, 2]
    )
...
class CropSampler:
    ...
    def __call__(self, full_imgs, center, bbox_size):
        ...
        transforms = (
            torch.eye(3, dtype=full_imgs.dtype, device=full_imgs.device)
            .reshape(1, 3, 3)
        ...
        hd_to_crop[:, 0, 2] = -(center[:, 0] - bbox_size * 0.5) * hd_to_crop[:, 0, 0] - 1
        hd_to_crop[:, 1, 2] = -(center[:, 1] - bbox_size * 0.5) * hd_to_crop[:, 1, 1] - 1
        ...
        sampling_grid = (
            torch.bmm(full_transform[:, :2, :2], batch_grid.transpose(1, 2)) + full_transform[:, :2, [2]]
        ).transpose(1, 2)
        sampling_grid = sampling_grid.reshape(-1, self.crop_size, self.crop_size, 2).transpose(1, 2)
...
class SMPLXHandCropFunc:
    ...
    def __init__(
        ...
        self.left_wrist_kin_chain = find_joint_kin_chain(left_wrist_idx, self.body_model.parents)

        self.right_hand_idxs = get_keypoint_idxs_by_part("right_hand", self.convention)
        right_wrist_idx = get_keypoint_idx("right_wrist", self.convention)
        self.right_hand_idxs.append(right_wrist_idx)
        self.right_wrist_kin_chain = find_joint_kin_chain(right_wrist_idx, self.body_model.parents)
    ...
    def build_hand_mean(self, global_orient, body_pose, betas, left_hand_pose, raw_right_hand_pose, batch_size):
        ...
        shape_condition = (
            betas if self.condition_hand_shape else self.model_head.get_mean("shape", batch_size=batch_size)
        )
        right_finger_pose_condition = (
            right_hand_pose
            if self.condition_hand_finger_pose
            else self.model_head.get_mean("right_hand_pose", batch_size=batch_size)
        )
        right_hand_mean = torch.cat(
            [right_wrist_pose, right_finger_pose_condition, shape_condition, camera_mean], dim=1
        )
        ...
        left_hand_mean = torch.cat(
            [left_to_right_wrist_pose, left_finger_pose_condition, shape_condition, camera_mean], dim=1
        )
    ...
    def __call__(self, body_predictions, img_metas):
        ...
        pred_keypoints2d = weak_perspective_projection(
            pred_keypoints3d, scale=pred_cam[:, 0], translation=pred_cam[:, 1:3]
        )
        # concat ori_img
        full_images = []
        for img_meta in img_metas:
        ...
        # left hand
        left_hand_joints = (pred_keypoints2d[:, self.left_hand_idxs] * 0.5 + 0.5) * (self.img_res - 1)
        left_hand_points_to_crop = get_crop_info(left_hand_joints, img_metas, self.scale_factor, self.img_res)
        ...
        right_hand_points_to_crop = get_crop_info(right_hand_joints, img_metas, self.scale_factor, self.img_res)
...
class SMPLXFaceCropFunc:
    ...
    def __init__(
        ...
        self.neck_kin_chain = find_joint_kin_chain(neck_idx, self.body_model.parents)
    ...
    def build_face_mean(self, global_orient, body_pose, betas, raw_jaw_pose, expression, batch_size):
        ...
        neck_pose_condition = (
            neck_pose
            if self.condition_face_neck_pose
            else self.model_head.get_mean("global_orient", batch_size=batch_size)
        )

        jaw_pose_condition = (
            raw_jaw_pose.reshape(batch_size, -1)
            if self.condition_face_jaw_pose
            else self.model_head.get_mean("jaw_pose", batch_size=batch_size)
        )
        ...
        betas_condition = (
            ...
            if self.condition_face_shape
            else self.model_head.get_mean("shape", batch_size=batch_size)
        )
        ...
        expression_condition = (
            F.pad(expression.reshape(batch_size, -1), (0, expr_padding_size))
            if self.condition_face_expression
    ...
    def __call__(self, body_predictions, img_metas):
        ...
        face_joints = (pred_keypoints2d[:, self.face_idx] * 0.5 + 0.5) * (self.img_res - 1)
        face_points_to_crop = get_crop_info(face_joints, img_metas, self.scale_factor, self.img_res)
        ...
        crop_info = dict(face_inv_crop_transforms=face_inv_crop_transforms, face_crop_transform=face_crop_transform)
...
def get_partial_smpl(partial_mesh_path: str = "data/partial_mesh/"):
    ...
    for part in ["lhand", "rhand", "face", "arm", "forearm", "larm", "rarm", "lwrist", "rwrist"]:
        part_vid_fname = os.path.join(partial_mesh_path, f"smpl_{part}_vids.npz")
