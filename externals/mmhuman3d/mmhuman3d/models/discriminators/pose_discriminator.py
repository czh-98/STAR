...
class BaseDiscriminator(nn.Module):
    ...
    def _check(self):
        ...
        if not isinstance(self.use_dropout, tuple):
            raise TypeError(f"use_dropout require tuple, " f"get {type(self.use_dropout)}")
        ...
        if not isinstance(self.use_activation, tuple):
            raise TypeError(f"use_activation require tuple, " f"get {type(self.use_activation)}")
...
class ShapeDiscriminator(BaseDiscriminator):
    ...
    def __init__(self, fc_layers, use_dropout, drop_prob, use_activation):
        if fc_layers[-1] != 1:
            msg = f"the neuron count of the last layer " f"must be 1, but got {fc_layers[-1]}"
            raise ValueError(msg)
...
class PoseDiscriminator(nn.Module):
    ...
    def __init__(self, channels, joint_count):
        super().__init__()
        if channels[-1] != 1:
            msg = f"the neuron count of the last layer " f"must be 1, but got {channels[-1]}"
            raise ValueError(msg)
...
class FullPoseDiscriminator(BaseDiscriminator):
    ...
    def __init__(self, fc_layers, use_dropout, drop_prob, use_activation):
        if fc_layers[-1] != 1:
            msg = f"the neuron count of the last layer must be 1," f" but got {fc_layers[-1]}"
            raise ValueError(msg)
...
class SMPLDiscriminator(nn.Module):
    ...
    def __init__(
        self, beta_channel=(10, 5, 1), per_joint_channel=(9, 32, 32, 1), full_pose_channel=(23 * 32, 1024, 1024, 1)
    ):
        super().__init__()
        self.joint_count = 23
