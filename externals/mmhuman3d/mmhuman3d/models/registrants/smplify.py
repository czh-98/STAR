...
class SMPLify(object):
    ...
    def __init__(
        ...
        if isinstance(body_model, dict):
            ...
            raise TypeError(f"body_model should be either dict or " f"torch.nn.Module, but got {type(body_model)}")
        ...
        if camera is not None:
            if isinstance(camera, dict):
                ...
                raise TypeError(f"camera should be either dict or " f"torch.nn.Module, but got {type(camera)}")
    ...
    def __call__(
        ...
        """
        assert keypoints2d is not None or keypoints3d is not None, "Neither of 2D nor 3D keypoints are provided."
        assert not (keypoints2d is not None and keypoints3d is not None), "Do not provide both 2D and 3D keypoints."
        batch_size = keypoints2d.shape[0] if keypoints2d is not None else keypoints3d.shape[0]
        ...
        # collate results
        ret = {"global_orient": global_orient, "transl": transl, "body_pose": body_pose, "betas": betas}
    ...
    def _compute_loss(
        ...
        # 2D keypoint loss
        if keypoints2d is not None and not self._skip_loss(self.keypoints2d_mse_loss, keypoints2d_weight):
        ...
        # 3D keypoint loss
        if keypoints3d is not None and not self._skip_loss(self.keypoints3d_mse_loss, keypoints3d_weight):
        ...
        if not self._skip_loss(self.shape_prior_loss, shape_prior_weight):
            shape_prior_loss = self.shape_prior_loss(
                betas=betas, loss_weight_override=shape_prior_weight, reduction_override=reduction_override
            )
        ...
        if not self._skip_loss(self.joint_prior_loss, joint_prior_weight):
            joint_prior_loss = self.joint_prior_loss(
                body_pose=body_pose, loss_weight_override=joint_prior_weight, reduction_override=reduction_override
            )
        ...
        if not self._skip_loss(self.smooth_loss, smooth_loss_weight):
            smooth_loss = self.smooth_loss(
                body_pose=body_pose, loss_weight_override=smooth_loss_weight, reduction_override=reduction_override
            )
        ...
        if not self._skip_loss(self.pose_prior_loss, pose_prior_weight):
            pose_prior_loss = self.pose_prior_loss(
                body_pose=body_pose, loss_weight_override=pose_prior_weight, reduction_override=reduction_override
            )
        ...
        if not self._skip_loss(self.pose_reg_loss, pose_reg_weight):
            pose_reg_loss = self.pose_reg_loss(
                body_pose=body_pose, loss_weight_override=pose_reg_weight, reduction_override=reduction_override
            )
    ...
    def _match_init_batch_size(
        self, init_param: torch.Tensor, init_param_body_model: torch.Tensor, batch_size: int
    ) -> torch.Tensor:
        ...
        param = init_param.detach().clone() if init_param is not None else init_param_body_model.detach().clone()
        ...
        if param_batch_size != batch_size:
            if param_batch_size == 1:
                ...
                raise ValueError("Init param does not match the batch size of " "keypoints, and is not 1.")
    ...
    def _set_keypoint_idxs(self) -> None:
        ...
        shoulder_keypoint_idxs = get_keypoint_idxs_by_part("shoulder", convention=convention)
    ...
    def _get_weight(self, use_shoulder_hip_only: bool = False, body_weight: float = 1.0) -> torch.Tensor:
    ...
    def _skip_loss(loss, loss_weight_override):
        ...
        """
        if (loss is None) or (loss.loss_weight == 0 and loss_weight_override is None) or (loss_weight_override == 0):
            return True
        return False
