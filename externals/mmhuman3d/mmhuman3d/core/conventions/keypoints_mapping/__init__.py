...
def convert_kps(
    ...
    if isinstance(keypoints, torch.Tensor):
        ...
        raise TypeError(
            "Type of keypoints is neither" + " torch.Tensor nor np.ndarray.\n" + f"Type of keypoints: {type(keypoints)}"
        )

    if mask is not None:
        assert type(mask) == type(keypoints)
    else:
        mask = new_array_func(shape=(keypoints.shape[-2],), value=1, device_data=keypoints, if_uint8=True)
    ...
    out_keypoints = new_array_func(
        shape=(keypoints.shape[0], len(dst_names), keypoints.shape[-1]), value=0, device_data=keypoints, if_uint8=False
    )

    original_mask = mask
    if original_mask is not None:
        original_mask = original_mask.reshape(-1)
        assert original_mask.shape[0] == len(src_names), f"The length of mask should be {len(src_names)}"

    mask = new_array_func(shape=(len(dst_names),), value=0, device_data=keypoints, if_uint8=True)
    ...
    mask[dst_idxs] = to_type_uint8_func(original_mask[src_idxs]) if original_mask is not None else 1.0
...
def get_mapping(src: str, dst: str, approximate: bool = False, keypoints_factory: dict = KEYPOINTS_FACTORY):
    ...
    if (
        ...
        if unmapped_names:
            warn_message = f"Approximate mapping {unmapped_names}" + f" to {approximate_names}"
...
def get_flip_pairs(convention: str = "smplx", keypoints_factory: dict = KEYPOINTS_FACTORY) -> List[int]:
...
def get_keypoint_idx(
    name: str, convention: str = "smplx", approximate: bool = False, keypoints_factory: dict = KEYPOINTS_FACTORY
) -> List[int]:
...
def get_keypoint_num(convention: str = "smplx", keypoints_factory: dict = KEYPOINTS_FACTORY) -> List[int]:
