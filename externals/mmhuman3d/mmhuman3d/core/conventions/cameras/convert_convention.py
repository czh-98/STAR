...
def enc_camera_convention(convention, camera_conventions=CAMERA_CONVENTIONS):
    ...
    if convention in camera_conventions:
        ...
        assert set(convention).issubset({"x", "y", "z", "+", "-"}), "Wrong convention string, choose either in"
...
def convert_camera_matrix(
    ...
    resolution_src: Optional[Union[int, Tuple[int, int], torch.Tensor, np.ndarray]] = None,
    resolution_dst: Optional[Union[int, Tuple[int, int], torch.Tensor, np.ndarray]] = None,
    ...
    left_mm_ex_src = CAMERA_CONVENTIONS[convention_src].get("left_mm_extrinsic", True)
    view_to_world_src = CAMERA_CONVENTIONS[convention_src].get("view_to_world", False)
    left_mm_in_src = CAMERA_CONVENTIONS[convention_src].get("left_mm_intrinsic", False)

    left_mm_ex_dst = CAMERA_CONVENTIONS[convention_dst].get("left_mm_extrinsic", True)
    view_to_world_dst = CAMERA_CONVENTIONS[convention_dst].get("view_to_world", False)
    left_mm_in_dst = CAMERA_CONVENTIONS[convention_dst].get("left_mm_intrinsic", False)

    sign_src, axis_src = enc_camera_convention(convention_src, camera_conventions)
    sign_dst, axis_dst = enc_camera_convention(convention_dst, camera_conventions)
    ...
    if isinstance(K, np.ndarray):
        ...
        raise TypeError(f"K should be `torch.Tensor` or `numpy.ndarray`, type(K): " f"{type(K)}")
    ...
    if isinstance(R, np.ndarray):
        ...
        raise TypeError(f"R should be `torch.Tensor` or `numpy.ndarray`, type(R): " f"{type(R)}")
    ...
    if isinstance(T, np.ndarray):
        ...
        raise TypeError(f"T should be `torch.Tensor` or `numpy.ndarray`, type(T): " f"{type(T)}")

    if axis_dst != axis_src:
        new_R = ee_to_rotmat(rotmat_to_ee(new_R, convention=axis_src), convention=axis_dst)
    ...
    if in_ndc_dst is False and in_ndc_src is True:
        assert resolution_dst is not None, "dst in screen, should specify resolution_dst."

    if in_ndc_src is False and in_ndc_dst is True:
        assert resolution_src is not None, "src in screen, should specify resolution_dst."
    ...
    if new_K is not None:
        ...
        if in_ndc_src is True and (in_ndc_dst is False):
            new_K = convert_ndc_to_screen(
                K=new_K, is_perspective=is_perspective, sign=sign.to(new_K.device), resolution=resolution_dst
            )
        # src in screen, dst in ndc
        elif in_ndc_src is False and in_ndc_dst is True:
            new_K = convert_screen_to_ndc(
                K=new_K, is_perspective=is_perspective, sign=sign.to(new_K.device), resolution=resolution_src
            )
...
def convert_K_3x3_to_4x4(
    K: Union[torch.Tensor, np.ndarray], is_perspective: bool = True
    ...
    if isinstance(K, torch.Tensor):
        ...
        raise TypeError("K should be `torch.Tensor` or `numpy.ndarray`, " f"type(K): {type(K)}.")
    if K.shape[-2:] == (4, 4):
        warnings.warn(f"shape of K already is {K.shape}, will pass converting.")
        return K
    use_numpy = False
    if K.ndim == 2:
...
def convert_ndc_to_screen(
    K: Union[torch.Tensor, np.ndarray],
    resolution: Union[int, Tuple[int, int], List[int], torch.Tensor, np.ndarray],
    ...
    if isinstance(K, torch.Tensor):
        ...
        raise TypeError(f"K should be `torch.Tensor` or `np.ndarray`, type(K): {type(K)}")
    if K.ndim == 2:
        K = K[None].reshape(-1, 4, 4)
    elif K.ndim == 3:
        K = K.reshape(-1, 4, 4)
...
def convert_world_view(
    ...
    if not (type(R) is type(T)):
        raise TypeError(f"R: {type(R)}, T: {type(T)} should have the same type.")
    if isinstance(R, torch.Tensor):
        ...
        raise TypeError(f"R: {type(R)}, T: {type(T)} should be torch.Tensor " f"or numpy.ndarray.")
    return R, T
