...
class body_segmentation(object):
    ...
    def __init__(self, model_type="smpl") -> None:
        if model_type == "smpl":
            ...
            raise ValueError(f"Wrong model_type: {model_type}." f' Should be in {["smpl", "smplx", "star"]}')
    ...
    def __getitem__(self, key):
        if key in self.DICT.keys():
            ...
            for continuous in raw_segmentation:
                if len(continuous) == 2:
                    part_segmentation.extend(list(range(continuous[0], continuous[1] + 1)))
        ...
        for index in range(len(part_indexes)):
            if len(final_part_indexes[-1]) == 0:
                ...
                if index != len(part_indexes) - 1:
                    ...
                    if (this_index == last_index + 1) and (this_index == next_index - 1):
                        pass
                    elif (this_index == last_index + 1) and (this_index != next_index - 1):
