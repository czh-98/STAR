...
def _plot_kp2d_frame(
    kp2d_person: np.ndarray,
    canvas: np.ndarray,
    limbs: Union[list, dict, np.ndarray] = HUMAN_DATA_LIMBS_INDEX,
    ...
    assert (
        kp2d_person.ndim == 2 and kp2d_person.shape[-1] == 2
    ), f"Wrong input array shape {kp2d_person.shape}, \
            should be (num_kp, 2)"
    ...
    for part_name, part_limbs in limbs.items():
        ...
        if not disable_limbs:
            scatter_points_index = list(set(np.array([part_limbs]).reshape(-1).tolist()))
        else:
        ...
        for index in scatter_points_index:
            ...
            if 0 <= x < canvas.shape[1] and 0 <= y < canvas.shape[0]:
                if disable_limbs:
                    ...
                    point_color = color[min(color.shape[0] - 1, len(scatter_points_index) - 1)].tolist()

                cv2.circle(canvas, (int(x), int(y)), radius, point_color, thickness=-1)
                if with_number:
                    cv2.putText(
                        canvas,
                        str(index),
                        ...
                        font_size,
                        np.array([255, 255, 255]).astype(np.int32).tolist(),
                        2,
                    )
    # draw the bboxes
    if bbox is not None:
        bbox = bbox.astype(np.int32)
        cv2.rectangle(canvas, (bbox[0], bbox[2]), (bbox[1], bbox[3]), (0, 255, 255), 1)
    return canvas


def _get_bbox(keypoint_np: np.ndarray, img_mat: Optional[np.ndarray] = None, expand: bool = False):
    """get bbox of kp2d."""
...
def _prepare_limb_palette(limbs, palette, pop_parts, data_source, mask, search_limbs_func=search_limbs):
    ...
    if limbs is not None:
        ...
        limbs_target, limbs_palette = search_limbs_func(data_source=data_source, mask=mask)
...
def _prepare_output_path(output_path, overwrite):
    """Prepare output path."""
    prepare_output_path(
        output_path, allowed_suffix=[".mp4", ""], tag="output video", path_type="auto", overwrite=overwrite
    )
...
def _check_frame_path(frame_list):
    ...
    for frame_path in frame_list:
        if check_path_existence(frame_path, "file") != Existence.FileExist or not check_path_suffix(
            frame_path, [".png", ".jpg", ".jpeg"]
        ):
            raise FileNotFoundError(f"The frame should be .png or .jp(e)g: {frame_path}")
...
def _check_temp_path(temp_folder, frame_list, overwrite):
    ...
    if not overwrite and frame_list is not None and len(frame_list) > 0:
        if Path(temp_folder).absolute() == Path(frame_list[0]).parent.absolute():
            raise FileExistsError(f"{temp_folder} exists (set --overwrite to overwrite).")


class _CavasProducer:
    """Prepare background canvas, pure white if not set."""

    def __init__(self, frame_list, resolution, kp2d=None, image_array=None, default_scale=1.5):
        ...
        if self.resolution is None:
            ...
            elif len(self.frame_list) > 1 and check_path_existence(self.frame_list[0], "file") == Existence.FileExist:
                tmp_image_array = cv2.imread(self.frame_list[0])
                self.auto_resolution = tmp_image_array.shape[:2]
            else:

                self.auto_resolution = [int(np.max(kp2d) * default_scale), int(np.max(kp2d) * default_scale)]
    ...
    def __getitem__(self, frame_index):
        ...
        # no frame file, resolution has been set
            ...
            canvas = np.ones((self.resolution[0], self.resolution[1], 3), dtype=np.uint8) * 255
            if self.kp2d is None:
                kp2d_frame = None
            else:
                kp2d_frame = self.kp2d[frame_index]
        # frame file exists, resolution has been set
            ...
            canvas = cv2.resize(canvas, (self.resolution[1], self.resolution[0]), cv2.INTER_CUBIC)
            if self.kp2d is None:
                kp2d_frame = None
            else:
                kp2d_frame = np.array([[w_scale, h_scale]]) * self.kp2d[frame_index]
        # no frame file, no resolution
        else:
            canvas = np.ones((self.auto_resolution[0], self.auto_resolution[1], 3), dtype=np.uint8) * 255
            if self.kp2d is None:
                kp2d_frame = None
            else:
                kp2d_frame = self.kp2d[frame_index]
...
def update_frame_list(frame_list, origin_frames, img_format, start, end):
    ...
    if origin_frames is not None:
        ...
        if Path(origin_frames).is_file():
            ...
            video_to_images(origin_frames, input_temp_folder, start=start, end=end)
...
def visualize_kp2d(
    ...
    if image_array is not None:
        ...
        frame_list, input_temp_folder = update_frame_list(frame_list, origin_frames, img_format, start, end)
    ...
    if data_source not in keypoints_factory:
        raise ValueError("Wrong data_source. Should choose in" f"{list(keypoints_factory.keys())}")
    if mask is not None:
    ...
    if disable_limbs:
        ...
        # *** changed by wyj ***
        limbs_target, limbs_palette = _prepare_limb_palette(limbs, palette, pop_parts, data_source, mask)
    ...
    for frame_index in tqdm(range(kp2d.shape[0]), disable=disable_tqdm):
        ...
        for person_index in range(num_person):
            if num_person >= 2 and not disable_limbs:
                limbs_palette = get_different_colors(num_person)[person_index].reshape(1, 3)
        ...
        if with_file_name and frame_list is not None:
            ...
            if frame_index <= len(frame_list) - 1:
                cv2.putText(
                    canvas,
                    str(Path(frame_list[frame_index]).name),
                    (w // 2, h // 10),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.5 * h / 500,
                    np.array([255, 255, 255]).astype(np.int32).tolist(),
        ...
        if output_path is not None:
            ...
            if (
                frame_list is not None
                and check_path_suffix(output_path, "")
                and len(frame_list) >= len(canvas_producer)
            ):
                frame_path = os.path.join(output_temp_folder, Path(frame_list[frame_index]).name)
                img_format = None
            else:
                frame_path = os.path.join(output_temp_folder, f"{frame_index:06d}.png")
                img_format = "%06d.png"
