...
def _norm_pose(
    pose_numpy: np.ndarray, min_value: Union[float, int], max_value: Union[float, int], mask: Union[np.ndarray, list]
):
    ...
    for axis_index in range(axis_num):
        pose_np_normed[..., axis_index] = pose_np_normed[..., axis_index] - axis_stat[axis_index, 2]
...
def visualize_kp3d(
    ...
    if not isinstance(kp3d, np.ndarray):
        raise TypeError(f"Input type is {type(kp3d)}, which should be numpy.ndarray.")
    kp3d = kp3d.copy()
    if kp3d.shape[-1] == 2:
    ...
    if output_path is not None:
        prepare_output_path(output_path, path_type="auto", tag="output video", allowed_suffix=[".mp4", ".gif", ""])
    ...
    if value_range is not None:
        ...
        pose_np_normed = _norm_pose(kp3d, value_range[0] + margin_width, value_range[1] - margin_width, mask_index)
    ...
    if limbs is not None:
        ...
        limbs_target, limbs_palette = search_limbs(data_source=data_source, mask=mask)
    ...
    if isinstance(frame_names, str):
        if "%" in frame_names:
            frame_names = [frame_names % index for index in range(input_pose_np.shape[0])]
        else:
