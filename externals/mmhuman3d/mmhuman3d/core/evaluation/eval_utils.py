...
def vertice_pve(pred_verts, target_verts, alignment="none"):
    ...
    if alignment == "none":
        ...
        pred_verts = np.stack(
            [compute_similarity_transform(pred_i, gt_i) for pred_i, gt_i in zip(pred_verts, target_verts)]
        )
    elif alignment == "scale":
...
def keypoint_3d_pck(pred, gt, mask, alignment="none", threshold=150.0):
...
def fg_vertices_to_mesh_distance(
    ...
    _, tform = compute_similarity_transform(
        predicted_mesh_landmark_points, grundtruth_landmark_points, return_tform=True
    )
    # Use tform to transform all vertices.
    predicted_mesh_vertices_aligned = (
        tform["scale"] * tform["rotation"].dot(predicted_mesh_vertices.T) + tform["translation"]
    ).T

    # Compute the mask: A circular area around the center of the face.
    nose_bottom = np.array(grundtruth_landmark_points[4])
    nose_bridge = (
        np.array(grundtruth_landmark_points[1]) + np.array(grundtruth_landmark_points[2])
