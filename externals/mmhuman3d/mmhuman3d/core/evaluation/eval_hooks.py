    ...
    def __init__(
        ...
        **eval_kwargs
        ...
        if "gpu_collect" in eval_kwargs:
            warnings.warn(
                '"gpu_collect" will be deprecated in EvalHook.' "Please remove it from the config.", DeprecationWarning
            )
        ...
        if "key_indicator" in eval_kwargs or isinstance(save_best, bool):
            ...
            if save_best is True and key_indicator is None:
                raise ValueError("key_indicator should not be None, when " "save_best is set to True.")
            save_best = key_indicator

        super().__init__(
            dataloader, start, interval, by_epoch, save_best, rule, test_fn, greater_keys, less_keys, **eval_kwargs
        )
