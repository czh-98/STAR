...
class SmoothNet(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(num_blocks):
            res_blocks.append(
                SmoothNetResBlock(in_channels=hidden_size, hidden_channels=res_hidden_size, dropout=dropout)
            )
...
class SmoothNetFilter:
    ...
    def __init__(
        ...
        self.window_size = window_size
        self.device = device
        self.smoothnet = SmoothNet(window_size, output_size, hidden_size, res_hidden_size, num_blocks)
    ...
    def __call__(self, x: np.ndarray):
        ...
        assert x.ndim == 3, "Input should be an array with shape [T, K, C]" f", but got invalid shape {x.shape}"
        ...
        if T < self.window_size:
            ...
            if input_type == "matrix":
                smoothed = rot6d_to_rotmat(smoothed.reshape(-1, 6)).reshape(T, K, C)
            elif input_type == "axis_angles":
                smoothed = rotmat_to_aa(rot6d_to_rotmat(smoothed.reshape(-1, 6))).reshape(T, K, C)
