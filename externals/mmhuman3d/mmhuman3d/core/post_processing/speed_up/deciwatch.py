...
class DeciWatchPostProcessing:
    ...
    def __call__(self, x=None):
        ...
        assert x.shape[1:] == (24, 3, 3) or x.shape[1:] == (self.input_dimension) or x.shape[1:] == (24, 3)
        ...
        if x.shape[1:] == (24, 3, 3):
            ...
            x = torch.tensor(x).to(self.device)
            x = rotmat_to_rot6d(aa_to_rotmat(x.reshape(-1, 3))).reshape(-1, self.input_dimension)
        else:
            x = torch.tensor(x).to(self.device)
        ...
        slide_window_x = torch.as_strided(
            input,
            ((seq_len - self.slide_window_size) // (self.interval) + 1, self.slide_window_size, self.input_dimension),
            (self.interval * self.input_dimension, self.input_dimension, 1),
            storage_offset=0,
        ).reshape(-1, self.slide_window_size, self.input_dimension)

        smoothed_len = (seq_len - self.slide_window_size) // self.interval * self.interval + self.slide_window_size
        ...
        for i in range(smooth_poses.shape[0]):
            for j in range(self.slide_window_size):
                output_poses[i * self.interval + j].append(smooth_poses[i, j, :])

        smooth_poses = torch.cat(
            (
                smooth_poses[:, : self.slide_window_size - 1, :].reshape(-1, self.input_dimension),
                smooth_poses[-1, -1, :].reshape(-1, self.input_dimension),
            ),
            dim=0,
        )
        ...
        if smoothed_len < seq_len:
            output_poses = torch.cat((output_poses, x[smoothed_len:, :]), dim=0)

        if input_type == "matrix":
            output_poses = rot6d_to_rotmat(output_poses.reshape(-1, 6)).reshape(-1, 24, 3, 3)
        elif input_type == "axis_angles":
            output_poses = rotmat_to_aa(rot6d_to_rotmat(output_poses.reshape(-1, 6))).reshape(-1, 24, 3)
...
class PositionEmbeddingSine_1D(nn.Module):
    ...
    def forward(self, B, L):
        ...
        dim_t = self.temperature ** (2 * (torch.div(dim_t, 1)) / self.num_pos_feats)
...
class DeciWatchTransformer(nn.Module):
    ...
    def __init__(
        ...
        encoder_layer = DeciWatchTransformerEncoderLayer(
            encoder_hidden_dim, nhead, dim_feedforward, dropout, activation, pre_norm
        )
        encoder_norm = nn.LayerNorm(encoder_hidden_dim) if pre_norm else None
        self.encoder = DeciWatchTransformerEncoder(encoder_layer, num_encoder_layers, encoder_norm)

        decoder_layer = DeciWatchTransformerDecoderLayer(
            decoder_hidden_dim, nhead, dim_feedforward, dropout, activation, pre_norm
        )
        decoder_norm = nn.LayerNorm(decoder_hidden_dim)
        self.decoder = DeciWatchTransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm)
...
class DeciWatchTransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, encoder_hidden_dim, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", pre_norm=False):
        super().__init__()
    ...
    def forward_post(
        ...
        src2 = self.self_attn(q, k, value=src, attn_mask=src_mask, key_padding_mask=src_key_padding_mask.bool())[0]
...
class DeciWatchTransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, decoder_hidden_dim, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", pre_norm=False):
        super().__init__()
    ...
    def forward_post(
        ...
        tgt2 = self.self_attn(q, k, value=tgt, attn_mask=tgt_mask, key_padding_mask=tgt_key_padding_mask.bool())[0]
        ...
        tgt2 = self.multihead_attn(
            ...
            key_padding_mask=memory_key_padding_mask.bool(),
        )[0]
