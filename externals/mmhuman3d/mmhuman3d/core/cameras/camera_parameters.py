...
class CameraParameter:
    ...
    def __init__(self, name: str = "default", H: int = 1080, W: int = 1920) -> None:
        """
        Args:
            name (str, optional):
    ...
    def set_KRT(self, K_mat: np.ndarray, R_mat: np.ndarray, T_vec: np.ndarray, inverse_extrinsic: bool = False) -> None:
    ...
    def load_from_dict(self, json_dict: dict) -> None:
        ...
        for key in json_dict.keys():
            if key == "name":
                ...
                self.parameters_dict["rotation_mat"] = np.array(json_dict[key]).reshape(3, 3).tolist()
            elif key == "translation":
                self.parameters_dict[key] = np.array(json_dict[key]).reshape((3)).tolist()
            else:
                self.parameters_dict[key] = json_dict[key]
                if "_mat" in key:
                    self.parameters_dict[key] = np.array(self.parameters_dict[key]).reshape(3, 3).tolist()

    def load_from_chessboard(self, chessboard_dict: dict, name: str, inverse: bool = True) -> None:
        """Load name and parameters from a dict.
        ...
        Args:
            ...
            inverse (bool, optional):
                ...
                Defaults to False.
        """
        camera_param_dict = __parse_chessboard_param__(chessboard_dict, name, inverse=inverse)
    ...
    def load_kinect_from_smc(self, smc_reader, kinect_id: int) -> None:
        ...
        """
        name = kinect_id
        extrinsics_dict = smc_reader.get_kinect_color_extrinsics(kinect_id, homogeneous=False)
        rot_np = extrinsics_dict["R"]
        trans_np = extrinsics_dict["T"]
        intrinsics_np = smc_reader.get_kinect_color_intrinsics(kinect_id)
        resolution = smc_reader.get_kinect_color_resolution(kinect_id)
    ...
    def load_iphone_from_smc(self, smc_reader, iphone_id: int = 0, frame_id: int = 0) -> None:
        ...
        extrinsics_mat = smc_reader.get_iphone_extrinsics(iphone_id, homogeneous=True)
        rot_np = extrinsics_mat[:3, :3]
        trans_np = extrinsics_mat[:3, 3]
        intrinsics_np = smc_reader.get_iphone_intrinsics(iphone_id, frame_id)
        resolution = smc_reader.get_iphone_color_resolution(iphone_id)
        rmatrix = np.linalg.inv(rot_np).reshape(3, 3)
        tvec = -np.dot(rmatrix, trans_np)
        self.name = name
        self.set_mat_np("in_mat", intrinsics_np)
    ...
    @classmethod
    def load_from_perspective_cameras(cls, cam, name: str, resolution: Union[List, Tuple] = None):
        ...
        Args:
            ...
            name (str):
                Name of this camera.
        """
        assert isinstance(cam, PerspectiveCameras), "Wrong input, support PerspectiveCameras only!"
        if len(cam) > 1:
            warnings.warn("Will only use the first camera in the batch.")
        cam = cam[0]

        resolution = resolution if resolution is not None else cam.resolution[0].tolist()
        ...
        k_3x3 = convert_K_4x4_to_3x3(k_4x4, is_perspective=is_perspective)
    ...
    def __validate_value_type__(self, key: Any, val: Any) -> Any:
        ...
        ret_val = None
        if supported_keys[key]["type"] == int or supported_keys[key]["type"] == float:
            ...
            if type(val) == self.__class__.SUPPORTED_KEYS[key]["type"]:
                ...
                if np.issubdtype(type(val), np_type_mapping[supported_keys[key]["type"]]):
                    validation_result = _TypeValidation.MATCH
        ...
        if validation_result != _TypeValidation.MATCH:
            ...
            if validation_result == _TypeValidation.ARRAY:
                err_msg += "A single value is expected, " + "neither an array nor a slice.\n"
...
def __parse_chessboard_param__(chessboard_camera_param, name, inverse=True):
    ...
    if inverse:
        rmatrix = np.linalg.inv(np.array(camera_param_dict["rotation"]).reshape(3, 3))
...
__distort_coefficient_names__ = ["k1", "k2", "k3", "k4", "k5", "k6", "p1", "p2"]
