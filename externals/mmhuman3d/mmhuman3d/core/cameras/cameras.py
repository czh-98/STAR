...
class MMCamerasBase(cameras.CamerasBase):
    ...
    def compute_depth_of_points(self, points: torch.Tensor) -> torch.Tensor:
        ...
        world_to_view_points = world_to_view_transform.transform_points(points.to(self.device))
    ...
    def compute_normal_of_meshes(self, meshes: Meshes) -> torch.Tensor:
        ...
        """
        world_to_view_transform = self.get_world_to_view_transform()
        world_to_view_normals = world_to_view_transform.transform_normals(meshes.verts_normals_padded().to(self.device))
    ...
    def __getitem__(self, index: Union[slice, int, torch.Tensor, List, Tuple]) -> "MMCamerasBase":
        ...
        return self.__class__(
            ...
            R=self.R[index],
            T=self.T[index],
            image_size=self.get_image_size()[index] if self.get_image_size() is not None else None,
            in_ndc=self.in_ndc(),
    ...
    @classmethod
    def get_default_projection_matrix(
        cls,
    ):
    ...
    def to_screen_(self, **kwargs) -> "MMCamerasBase":
        ...
        if self.in_ndc():
            ...
            self.K = convert_ndc_to_screen(K=self.K, resolution=self.image_size, is_perspective=self._is_perspective)
            self._in_ndc = False
        else:
    ...
    def to_ndc_(self, **kwargs) -> "MMCamerasBase":
        ...
        if self.in_ndc():
            ...
            if self.get_image_size() is None:
                ...
                self.image_size = self.get_image_size()
            self.K = convert_screen_to_ndc(K=self.K, resolution=self.image_size, is_perspective=self._is_perspective)
            self._in_ndc = True
    ...
    def to_screen(self, **kwargs) -> "MMCamerasBase":
        ...
        if self.in_ndc():
            ...
            K = convert_ndc_to_screen(K=self.K, resolution=self.image_size, is_perspective=self._is_perspective)
            return self.__class__(K=K, R=self.R, T=self.T, in_ndc=False, resolution=self.image_size)
        else:
            print("Redundant operation, already in screen.")
    ...
    def to_ndc(self, **kwargs) -> "MMCamerasBase":
        ...
        if self.in_ndc():
            ...
            if self.get_image_size() is None:
                ...
                self.image_size = self.get_image_size()
            K = convert_screen_to_ndc(K=self.K, resolution=self.image_size, is_perspective=self._is_perspective)
            return self.__class__(K=K, R=self.R, T=self.T, in_ndc=True, resolution=self.image_size)

    def detach(self) -> "MMCamerasBase":
        image_size = self.image_size.detach() if self.image_size is not None else None
        return self.__class__(
            K=self.K.detach(),
            R=self.R.detach(),
            T=self.T.detach(),
...
@CAMERAS.register_module(name=("WeakPerspectiveCameras", "WeakPerspective", "weakperspective"))
class WeakPerspectiveCameras(MMCamerasBase):
    ...
    @staticmethod
    def convert_orig_cam_to_matrix(orig_cam: torch.Tensor, **kwargs) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        _N = orig_cam.shape[0]
        scale_x, scale_y, transl_x, transl_y = orig_cam[:, 0], orig_cam[:, 1], orig_cam[:, 2], orig_cam[:, 3]
    ...
    def compute_projection_matrix(self, scale_x, scale_y, transl_x, transl_y, aspect_ratio) -> torch.Tensor:
...
@CAMERAS.register_module(name=("PerspectiveCameras", "perspective", "Perspective"))
class PerspectiveCameras(cameras.PerspectiveCameras, MMCamerasBase):
    ...
    def get_default_projection_matrix(cls, **args) -> torch.Tensor:
        ...
        return cameras._get_sfm_calibration_matrix(
            N=batch_size, device=device, focal_length=focal_length, principal_point=principal_point, orthographic=False
        )
...
@CAMERAS.register_module(name=("FoVPerspectiveCameras", "FoVPerspective", "fovperspective"))
...
@CAMERAS.register_module(name=("OrthographicCameras", "Orthographic", "orthographic"))
class OrthographicCameras(cameras.OrthographicCameras, MMCamerasBase):
    ...
    def get_default_projection_matrix(cls, **args) -> torch.Tensor:
        ...
        return cameras._get_sfm_calibration_matrix(
            N=batch_size, device=device, focal_length=focal_length, principal_point=principal_point, orthographic=True
        )


@CAMERAS.register_module(name=("FoVOrthographicCameras", "FoVOrthographic", "fovorthographic"))
class FoVOrthographicCameras(cameras.FoVOrthographicCameras, MMCamerasBase):
    ...
    def __init__(
        ...
        scale_xyz: Union[Iterable[float], Iterable[int]] = ((1.0, 1.0, 1.0),),  # (1, 3)
        R: Optional[torch.Tensor] = None,
        T: Optional[torch.Tensor] = None,
        K: Optional[torch.Tensor] = None,
        device: Union[torch.device, str] = "cpu",
...
def concat_cameras(cameras_list: List[MMCamerasBase]) -> MMCamerasBase:
    ...
    concated_cameras = cam_cls(
        K=K, R=R, T=T, device=device, is_perspective=is_perspective, in_ndc=in_ndc, image_size=image_size
    )
...
def compute_orbit_cameras(
    ...
    if batch_size > 1 and orbit_speed[0] != 0:
        azim = torch.linspace(azim, azim + batch_size * orbit_speed[0], batch_size)
    if batch_size > 1 and orbit_speed[1] != 0:
        elev = torch.linspace(elev, elev + batch_size * orbit_speed[1], batch_size)
    if batch_size > 1 and dist_speed != 0:
        dist = torch.linspace(dist, dist + batch_size * dist_speed, batch_size)

    if convention == "opencv":
        rotation_compensate = ee_to_rotmat(torch.Tensor([math.pi, 0, 0]).view(1, 3))
        at = rotation_compensate.permute(0, 2, 1) @ at.view(-1, 3, 1)
        at = at.view(1, 3)
    R, T = cameras.look_at_view_transform(dist=dist, elev=elev, azim=azim, at=at)
    if K is None:
        K = FoVPerspectiveCameras.get_default_projection_matrix(batch_size=batch_size)
    if convention == "opencv":
        rotation_compensate = ee_to_rotmat(torch.Tensor([math.pi, 0, 0]).view(1, 3))
...
def compute_direction_cameras(
    ...
    y_vec = y_vec - torch.bmm(y_vec.view(-1, 1, 3), z_vec.view(-1, 3, 1)).view(-1, 1) * z_vec
    y_vec = norm_vec(y_vec)
    x_vec = torch.cross(y_vec, z_vec)
    R = torch.cat([x_vec.view(-1, 3, 1), y_vec.view(-1, 3, 1), z_vec.view(-1, 3, 1)], 1).view(-1, 3, 3)
    T = eye
    ...
    if K is None:
        K = FoVPerspectiveCameras.get_default_projection_matrix(batch_size=batch_size)
    K, R, T = convert_camera_matrix(
        K=K, R=R, T=T, is_perspective=True, convention_src="pytorch3d", convention_dst=convention
    )
    return K, R, T
