...
class VedoRenderer(object):
    ...
    def __init_axis(self, axis_len=80):
        ...
        for axis_index in range(3):
            ret_list.append(vedo.Arrows([[0, 0, 0]], [arrow_end_np[axis_index]]).c(colors[axis_index]))
        return ret_list
    ...
    def set_y_reverse(self):
        ...
        """
        self.y_reverse = True
        self.y_reverse_rotation = scipy_Rotation.from_euler("z", 180, degrees=True)
    ...
    def add_camera(self, camera_parameter, arrow_len=30):
        ...
        if self.y_reverse:
            ...
            for axis_index in range(3):
                arrow_end_camera[axis_index, :] = self.y_reverse_rotation.apply(arrow_end_camera[axis_index, :])
        vedo_list = []
        for i in range(3):
            vedo_list.append(vedo.Arrows([cam_center], [cam_center + arrow_end_camera[i]]).c(colors[i]))
        vedo_list.append(vedo.Text3D(camera_parameter.name, cam_center, s=self.scale * 10))
