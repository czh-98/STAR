...
from mmhuman3d.core.conventions.cameras.convert_convention import enc_camera_convention  # prevent yapf isort conflict
...
class Axes3dBaseRenderer(object):
    ...
    def init_camera(self, cam_elev_angle=10, cam_elev_speed=0.0, cam_hori_angle=45, cam_hori_speed=0.5):
    ...
    def _draw_scene(self, visual_range, axis_len=1.0, cam_elev_angle=10, cam_hori_angle=45):
        ...
        y_axis = np.array([0, (visual_range[1][1] - mid_point[1]) * axis_len, 0])
        z_axis = np.array([0, 0, (visual_range[2][1] - mid_point[2]) * axis_len])
    ...
    def render_kp3d_to_video(
        ...
        if output_path is not None:
            if check_path_suffix(output_path, [".mp4", ".gif"]):
                self.temp_path = os.path.join(Path(output_path).parent, Path(output_path).name + "_output_temp")
        ...
        if not self.if_frame_updated:
            ...
            image_array = self._export_frames(
                keypoints_np, resolution, visual_range, frame_names, disable_limbs, return_array
            )
            self.if_frame_updated = True

        if output_path is not None:
            if check_path_suffix(output_path, ".mp4"):
                images_to_video(self.temp_path, output_path, img_format="frame_%06d.png", fps=fps)
        return image_array

    def _export_frames(self, keypoints_np, resolution, visual_range, frame_names, disable_limbs, return_array):
        ...
        for frame_index in range(keypoints_np.shape[0]):
            ...
            fig, ax = self._draw_scene(
                visual_range=visual_range, axis_len=0.5, cam_elev_angle=cam_ele, cam_hori_angle=cam_hor
            )
            #  draw limbs
            ...
            for person_index, keypoints_person in enumerate(keypoints_frame):
                ...
                scatter_points_index = list(set(np.array(self.limbs_connection["body"]).reshape(-1).tolist()))
                ax.scatter(
            ...
            if frame_names is not None:
                cv2.putText(
                    resized_mat,
                    str(frame_names[frame_index]),
                    (resolution[0] // 10, resolution[1] // 10),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.5 * resolution[0] / 500,
...
def _set_new_pose(pose_np, sign, axis):
    ...
    for dim_index in range(pose_rearrange_axis_result.shape[-1]):
        pose_rearrange_axis_result[..., dim_index] = (
            sign[dim_index] / target_sign[dim_index] * pose_rearrange_axis_result[..., dim_index]
        )
    return pose_rearrange_axis_result


def _plot_line_on_fig(ax, point1_location, point2_location, color, linewidth=1):
    """Draw line on fig with matplotlib."""
    ax.plot(
        [point1_location[0], point2_location[0]],
