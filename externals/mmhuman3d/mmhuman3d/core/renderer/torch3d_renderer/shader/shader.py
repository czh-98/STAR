...
class SilhouetteShader(SoftSilhouetteShader):
    ...
    def __init__(self, blend_params: Optional[BlendParams] = None, **kwargs) -> None:
...
class DepthShader(nn.Module):
    ...
    def forward(self, fragments, meshes, cameras, **kwargs) -> torch.Tensor:
        ...
        depth_map = interpolate_face_attributes(
            pix_to_face=fragments.pix_to_face, barycentric_coords=fragments.bary_coords, face_attributes=faces_depth
        )
        return depth_map[..., 0, :]
...
class NormalShader(nn.Module):
    ...
    def forward(self, fragments, meshes, cameras, **kwargs) -> torch.Tensor:
        ...
        normal_map = interpolate_face_attributes(
            pix_to_face=fragments.pix_to_face, barycentric_coords=fragments.bary_coords, face_attributes=faces_normal
        )
        return normal_map[..., 0, :]
...
class SegmentationShader(nn.Module):
    ...
    def forward(self, fragments, meshes, **kwargs) -> torch.Tensor:
        ...
        segmentation_map = interpolate_face_attributes(
            pix_to_face=fragments.pix_to_face, barycentric_coords=fragments.bary_coords, face_attributes=faces_class
        ).long()
