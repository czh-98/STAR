...
class SMPLRenderer(BaseRenderer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.image_renderer = build_renderer(dict(device=device, resolution=resolution, **kwargs))

        if plot_kps:
            self.alpha = max(min(0.8, alpha), 0.1)
            self.joints_renderer = build_renderer(
                dict(type="pointcloud", resolution=resolution, device=device, radius=0.008)
            )
        else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        if isinstance(lights, DirectionalLights):
            ...
            lights = lights.clone()
            lights.location = -cameras.get_camera_plane_normals() - cameras.get_camera_center()

        rendered_tensor = self.image_renderer(meshes=meshes, cameras=cameras, lights=lights, indexes=indexes)
        ...
        images = (
            normalize(images, origin_value_range=[0, 255], out_value_range=[0, 1], dtype=torch.float32)
            if images is not None
            else None
        )
        ...
        if self.output_path is not None:
            ...
            if images is not None:
                output_images = (
                    bgrs * valid_masks * self.alpha
                    + images * valid_masks * (1 - self.alpha)
            ...
            if self.plot_kps:
                ...
                joints_2d = cameras.transform_points_screen(joints, image_size=self.resolution)[..., :2]
                if joints_gt is None:
                    joints_padded = joints
                    num_joints = joints_padded.shape[1]
                    joints_rgb_padded = torch.ones(num_frames, num_joints, 4) * (
                    ...
                    joints_rgb_gt = torch.ones(num_frames, num_joints_gt, 4) * (
                        torch.tensor([1.0, 0.0, 0.0, 1.0]).view(1, 1, 4)
                    )
                    joints_rgb_padded = torch.cat([joints_rgb, joints_rgb_gt], dim=1)

                pointcloud_images = self.joints_renderer(
                    vertices=joints_padded, verts_rgba=joints_rgb_padded.to(self.device), cameras=cameras
                )

                pointcloud_rgb = pointcloud_images[..., :3]
                pointcloud_bgr = rgb2bgr(pointcloud_rgb)
                pointcloud_mask = (pointcloud_images[..., 3:] > 0) * 1.0
                output_images = output_images * (1 - pointcloud_mask) + pointcloud_mask * pointcloud_bgr
            ...
            for frame_idx, real_idx in enumerate(indexes):
                ...
                cv2.imwrite(osp.join(folder, self.out_img_format % real_idx), im)
        ...
        if self.return_tensor:
            ...
            if self.final_resolution != self.resolution:
                rendered_map = interpolate(rendered_map, size=self.final_resolution, mode="bilinear")
            return rendered_map
        else:
            return None
