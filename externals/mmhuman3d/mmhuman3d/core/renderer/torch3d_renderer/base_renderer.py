...
class BaseRenderer(nn.Module):
    ...
    def _init_renderer(
        ...
        lights: Union[dict, DirectionalLights, PointLights, AmbientLights] = None,
        ...
        **kwargs,
        ...
        elif isinstance(lights, (AmbientLights, PointLights, DirectionalLights)):
            self.lights = lights
        else:
        ...
        if isinstance(blend_params, dict):
            ...
            raise TypeError(f"Wrong type of blend_params: {type(blend_params)}.")
        ...
        if isinstance(rasterizer, nn.Module):
            ...
            self.rasterizer = MeshRasterizer(
                raster_settings=RasterizationSettings(
                    image_size=self.resolution, bin_size=0, blur_radius=0, faces_per_pixel=1, perspective_correct=False
                )
            )
        else:
            raise TypeError(f"Wrong type of rasterizer: {type(self.rasterizer)}.")

        if self.resolution is None:
            self.resolution = self.rasterizer.raster_settings.image_size
        assert self.resolution is not None
        self.resolution = (
            (self.resolution, self.resolution) if isinstance(self.resolution, int) else tuple(self.resolution)
        )
        if isinstance(shader, nn.Module):
            self.shader = shader
        elif isinstance(shader, dict):
            shader.update(materials=materials, lights=self.lights, blend_params=blend_params)
            self.shader = build_shader(shader)
        elif shader is None:
            self.shader = build_shader(
                dict(type=self.shader_type, materials=materials, lights=self.lights, blend_params=blend_params)
            )
        else:
    ...
    def _set_output_path(self, output_path):
        if output_path is not None:
            ...
            if check_path_suffix(output_path, [".mp4", ".gif"]):
                self.temp_path = osp.join(Path(output_path).parent, Path(output_path).name + "_output_temp")
            else:
    ...
    def _update_resolution(self, cameras, **kwargs):
        if isinstance(cameras, MMCamerasBase):
            self.resolution = (int(cameras.resolution[0][0]), int(cameras.resolution[0][1]))
    ...
    def export(self):
        ...
        if self.output_path is not None:
            folder = self.temp_path if self.temp_path is not None else self.output_path
            if check_path_suffix(self.output_path, [".mp4"]):
                images_to_video(input_folder=folder, output_path=self.output_path, img_format=self.out_img_format)
            elif check_path_suffix(self.output_path, [".gif"]):
                images_to_gif(input_folder=folder, output_path=self.output_path, img_format=self.out_img_format)
    ...
    def _write_images(self, rgba, backgrounds, indexes):
        ...
        if backgrounds is not None:
            ...
            backgrounds = normalize(backgrounds, origin_value_range=(0, image_max), out_value_range=(0, 1))
            output_images = bgrs * valid_masks + (1 - valid_masks) * backgrounds
        ...
        for idx, real_idx in enumerate(indexes):
            folder = self.temp_path if self.temp_path is not None else self.output_path
            cv2.imwrite(osp.join(folder, self.out_img_format % real_idx), output_images[idx])
    ...
    def tensor2rgba(self, tensor: torch.Tensor):
        ...
        rgbs = normalize(rgbs, origin_value_range=[0, 1], out_value_range=[0, 1])
