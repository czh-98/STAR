...
def render(
    ...
    **forward_params
    ...
    if isinstance(renderer, dict):
        ...
        if isinstance(renderer.shader, SoftSilhouetteShader):
            renderer = build_renderer(
                dict(type="silhouette", resolution=resolution, shader=renderer.shader, rasterizer=renderer.rasterizer)
            )
        else:
            renderer = build_renderer(
                dict(type="mesh", resolution=resolution, shader=renderer.shader, rasterizer=renderer.rasterizer)
            )
    ...
    if isinstance(cameras, dict):
        ...
        cameras = cameras
    elif isinstance(cameras, CamerasBase) and not isinstance(cameras, MMCamerasBase):
    ...
    for k in forward_params:
        if isinstance(forward_params[k], np.ndarray):
            forward_params.update({k: torch.tensor(forward_params[k]).to(device)})
    if verbose:
        iter_func = trange
    else:
        iter_func = range
    for i in iter_func(math.ceil(num_frames // batch_size)):
        indexes = list(range(i * batch_size, min((i + 1) * batch_size, len(meshes))))
        foward_params_batch = {}
