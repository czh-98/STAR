...
class UVRenderer(nn.Module):
    ...
    def __init__(
        ...
        if uv_param_path is not None:
            ...
            self.faces_uv = torch.LongTensor(param_dict["faces_uv"]).to(self.device)

            self.NUM_VT = self.verts_uv.shape[0]

            self.faces_tensor = torch.LongTensor(param_dict["faces"].astype(np.int64)).to(self.device)
            ...
            self.faces_uv = mesh_template.textures.faces_uvs_padded()[0].to(self.device)
            self.verts_uv = mesh_template.textures.verts_uvs_padded()[0].to(self.device)
            self.NUM_VT = self.verts_uv.shape[0]
    ...
    def update_fragments(self):
        ...
        rasterizer = MeshRasterizer(
            cameras=FoVOrthographicCameras(min_x=1, max_x=0, max_y=1, min_y=0, device=self.device),
        ...
        verts_uv = torch.cat([self.verts_uv[None], torch.ones(1, self.NUM_VT, 1).to(self.device)], -1)

        fragments = rasterizer(Meshes(verts=verts_uv, faces=self.faces_uv[None]))
    ...
    def update_face_uv_pixel(self):
        ...
        verts_uv_pixel = (
            cameras.transform_points_screen(verts_uv, image_size=self.resolution).round().long()[0, ..., :2]
        )
        verts_uv_pixel[..., 0] = torch.clip(verts_uv_pixel[..., 0], min=0, max=W - 1)
        verts_uv_pixel[..., 1] = torch.clip(verts_uv_pixel[..., 1], min=0, max=H - 1)
        verts_uv_pixel = verts_uv_pixel.long()
        mask = self.mask

        wrong_indexes = torch.where(mask[verts_uv_pixel.view(-1, 2)[:, 1], verts_uv_pixel.view(-1, 2)[:, 0]] == 0)[0]
        for wrong_index in wrong_indexes:
            ...
            if len(faces_xy) > 0:
                ...
                min_arg = torch.argmin(torch.sqrt(((faces_xy - vert_xy) * (faces_xy - vert_xy)).sum(-1).float()))
        ...
        left_bound = torch.cat([left_bound, torch.zeros(H, 1).to(device)], 1).unsqueeze(-1)
        right_bound = torch.cat([torch.zeros(H, 1).to(device), right_bound], 1).unsqueeze(-1)
        up_bound = torch.cat([up_bound, torch.zeros(1, W).to(device)], 0).unsqueeze(-1)
        bottom_bound = torch.cat([torch.zeros(1, W).to(device), bottom_bound], 0).unsqueeze(-1)
        ...
        verts_uv_pixel = verts_uv_pixel + stride_uv_mask[
            verts_uv_pixel.view(-1, 2)[:, 1], verts_uv_pixel.view(-1, 2)[:, 0]
        ].view(self.NUM_VT, 2)
    ...
    def forward(self, verts_attr: Optional[torch.Tensor], resolution: Optional[Iterable[int]] = None) -> torch.Tensor:
        ...
        faces_packed = self.faces_tensor[None].repeat(N, 1, 1) + offset_idx.view(-1, 1, 1).to(self.device)
        faces_packed = faces_packed.view(-1, 3)
        face_attr = verts_attr[faces_packed]
        assert face_attr.shape == (N * self.num_faces, 3, C)
        pix_to_face = self.pix_to_face.unsqueeze(0).repeat(N, 1, 1).unsqueeze(-1)
    ...
    def forward_normal_map(
        ...
        if meshes is not None:
            ...
            meshes = Meshes(verts=vertices, faces=self.faces_tensor[None].repeat(vertices.shape[0], 1, 1))
            verts_normals = meshes.verts_normals_padded()
        else:
            raise ValueError("No valid input.")
        verts_normals = meshes.verts_normals_padded()
        if cameras:
            verts_normals = cameras.get_world_to_view_transform().transform_normals(verts_normals)
        normal_map = self.forward(verts_attr=verts_normals, resolution=resolution)
        return normal_map
    ...
    def forward_uvd_map(
        ...
        if vertices is not None:
            ...
            raise ValueError("No valid input.")
        if cameras:
            verts_uvd = cameras.get_world_to_view_transform().transform_normals(verts_uvd)
    ...
    def vertex_resample(
        ...
        verts_feature_packed = torch.zeros(N * self.NUM_VERTS, C).to(self.device)

        face_uv_pixel = self.face_uv_pixel.view(-1, 2)
        verts_feature_packed[faces_packed] = maps_padded[:, face_uv_pixel[:, 1], face_uv_pixel[:, 0]].view(
    ...
    def wrap_texture(
        ...
        if resolution != (H, W):
            texture_map_padded = F.interpolate(texture_map_padded.view(0, 3, 1, 2), resolution, mode=mode).view(
                0, 2, 3, 1
            )
        ...
        textures = TexturesUV(faces_uvs=faces_uvs, verts_uvs=verts_uvs, maps=texture_map_padded)
        return textures
