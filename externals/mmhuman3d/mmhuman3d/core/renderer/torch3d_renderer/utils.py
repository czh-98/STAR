...
def normalize(
    value, origin_value_range=None, out_value_range=(0, 1), dtype=None, clip=False
) -> Union[torch.Tensor, np.ndarray]:
    """Normalize the tensor or array and convert dtype."""
    if origin_value_range is not None:
        value = (value - origin_value_range[0]) / (origin_value_range[1] - origin_value_range[0] + 1e-9)

    else:
        value = (value - value.min()) / (value.max() - value.min())
    value = value * (out_value_range[1] - out_value_range[0]) + out_value_range[0]
    if clip:
        value = torch.clip(value, min=out_value_range[0], max=out_value_range[1])
...
def array2tensor(image: np.ndarray) -> torch.Tensor:
    ...
    image = normalize(image, origin_value_range=(0, 255), out_value_range=(0, 1), dtype=torch.float32)
    return image
...
def align_input_to_padded(
    tensor=Union[List[torch.Tensor], torch.Tensor],
    ndim: int = 3,
    batch_size: int = None,
    padding_mode: Literal["ones", "zeros", "repeat", "none"] = "none",
):
    ...
    if batch_size is not None:
        ...
        if current_batch_size == 1:
            ...
            if padding_mode == "ones":
                tensor = torch.cat(
                    [tensor, torch.ones_like(tensor)[:1].repeat_interleave(batch_size - current_batch_size, 0)]
                )
            elif padding_mode == "ones":
                tensor = torch.cat(
                    [tensor, torch.zeros_like(tensor)[:1].repeat_interleave(batch_size - current_batch_size, 0)]
                )
            elif padding_mode == "repeat":
                tensor = tensor.repeat_interleave(batch_size // current_batch_size + 1, 0)[:batch_size]
            else:
                raise ValueError("Wrong batch_size to allocate," " please specify padding mode.")
        elif current_batch_size > batch_size:
