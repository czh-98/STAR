...
class DepthRenderer(BaseRenderer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        super().__init__(
            resolution=resolution, device=device, output_path=output_path, out_img_format=out_img_format, **kwargs
        )
        self.depth_max = depth_max

    def _init_renderer(self, rasterizer=None, shader=None, materials=None, lights=None, blend_params=None, **kwargs):
        shader = build_shader(dict(type="DepthShader")) if shader is None else shader
        return super()._init_renderer(rasterizer, shader, materials, lights, blend_params, **kwargs)
    ...
    def forward(
        ...
        **kwargs,
        ...
        depth_map = self.shader(fragments=fragments, meshes=meshes, cameras=cameras)
    ...
    def tensor2rgba(self, tensor: torch.Tensor):
        ...
        rgbs = normalize(rgbs, origin_value_range=(0, depth_max), out_value_range=(0, 1))
