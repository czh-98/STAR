...
class TexturesNearest(TexturesVertex):
    ...
    def sample_textures(self, fragments, faces_packed=None) -> torch.Tensor:
        ...
        mask = torch.arange(bary_coords.size(-1)).reshape(1, 1, -1).to(self.device) == idx.unsqueeze(-1)
        bary_coords *= 0
        bary_coords[mask] = 1
        texels = interpolate_face_attributes(fragments.pix_to_face, bary_coords, faces_verts_features)
        return texels
