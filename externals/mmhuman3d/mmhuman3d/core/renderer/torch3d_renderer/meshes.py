...
class ParametricMeshes(Meshes):
    ...
    def __init__(
        ...
        vertex_color: Union[Iterable[float], torch.Tensor, np.ndarray] = ((1, 1, 1),),
        use_nearest: bool = False,
        texture_images: Union[torch.Tensor, List[torch.Tensor], None] = None,
        model_type: str = "smpl",
        ...
        **pose_params,
        ...
        self.model_type = body_model._get_name().lower() if body_model is not None else model_type
        ...
        if N_individual_override is not None:
            verts = verts.view(-1, self.model_class.NUM_VERTS * N_individual_override, 3)
        ...
        verts = verts.view(-1, self.model_class.NUM_VERTS * self._N_individual, 3)
        device = verts.device
        N, V, _ = verts.shape
        ...
        if isinstance(faces, list):
            ...
            self.face_individual = faces[0][: self.model_class.NUM_FACES].to(device)
        elif faces is None:
            assert body_model is not None
            ...
            self.face_individual = faces[:1, : self.model_class.NUM_FACES].to(device)
        else:
            raise ValueError(f"Wrong type of faces: {type(faces)}.")

        assert faces.shape == (N, self.model_class.NUM_FACES * self._N_individual, 3)
        F = faces.shape[1]
        if textures is None:
            if texture_images is None:
                ...
                vertex_color = align_input_to_padded(vertex_color, ndim=3, batch_size=N)
                assert vertex_color.shape == verts.shape
                if use_nearest:
                    textures = TexturesNearest(verts_features=vertex_color).to(device)
                else:
                    textures = TexturesVertex(verts_features=vertex_color).to(device)
            else:

                texture_images = align_input_to_padded(texture_images, ndim=4, batch_size=N).to(device)
    ...
    def get_faces_padded(self, N_batch, N_individual):
        ...
        faces_offset = (
            torch.arange(N_individual)
            .view(N_individual, 1)
            .repeat(1, self.model_class.NUM_FACES)
            .view(1, -1, 1)
            .to(faces.device)
        )
    ...
    def extend(self, N_batch: int, N_scene: int = 1):
        ...
        if N_scene == 1:
            ...
            meshes = join_batch_meshes_as_scene([meshes_batch for _ in range(N_scene)])
        return meshes
    ...
    def clone(self):
        ...
        other = self.__class__(verts=new_verts_list, faces=new_faces_list, model_type=self.model_type)
    ...
    def update_padded(self, new_verts_padded: torch.Tensor):
        ...
        new = self.__class__(verts=new_verts_padded, faces=self.faces_padded(), model_type=self.model_type)
    ...
    def __getitem__(self, index: Union[tuple, int, list, slice, torch.Tensor]):
        ...
        batch_index = torch.tensor(batch_index) if not isinstance(batch_index, torch.Tensor) else batch_index
        ...
        if individual_index is None:
            return self.__class__(
                ...
                textures=self.textures[batch_index] if self.textures is not None else None,
        ...
        if isinstance(individual_index, int):
            ...
            individual_index = torch.arange(self._N_individual)[individual_index]
        individual_index = (
            torch.tensor(individual_index) if not isinstance(individual_index, torch.Tensor) else individual_index
        )
        if (individual_index > self._N_individual).any():
            raise IndexError("list index out of range")
        vertex_index = [
            torch.arange(self.model_class.NUM_VERTS) + idx * self.model_class.NUM_VERTS for idx in individual_index
        ]
        ...
        faces_padded = self.get_faces_padded(len(verts_padded), len(individual_index))
        ...
        if isinstance(textures_batch, TexturesUV):
            ...
            maps_index = [
                torch.arange(width_individual * idx, width_individual * (idx + 1)) for idx in individual_index
            ]
            maps_index = torch.cat(maps_index).to(self.device)
            verts_uvs_padded = textures_batch.verts_uvs_padded()[:, : len(vertex_index)] * torch.Tensor(
                [self._N_individual / len(individual_index), 1]
            ).view(1, 1, 2).to(self.device)
            faces_uvs_padded = textures_batch.faces_uvs_padded()[:, :new_face_num]
            maps_padded = maps[:, :, maps_index]
            textures = TexturesUV(faces_uvs=faces_uvs_padded, verts_uvs=verts_uvs_padded, maps=maps_padded)
        elif isinstance(textures_batch, (TexturesVertex, TexturesNearest)):
            verts_features_padded = textures_batch.verts_features_padded()[:, vertex_index]
            textures = textures_batch.__class__(verts_features_padded)
        meshes = self.__class__(verts=verts_padded, faces=faces_padded, textures=textures, model_type=self.model_type)
        return meshes
...
def join_meshes_as_batch(meshes: List[ParametricMeshes], include_textures: bool = True) -> ParametricMeshes:
    ...
    assert all(mesh.model_type == first.model_type for mesh in meshes), "model_type should all be the same."
...
def join_meshes_as_scene(
    meshes: Union[ParametricMeshes, List[ParametricMeshes]], include_textures: bool = True
) -> ParametricMeshes:
    ...
    mesh = ParametricMeshes(
        verts=verts.unsqueeze(0), faces=faces.unsqueeze(0), textures=textures, model_type=first.model_type
    )

    return mesh


def join_batch_meshes_as_scene(meshes: List[ParametricMeshes], include_textures: bool = True) -> ParametricMeshes:
    ...
    if not all(mesh.shape[1] == first.shape[1] for mesh in meshes):
        ...
        for mesh_scene in meshes:
            meshes_temp.extend(
                [mesh_scene[:, individual_index] for individual_index in range(mesh_scene._N_individual)]
            )
        meshes = meshes_temp
    for mesh in meshes:
        mesh._verts_list = padded_to_list(mesh.verts_padded(), mesh.num_verts_per_mesh().tolist())
