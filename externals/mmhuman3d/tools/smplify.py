...
def parse_args():
    ...
    parser.add_argument(
        "--input_type", choices=["keypoints2d", "keypoints3d"], default="keypoints3d", help="input type"
    )
    parser.add_argument("--J_regressor", type=str, default=None, help="the path of the J_regressor")
    parser.add_argument("--keypoint_type", default="human_data", help="the source type of input keypoints")
    ...
    parser.add_argument(
        "--use_one_betas_per_video", action="store_true", help="use one betas to keep shape consistent through a video"
    )
    parser.add_argument("--device", choices=["cpu", "cuda"], default="cuda", help="device used for smplify")
    parser.add_argument(
        "--gender", choices=["neutral", "male", "female"], default="neutral", help="gender of SMPL model"
    )
    parser.add_argument("--output", help="output result file")
    parser.add_argument("--show_path", help="directory to save rendered images or video")
    parser.add_argument(
        "--overwrite", action="store_true", help="Whether to overwrite if there is already a result file."
    )
...
def main():
    ...
    keypoints, mask = convert_kps(
        keypoints_src, mask=keypoints_src_mask, src=args.keypoint_type, dst=smplify_config.body_model["keypoint_dst"]
    )
    ...
    keypoints_conf = torch.tensor(keypoints_conf, dtype=torch.float32, device=device)
    ...
    if args.input_type == "keypoints3d":
        ...
        human_data = dict(keypoints2d=keypoints, keypoints2d_conf=keypoints_conf)
    else:
    ...
    if args.show_path is not None:
        ...
        body_model_config.update(model_path=body_model_dir, model_type=smplify_config.body_model.type.lower())
        visualize_smpl_pose(
            poses=poses, body_model_config=body_model_config, output_path=args.show_path, orbit_speed=1, overwrite=True
        )
