...
hrnet_extra = dict(
    ...
    stage2=dict(num_modules=1, num_branches=2, block="BASIC", num_blocks=(4, 4), num_channels=(width, width * 2)),
    stage3=dict(
        num_modules=4, num_branches=3, block="BASIC", num_blocks=(4, 4, 4), num_channels=(width, width * 2, width * 4)
    ),
    stage4=dict(
        num_modules=3,
        num_branches=4,
...
model = dict(
    ...
    init_cfg=dict(type="Pretrained", checkpoint=("data/pretrained_models/hrnet_w32_conv_pare_coco.pth")),
)
