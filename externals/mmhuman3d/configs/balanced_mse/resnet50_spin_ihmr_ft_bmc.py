...
# optimizer
optimizer = dict(
    backbone=dict(type="Adam", lr=0), head=dict(type="Adam", lr=1e-4), loss_smpl_pose=dict(type="Adam", lr=1e-2)
)
...
registrant = dict(
    ...
    joint_prior_loss=dict(type="JointPriorLoss", loss_weight=15.2**2, reduction="sum"),
    pose_prior_loss=dict(
        type="MaxMixturePrior", prior_folder="data", num_gaussians=8, loss_weight=4.78**2, reduction="sum"
    ),
...
model = dict(
    ...
    head=dict(type="HMRHead", feat_dim=2048, smpl_mean_params="data/body_models/smpl_mean_params.npz"),
    ...
    convention="smpl_49",
    loss_keypoints3d=dict(type="KeypointMSELoss", loss_weight=300, keypoint_weight=keypoint_weight),
    loss_keypoints2d=dict(type="KeypointMSELoss", loss_weight=300, keypoint_weight=keypoint_weight),
    loss_smpl_pose=dict(
        type="BMCLossMD", init_noise_sigma=1, all_gather=True, loss_mse_weight=60, loss_debias_weight=5
    ),
    loss_smpl_betas=dict(type="MSELoss", loss_weight=0.06),
    loss_camera=dict(type="CameraPriorLoss", loss_weight=60),
    init_cfg=dict(type="Pretrained", checkpoint="data/pretrained_models/spin_official_nofc.pth"),
)
# dataset settings
...
test_pipeline = [
    ...
    dict(type="ImageToTensor", keys=["img"]),
    dict(type="ToTensor", keys=data_keys),
    dict(type="Collect", keys=["img", *data_keys], meta_keys=["image_path", "center", "scale", "rotation"]),
]
...
inference_pipeline = [
    ...
    dict(type="Normalize", **img_norm_cfg),
    dict(type="ImageToTensor", keys=["img"]),
    dict(type="Collect", keys=["img", "sample_idx"], meta_keys=["image_path", "center", "scale", "rotation"]),
]
