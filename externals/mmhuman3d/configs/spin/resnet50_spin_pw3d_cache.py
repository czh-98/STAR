...
registrant = dict(
    ...
    stages=[
        # stage 1
        dict(num_iter=50, fit_global_orient=True, fit_transl=True, fit_body_pose=False, fit_betas=False),
        # stage 2
        dict(num_iter=50, fit_global_orient=True, fit_transl=False, fit_body_pose=True, fit_betas=True),
    ],
...
# optimizer
optimizer = dict(backbone=dict(type="Adam", lr=3e-5), head=dict(type="Adam", lr=3e-5))
...
model = dict(
    ...
    backbone=dict(
        type="ResNet", depth=50, out_indices=[3], norm_eval=False, norm_cfg=dict(type="SyncBN", requires_grad=True)
    ),
    ...
    init_cfg=dict(type="Pretrained", checkpoint="data/pretrained/spin_pretrain.pth"),
)
# dataset settings
