...
evaluation = dict(
    ...
    body_part=[["body", "right_hand", "left_hand"], ["", "right_hand", "left_hand", "face"]],
)
...
extra_hand_model_cfg = dict(
    backbone=dict(
        type="ResNet",
        depth=18,
        init_cfg=dict(type="Pretrained", checkpoint=__hand_model_checkpoint__, prefix="backbone"),
    ),
    head=dict(
        type="ExPoseHandHead",
        num_betas=10,
        ...
        pose_param_conf=[
            dict(name="global_orient", num_angles=1, use_mean=False, rotate_axis_x=False),
            dict(name="right_hand_pose", num_angles=15, use_mean=True, rotate_axis_x=False),
        ],
        input_feat_dim=512,
        regressor_cfg=dict(layers=[1024, 1024], activ_type="ReLU", dropout=0.5, gain=0.01),
        camera_cfg=dict(pos_func="softplus", mean_scale=0.9),
        init_cfg=dict(type="Pretrained", checkpoint=__hand_model_checkpoint__, prefix="head"),
    ),
    crop_cfg=dict(
        img_res=img_res,
        scale_factor=3.0,
...
extra_face_model_cfg = dict(
    backbone=dict(
        type="ResNet",
        depth=18,
        init_cfg=dict(type="Pretrained", checkpoint=__face_model_checkpoint__, prefix="backbone"),
    ),
    head=dict(
        type="ExPoseFaceHead",
        num_betas=100,
        ...
        mean_pose_path=__mean_pose_path__,
        pose_param_conf=[
            dict(name="global_orient", num_angles=1, use_mean=False, rotate_axis_x=True),
            dict(name="jaw_pose", num_angles=1, use_mean=False, rotate_axis_x=False),
        ],
        input_feat_dim=512,
        regressor_cfg=dict(layers=[1024, 1024], activ_type="ReLU", dropout=0.5, gain=0.01),
        camera_cfg=dict(pos_func="softplus", mean_scale=8.0),
        init_cfg=dict(type="Pretrained", checkpoint=__face_model_checkpoint__, prefix="head"),
    ),
    crop_cfg=dict(
        img_res=img_res,
        scale_factor=2.0,
...
model = dict(
    ...
    backbone=dict(
        ...
        extra=hrnet_extra,
        init_cfg=dict(type="Pretrained", checkpoint=__body_model_checkpoint__, prefix="backbone"),
    ),
    head=dict(
        type="ExPoseBodyHead",
        num_betas=10,
        ...
        pose_param_conf=[
            dict(name="global_orient", num_angles=1, use_mean=False, rotate_axis_x=True),
            dict(name="body_pose", num_angles=21, use_mean=True, rotate_axis_x=False),
            dict(name="left_hand_pose", num_angles=15, use_mean=True, rotate_axis_x=False),
            dict(name="right_hand_pose", num_angles=15, use_mean=True, rotate_axis_x=False),
            dict(name="jaw_pose", num_angles=1, use_mean=False, rotate_axis_x=False),
        ],
        input_feat_dim=2048,
        regressor_cfg=dict(layers=[1024, 1024], activ_type="none", dropout=0.5, gain=0.01),
        camera_cfg=dict(pos_func="softplus", mean_scale=0.9),
        init_cfg=dict(type="Pretrained", checkpoint=__body_model_checkpoint__, prefix="head"),
    ),
