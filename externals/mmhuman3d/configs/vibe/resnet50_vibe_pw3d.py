...
# optimizer
optimizer = dict(neck=dict(type="Adam", lr=2.5e-4), head=dict(type="Adam", lr=2.5e-4))
...
model = dict(
    ...
    head=dict(
        ...
        init_cfg=dict(type="Pretrained", checkpoint="data/pretrained/spin.pth"),
    ),
...
train_adv_pipeline = [dict(type="ToTensor", keys=adv_data_keys), dict(type="Collect", keys=adv_data_keys, meta_keys=[])]
