...
class JacobianInverseKinematics:
    ...
    def __call__(self, descendants=None, gamma=1.0):
        ...
        for i in range(self.iterations):
            ...
            x += np.array(
                list(map(lambda jf, ef: linalg.lu_solve(linalg.lu_factor(jf.T.dot(jf) + d), jf.T.dot(ef)), j, e))
            )
            ...
            if self.references is not None:
                ...
                ns = np.array(
                    list(map(lambda jf: np.eye(x.shape[1]) - linalg.solve(jf.T.dot(jf) + d, jf.T.dot(jf)), j))
                )

                if self.recalculate or i == 0:
                    j2 = self.jacobian(
                        x, gp, gr, self.second_targets, self.second_descendants, self.second_tdescendants
                    )
...
class ICP:
    ...
    def jacobian(self, x, fp, fr, goal, weights, des_r, des_t):
        ...
        if self.translate:
            ...
            jt = des_t[np.newaxis, :, :, :, np.newaxis] * es[:, :, np.newaxis, np.newaxis, :].repeat(
                goal.shape[1], axis=2
            )
    ...
    def __call__(self, descendants=None, maxjoints=4, gamma=1.0, transpose=False):
        ...
        for i in range(self.iterations):
            ...
            if self.find_closest:
                mapping = np.argmin((curr[:, :, np.newaxis] - self.goal[:, np.newaxis, :]) ** 2.0, axis=2)
