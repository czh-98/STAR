...
def load_to_maya(positions, names=None, parents=None, color=None, radius=0.1, thickness=5.0):
    ...
    for i, name in enumerate(names):
        ...
        for j, attr, attr_name in zip(
            xrange(3), [point.tx, point.ty, point.tz], ["_translateX", "_translateY", "_translateZ"]
        ):
...
def load_from_maya(root, start, end):
    ...
    for i in range(start, end):
        ...
        pm.currentTime(i)
        for j in joints:
            positions[i - start, names.index(j.name())] = j.getTranslation(space="world")
...
def loop(positions, forward="z"):
    ...
    data[:, :, fid] -= trajectory
    diff = data[0] - data[-1]
    data += np.linspace(0, 1, len(data))[:, np.newaxis, np.newaxis] * diff[np.newaxis]
    data[:, :, fid] += trajectory
