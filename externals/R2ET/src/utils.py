        ...
        for ai in range(len(animations)):
            ...
            for j in range(len(parents)):
                if parents[j] != -1:
                    ...
                    lines[ai][j].set_3d_properties([animations[ai][i, j, 1], animations[ai][i, parents[j], 1]])
            changed += lines
...
def load_testdata(min_steps, max_steps, is_h36m=False):
    ...
    for test_item in test_list:
        for inp, tgt in test_item.iteritems():
            files = sorted([f for f in listdir(data_path + inp) if not f.startswith(".") and f.endswith("_seq.npy")])
            for cfile in files:
                ...
                if tgtskel.shape[0] >= min_steps + 1:
                    if not ("Claire" in inp and "Warrok" in tgt):
                        count += 1
                    inpanim, inpnames, inpftime = BVH.load(data_path + inp + "/" + cfile[:-8] + ".bvh")

                    tgtanim, tgtnames, tgtftime = BVH.load(data_path + tgt + "/" + cfile[:-8] + ".bvh")

                    gtanim = tgtanim.copy()

                    ibvh_file = open(data_path + inp + "/" + cfile[:-8] + ".bvh").read().split("JOINT")
                    ...
                    tbvh_file = open(data_path + tgt + "/" + cfile[:-8] + ".bvh").read().split("JOINT")
