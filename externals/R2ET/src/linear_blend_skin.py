...
def linear_blend_skinning(parents, quat, rest_skel, mesh_vertices, skin_weights):
    ...
    rot_mat = torch.cat([rot_mat, rest_skel[None, :, :, None].repeat(quat.shape[0], 1, 1, 1)], dim=-1)
    rot_mat = torch.cat(
        [
            rot_mat,
    ...
    root_recover_matrix = torch.cat([torch.eye(3, dtype=torch.float).cuda(quat.device), root_recover], dim=-1)
