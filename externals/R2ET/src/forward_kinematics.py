...
class FK(object):
    ...
    def transforms_blank(rotations):
        diagonal = torch.diag(torch.tensor([1.0, 1.0, 1.0, 1.0]))[None, None, :, :].cuda(rotations.device)
        ts = torch.tile(diagonal, [int(rotations.shape[0]), int(rotations.shape[1]), 1, 1])
    ...
    def transforms_local(positions, rotations):
        ...
        zeros = torch.zeros([int(transforms.shape[0]), int(transforms.shape[1]), 1, 3]).cuda(positions.device)
        ones = torch.ones([int(transforms.shape[0]), int(transforms.shape[1]), 1, 1]).cuda(positions.device)
...
if __name__ == "__main__":
    ...
    parents = torch.from_numpy(
        np.array([-1, 0, 1, 2, 3, 4, 0, 6, 7, 8, 0, 10, 11, 12, 3, 14, 15, 16, 3, 18, 19, 20])
    ).cuda()
