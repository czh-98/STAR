...
class qlinear(nn.Module):
    def __init__(self, in_channel, out_channel, bias=True):
        ...
        W_ = nn.init.normal_(torch.zeros((out_channel, in_channel), dtype=torch.float32))
...
def get_vels(input_, n_joints, dmean, dstd, omean, ostd):
    joints = torch.reshape(input_[:, :, :-4], input_.shape[:-1].as_list() + [n_joints, 3]) * dstd[None] + dmean[None]
    ...
    rotation = np.repeat(np.array([[[1.0, 0.0, 0.0, 0.0]]]), int(input_.shape[0]), axis=0).astype("float32")
    rotation = torch.from_numpy(rotation).cuda(joints.device)
    translation = np.repeat(np.array([[[0.0, 0.0, 0.0]]]), int(input_.shape[0]), axis=0).astype("float32")
    translation = torch.from_numpy(translation).cuda(joints.device)
    axis = np.repeat(np.array([[0.0, 1.0, 0.0]]), int(input_.shape[0]), axis=0).astype("float32")
    ...
    joints = torch.reshape(torch.stack(joints_list, dim=1), input_.shape[:-1].as_list() + [-1])
...
def get_wjs(local_motion, global_motion):
    ...
    rotation = np.repeat(np.array([[[1.0, 0.0, 0.0, 0.0]]]), int(local_motion.shape[0]), axis=0).astype("float32")
    rotation = torch.from_numpy(rotation).cuda(local_motion.device)
    translation = np.repeat(np.array([[[0.0, 0.0, 0.0]]]), int(local_motion.shape[0]), axis=0).astype("float32")
    translation = torch.from_numpy(translation).cuda(local_motion.device)
    axis = np.repeat(np.array([[0.0, 1.0, 0.0]]), int(local_motion.shape[0]), axis=0).astype("float32")
    ...
    wjs = torch.reshape(torch.stack(joints_list, dim=1), list(local_motion.shape[:-2]) + [-1])
    return wjs
