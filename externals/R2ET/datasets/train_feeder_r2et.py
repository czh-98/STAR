...
class Feeder(Dataset):
    ...
    def load_data(self):
        ...
        for folder_name in folders:
            ...
            for cfile in files:
                ...
                positions = np.load(join(self.data_path, folder_name, file_name + "_skel.npy"))

                # After processed (Maybe, last 4 elements are dummy values)
                sequence = np.load(join(self.data_path, folder_name, file_name + "_seq.npy"))
                ...
                quat = np.load(join(self.data_path, folder_name, file_name + "_quat.npy"))
    ...
    def __getitem__(self, indexA):
        ...
        if clocalA.shape[0] < max_len:
            clocalA = np.concatenate((clocalA, np.zeros((max_len - clocalA.shape[0], n_joints, 3))))

        cglobalA = global_i[stidx : (stidx + max_len)]
        if cglobalA.shape[0] < max_len:
            cglobalA = np.concatenate((cglobalA, np.zeros((max_len - cglobalA.shape[0], n_joints, 3))))

        cskelA = skel_i[stidx : (stidx + max_len)]
        if cskelA.shape[0] < max_len:
            cskelA = np.concatenate((cskelA, np.zeros((max_len - cskelA.shape[0], n_joints, 3))))
        ...
        if cskelB.shape[0] < max_len:
            cskelB = np.concatenate((cskelB, np.zeros((max_len - cskelB.shape[0], n_joints, 3))))
