...
class Feeder(Dataset):
    def __init__(self, data_path, stats_path, shape_path, min_steps, max_steps, is_h36m=False):
    ...
    def load_data(self):
        ...
        for test_item in test_list:
            for inp, tgt in test_item.items():
                files = sorted(
                    [f for f in listdir(join(data_path, inp)) if not f.startswith(".") and f.endswith("_seq.npy")]
                )
                for cfile in files:
                    # Put the skels at the same height as the sequence
                    tgtskel = np.load(join(data_path, tgt + "/" + cfile[:-8] + "_skel.npy"))
                    inpskel = np.load(join(data_path, inp + "/" + cfile[:-8] + "_skel.npy"))
                    if tgtskel.shape[0] >= min_steps + 1:
                        if not ("Claire" in inp and "Warrok" in tgt):
                            count += 1
                    inpanim, inpnames, inpftime = BVH.load(join(data_path, inp + "/" + cfile[:-8] + ".bvh"))

                    tgtanim, tgtnames, tgtftime = BVH.load(join(data_path, tgt + "/" + cfile[:-8] + ".bvh"))

                    gtanim = tgtanim.copy()

                    ibvh_file = open(join(data_path, inp + "/" + cfile[:-8] + ".bvh")).read().split("JOINT")
                    ...
                    tbvh_file = open(join(data_path, tgt + "/" + cfile[:-8] + ".bvh")).read().split("JOINT")
                    ...
                    inseq = np.load(join(data_path, inp + "/" + cfile[:-8] + "_seq.npy"))

                    if inseq.shape[0] < min_steps:
                        continue

                    outseq = np.load(join(data_path, tgt + "/" + cfile[:-8] + "_seq.npy"))
                    ...
                    # load quat: (T, 22, 4)
                    tgtquat = np.load(join(data_path, tgt + "/" + cfile[:-8] + "_quat.npy"))
                    inpquat = np.load(join(data_path, inp + "/" + cfile[:-8] + "_quat.npy"))
    ...
    def __getitem__(self, index):
        ...
        localA_batch = self.testlocal[index][: self.max_steps].reshape([self.max_steps, -1])
