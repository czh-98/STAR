...
class BVHData:
    ...
    def simplified_topology(self):
        ...
        self._topology = tuple([-1, 0, 1, 2, 3, 4, 0, 6, 7, 8, 0, 10, 11, 12, 3, 14, 15, 16, 3, 18, 19, 20])
    ...
    def load(self, filename, world=False):
        ...
        self.quaternions = Quaternions.from_euler(np.radians(rot_euler), order=order, world=world)

        self.rot_euler = rot_euler
        self.rot_quat = self.quaternions.qs
        self.rot_angle, self.rot_axis = self.quaternions.angle_axis()

        self.rot_axis_angle = np.tile(np.expand_dims(self.rot_angle, axis=2), (1, 1, 3)) * self.rot_axis
    ...
    def simplify_pose_structure(self):
        ...
        self.details = [i for i, name in enumerate(self.joint_names) if name not in JOINT_NAME_MIXAMO_22]
    ...
    def get_simplified_joint_position(self):
        ...
        for i, pi in enumerate(self._topology):
            ...
            joint_positions[..., i, :] = np.matmul(transform[..., pi, :, :], offsets[..., i, :, :]).squeeze()
            transform[..., i, :, :] = np.matmul(transform[..., pi, :, :].copy(), transform[..., i, :, :].copy())
