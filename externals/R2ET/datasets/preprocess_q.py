...
for data_path in data_paths:
    ...
    folders = sorted(
        [f for f in listdir(data_path) if not f.startswith(".") and not f.endswith("py") and not f.endswith("npz")]
    )
    ...
    for folder in folders:
        ...
        for cfile in files:
            ...
            if anim.positions.shape[0] > 1:
                ...
                np.save(save_path + folder + "/" + cfile[:-4] + "_skel.npy", anim.positions)
                print(anim.parents)
                print("Success.")
