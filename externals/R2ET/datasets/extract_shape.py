...
def extract_data(fbx_path, subject_name, save_path):
    ...
    rest_x, rest_y, rest_z = source_arm.data.bones[0].head_local  # the location of hips under rest pose
    ...
    for obj in scene.objects:
        if obj.type == "MESH" and not obj.name == "Cube":
            ...
            for face in bm_rest_faces_tri:
                f_verts = face.verts
                rest_faces_lst.append((f_verts[0].index, f_verts[1].index, f_verts[2].index))
    ...
    np_simplified_skinning_weights = np.zeros((len(verts), len(bvh_data.simplified_joint_names)))
    for j, name in enumerate(vgrp_label):
        bone = source_arm.data.bones[name]
        while bone.parent is not None and rm_prefix(bone.name) not in bvh_data.simplified_joint_names:
            bone = bone.parent
    ...
    np.savez(
        ...
        rest_vertices=rest_vertices_data,  # vertex
        rest_faces=rest_faces_data,  # faces
