        ...
        with torch.no_grad():
            for i, data_dummy in tqdm(enumerate(dataloader)):
                ...
                for t in range(repeat_times):
                    ...
                    if is_mm:
                        mm_motions.append({"motion": pred_motions.cpu().numpy(), "length": m_lens[0].item()})
                if is_mm:
    ...
    def __getitem__(self, item):
        ...
        if m_length < self.opt.max_motion_length:
            motion = np.concatenate(
                [motion, np.zeros((self.opt.max_motion_length - m_length, motion.shape[1]))], axis=0
            )
...
def get_dataset_motion_loader(opt_path, batch_size, device):
    ...
    if opt.dataset_name == "t2m" or opt.dataset_name == "kit":
        ...
        dataloader = DataLoader(
            dataset, batch_size=batch_size, num_workers=4, drop_last=True, collate_fn=collate_fn, shuffle=True
        )
    else:
    ...
    def __getitem__(self, item):
        ...
        for mm_motion in mm_motions:
            ...
            if len(motion) < self.opt.max_motion_length:
                motion = np.concatenate(
                    [motion, np.zeros((self.opt.max_motion_length - len(motion), motion.shape[1]))], axis=0
                )
            motion = motion[None, :]
...
def build_models(opt):
    ...
    checkpoint = torch.load(
        pjoin("data/pretrained_models", opt.dataset_name, "text_mot_match", "model", "finest.tar"),
