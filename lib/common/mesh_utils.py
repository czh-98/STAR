...
def getRotMat(axis, theta):
    ...
    rot = math.cos(theta) * np.identity(3) + math.sin(theta) * cprod + (1 - math.cos(theta)) * np.outer(axis, axis)
    return rot
...
def psScreenshot(
    ...
    if save_base == True:
        ps_mesh = ps.register_surface_mesh(
            "mesh", vertices, faces, enabled=True, edge_color=edge_color, edge_width=edge_width, material=material
        )
    ...
    for i in range(len(angles)):
        ...
        ps_mesh = ps.register_surface_mesh(
            "mesh", rot_verts, faces, enabled=True, edge_color=edge_color, edge_width=edge_width, material=material
        )
        if scalars is not None:
            ps_mesh.add_scalar_quantity(
                f"scalar", scalars, defined_on=defined_on, cmap=cmap, enabled=True, vminmax=sminmax
            )
        ...
        if highlight_faces is not None:
            ...
            ps_curve = ps.register_curve_network(
                "curve", curve_v, curve_edges, color=highlight_color, radius=highlight_radius
            )
