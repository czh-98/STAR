...
class SMPLXSeg:
    ...
    hands_ids = (
        smplx_segs["leftHand"] + smplx_segs["rightHand"] + smplx_segs["leftHandIndex1"] + smplx_segs["rightHandIndex1"]
    )
    ...
    check_ids = list(set(front_face_ids) - set(forehead_ids) - set(lips_ids) - set(nose_ids) - set(eyes_ids))

    leye_ids = list(smplx_flame_vid[flame_segs["left_eye_region"]])
    reye_ids = list(smplx_flame_vid[flame_segs["right_eye_region"]])

    leyeball_ids = smplx_segs["leftEye"]
    reyeball_ids = smplx_segs["rightEye"]
