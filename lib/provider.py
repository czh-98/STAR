...
class ViewDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, opt, device, type="train", size=100):
        ...
        self.to_tensor = transforms.Compose([transforms.Resize((self.W, self.H)), transforms.ToTensor()])
    ...
    def get_frontal_view_data(self, is_train=True):
        ...
        if is_train:
            # random frontal rendering
            radius_range = [1.0, 1.2]

            radius_range = np.array(radius_range) * 1.0  # body_scale
            radius = np.random.rand() * (radius_range[1] - radius_range[0]) + radius_range[0]

            theta_range = [60, 90]
            theta_range = np.array(theta_range)

            thetas = np.random.rand() * (theta_range[1] - theta_range[0]) + theta_range[0]

            # phi_range = [-180, 180]
            phi_range = [-45, 45]
            phi_range = np.array(phi_range)

            phis = np.random.rand() * (phi_range[1] - phi_range[0]) + phi_range[0]
